...
: CREATEMM ALIGNED ( make size a multiple of 4 )
...
: RESIZE OVER HANDLEVALID CALL
         IF
            ...
            UNUSED CALL -4 AND MIN 2/ 2/ A:R> A:R1@
            HERE @ ALIGNED
            R> CELL+ @
            R@ MOVEBLOCKUP CALL DROP ( copy record on heap )
            ...
            SWAP 0<> OVER 0<> AND IF DUP CELL+ @ HERE @ ALIGNED R@ MOVEBLOCKUP CALL DROP ENDIF A:DROP ( fill allocated block )
            0=
            A:EI
