...
: F* A:R@ DROP A:R@ A:000fffffH A:AND ( save mantissa of multiplicator )
     ...
     3ff00000H >! NIP IF DROP A:2* 0ffe00000H 1 RSHIFTC 0H A:DROP A:2DROP A:2DROP EXIT ENDIF ( handle overflow )
     0<! IF DROP R@ 0 A:DROP A:2DROP A:2DROP EXIT ENDIF ( handle underflow!!!!!!! )
...
: F/ OVER 2* OVER OR 0=
     IF DROP NIP XOR 7fffffffH OR -1 EXIT ENDIF ( zero divide )
     OVER DUP
     000fffffH AND ( pure mantissa of upper half )
     OVER 7ff00000H AND
     IF 100000H OR ENDIF ( insert leading 1 into upper half of mantissa )
     NIP A:R@ A:R1@ 2DROP ( move mantissa to alternative stack )
     A:R@ DROP ( move exponent )
     ...
     R@ R1@ A:2DROP ( move dividend back )
     
     R@ 2* 21 RSHIFT
     R1@ 2* 21 RSHIFT
     - 3ffH + ( exponent1 - exponent2 )
     10000H + ( make high half word positive )
     A:XOR A:80000000H A:AND ( sign of new exponent )
     A:R@ DROP A:OR ( sign, raw new exponent )

     BEGIN OVER OVER R@ A:DROP R1@ A:R1@ NIP R1@ D< INVERT WHILE R@ R1@ A:2DROP 2* A:DUP A:+B A:R@ A:R1@ 2DROP A:1+ REPEAT ( make dividend < divisor )

     R@ 1ffffH AND
     107feH >! NIP IF 2DROP 2DROP DROP A:2* 0ffe00000H 1 RSHIFTC 0 A:2DROP A:DROP EXIT ENDIF ( overflow )
     0ffcbH <! NIP IF 2DROP 2DROP DROP A:80000000H A:AND R@ 0 A:2DROP A:DROP EXIT ENDIF ( underflow )
     10000H <! NIP IF A:80000000H A:AND 0ffff0000H OR ELSE DUP - ENDIF ( factor, minors loop counts )
     55 + ( loop count )

     A:800007ffH A:AND ( sign, new exponent )
     R@ A:DROP R1@ A:NIP A:R1@ NIP R1@ A:NIP ( move divisor )
     A:R@ DROP A:R1@ NIP R1@ A:NIP ( move count )

     A:BEGIN A:0>! WHILE A:1- A:R1@ A:R@ D- B@ INVERT F/1 0BRANCH
                         F/2 LABEL 2* A:R@ DROP DUP +B A:R@ DROP 2* A:R@ DROP DUP +B R@ 1+ A:DROP R@ R1@ A:2DROP R1@ R@ A:2DROP REPEAT
     A:BEGIN A:0>! WHILE A:1- A:R1@ A:R@ D+ B@ INVERT F/2 0BRANCH
                         F/1 LABEL 2* A:R@ DROP DUP +B A:R@ DROP 2* A:R@ DROP DUP +B R@ A:DROP R@ R1@ A:2DROP R1@ R@ A:2DROP REPEAT

     A:DROP 2DROP 2DROP ( remove garbage )
     A:R1@ A:R@
     A:R@ DROP 2/ A:1 A:RSHIFTC A:0 A:+B 0 +B R@ A:DROP ( round quotient )
     
     OVER 300000H AND 100000H =
     IF A:2DROP A:1- ( adjust exponent, because quotient < 0.0 ) ELSE 2DROP R1@ A:2 A:+ 0 +B 2/ A:1 A:RSHIFTC 2/ R@ A:2DROP 1 RSHIFTC ENDIF ( adjust quotient )
     A:R@ DROP
     0fffffH AND
     R1@ 21 LSHIFT R1@ 2* DROP 1 RSHIFTC OR ( upper word of mantissa with exponent )
     R@ A:2DROP ;
