...
: UD/DMOD 3 PICK 3 PICK 3 PICK 3 PICK UD/DIV CALL 
          A:R1@ A:R@
			 D* CALL D-                       ( remainder )
			 R1@ R@ A:2DROP                   ( quotient )
			 ;

( divide unsigned double integers
  parameter
      64-bit divisor
      64-bit dividend
  return
...
:LOCAL UD/DIV A:DI OVER OVER 1 U> OR ( d > 1 )
              IF
				     A:1 A:R1@ A:0<> A:32 A:AND
					  R@ IF OVER OVER 0<> - ELSE DUP ENDIF
					  A:+
					  A:R@ A:DUP A:1- A:AND A:0= A:+ ( adjust to nearest power of 2 )
					  A:16 0
					  A:BEGIN
					     OVER OVER 1 OVER R@ + LSHIFT NIP U< INVERT
						  IF R@ + ENDIF
						  A:2/ A:0=!
					  UNTIL
					  A:DROP A:R@ A:+            ( power calculated )
					  2DROP
					  64 R@ - SHIFTL CALL ( align d )
					  OVER OVER OR
					  IF
					     OVER OVER
					     7FFFFFFFFFFFFFFFH D- DNEGATE  ( initialize result )
						  5 0                           ( logarithmic convergence, starting with 6 bit accuracy in first step )
						  DO
						     OVER OVER 64! 3 PICK 3 PICK 64! 32@ 32@ 32@ 32@ 2DROP DNEGATE
						     64! 64! 32@ 32@ 32@ 32@ 2DROP
						     OVER OVER D+
						  LOOP
						  OVER OVER D+
						  0 3
						  ...
						  BEGIN
						     D-
						     OVER OVER 64! 3 PICK 3 PICK 64! 32@ 32@ 32@ 32@ 2DROP
							  5 PICK 5 PICK D+
							  OVER 0< INVERT
						  UNTIL
						  2DROP
				     ...
				     ( divide )
			        64!
			        OVER OVER 64! 32@ 32@ 32@ 32@ 2DROP A:R1@ A:R@
			        D- >R 1 RSHIFT R> 1 RSHIFTC R1@ R@ A:2DROP D+
			        R> 1- SHIFTR CALL
				  ELSE
				     NIP 0= IF 2DROP -1 1 RSHIFT -1 ENDIF
				  ENDIF
			     A:EI ;
...
:LOCAL UM* A:DI 0 OVER 64! DROP 0 OVER 64! DROP 32@ 32@ 2DROP 32@ 32@ A:EI ;
