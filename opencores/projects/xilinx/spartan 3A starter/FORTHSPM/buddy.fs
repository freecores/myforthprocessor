...
: HANDLEVALID 1 LOCKI/O CALL MMBASE @ -
...
: RESIZE OVER HANDLEVALID CALL
         IF
            ...
            UNUSED CALL -4 AND MIN 2/ 2/ >R
            OVER CELL+ @ ( data array )
            HERE @ A:R@ ALIGNED
				R1@ 2* 2* 3 + ALLOT  ( allocate scratch on heap )
            R1@ MOVEBLOCKUP CALL ( copy record on heap )
            A:R1@ A:@ ( copy reference counter )
            OVER WHITE OVER ! FREE CALL DROP ( free record )
            ...
            SWAP 0<> OVER 0<> AND IF R@ ALIGNED OVER CELL+ @ R1@ MOVEBLOCKUP CALL ENDIF ( fill allocated block )
				A:HERE A:! ( remove scratch )
				A:DROP
            0=
            A:EI
