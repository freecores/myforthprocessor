...
DEFINE INCCONSOLE 28
DEFINE DECCONSOLE 29
...
: LOCKI/O A:DI LOCKEDI/O OVER OVER @ + A:R@ OVER ! 2DROP
          A:0= A:TIMERCT A:@ A:AND
			 A:IF TESTTASKSWITCH CALL ENDIF
			 A:EI ;
...
:LOCAL TESTTASKSWITCH  JAVATIMER @ 0<>!
                       IF
							    0 TIMERCT !
							    DUP -1 EXECUTE CALL ( execute java timer primitive )
							  ENDIF
							  DROP ;
...
: CLOSE-FILE 1 LOCKI/O CALL
             A:DI 1 EI 15 OUTQUEUEAPPENDRAWCHAR CALL ( API )
...
:LOCAL OPEN_ 1 LOCKI/O CALL
             A:DI 1 EI A:R@ DROP	( move operation )
             ALLOCATEHANDLE CALL
...
:LOCAL READ-BUFFER 1 LOCKI/O CALL
                   A:DI >R HERE @ ALIGNED R@ 0F0H SETBUFFER CALL >R
                   15 OUTQUEUEAPPENDRAWCHAR CALL ( API )
...
:LOCAL SENDWORD 1 LOCKI/O CALL
                DUP OUTQUEUEAPPENDMASKEDCHAR CALL ( low word )
...
: REPOSITION-FILE 1 LOCKI/O CALL
                  A:DI 1 EI 15 OUTQUEUEAPPENDRAWCHAR CALL ( API )
...
: WRITE-FILE 1 LOCKI/O CALL
             A:DI 1 EI 2/ 2/ 2/ 2/ OUTQUEUEAPPENDRAWCHAR CALL ( fileID )
...
:LOCAL ABSOLUTE-RELATIVE 1 LOCKI/O CALL
                         A:DI 1 EI 15 OUTQUEUEAPPENDRAWCHAR CALL ( API )
...
: ADDCONSOLE   1 LOCKI/O CALL
               A:DI 1 EI 15 OUTQUEUEAPPENDRAWCHAR CALL ( API )
               INCCONSOLE OUTQUEUEAPPENDMASKEDCHAR CALL ( get current file date )
               26 OUTQUEUEAPPENDRAWCHAR CALL
               A:EI
               -1 LOCKI/O CALL ;
...
: SUBCONSOLE   1 LOCKI/O CALL
               A:DI 1 EI 15 OUTQUEUEAPPENDRAWCHAR CALL ( API )
               DECCONSOLE OUTQUEUEAPPENDMASKEDCHAR CALL ( get current file date )
               26 OUTQUEUEAPPENDRAWCHAR CALL
               A:EI
               -1 LOCKI/O CALL ;
