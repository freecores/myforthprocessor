...
public class Code3color
 {
   ...
   private static String toAppend;
    ...
    public static String getOffset(VariableType x)
     {
      return (x.offset + Code3color.area) + " + ";
     }
    ...
    * @return String
    */
   static String toLocal(Type x, boolean dup)
    {
     int count = 0;

     for(int i = 0; i < locals.size(); i++)
      {
       Type t = (Type)locals.get(i);

       if (t.dim > 0 || t.type == Keyword.NONESY)
        count += 4;
      }

     String s = "";
     if (x.dim > 0 || x.type == Keyword.NONESY)
      if (count == 0)
       s = (dup?"DUP ":"") + "§tempvar V! ";
      else
       s = (dup?"DUP ":"") + "§tempvar " + count + " + V! ";
     else
      s += (dup?"DUP ":"") + "TO " + locals.size() + "§ ";
       ...
       if (y instanceof VariableType && (y.modify & Keyword.STATICSY.value) != 0)
        {
         ...
         if (v.type.dim == 0 && v.type.type != Keyword.NONESY)
          if (v.type.type == Keyword.LONGSY || v.type.type == Keyword.FLOATSY || v.type.type == Keyword.DOUBLESY)
           s = '2' + s;
...
/*     MethodType [] m = null;
     ...
     if (t.type == Keyword.NONESY && t.dim == 0)
      {
       m = getMethod("~destructor", null, null, findClass(t.ident.string, t.version));
       if (m.length > 0 && (m[0].modify & Keyword.STATICSY.value) != 0)
        x = header + "DUP HANDLEVALID IF " + buildQualifiedName(m[0].scope, m[0], false, "") + "DUP ENDIF DROP ";
      }

     if (x.length() == 0)
      if (t.type == Keyword.NONESY || t.dim > 0)
       {
        m = getMethod("kill", null, null, findClass("JavaArray", 0));
        x = header + "TRUE " + buildQualifiedName(m[0].scope, m[0], false, "");
       }
*/
     return x;
    }
    ...
    * @param norhs store in dummy variable too
   ...
   private static int traverse(Operation root, boolean lhs, boolean rhs, boolean statement, Operation preorder,
                               boolean skipleft, boolean norhs) throws Exception
    {
     String label = null;
     boolean added = false;
     int modify = 0;
     ...
     if (root.operator == Keyword.LEAFSY)
      {
       // primary, selector
       modify = primarySelector(root.left, lhs, rhs, preorder, norhs);
     ...
     else if (Parser.assign.contains(root.operator))
      {
       ...
       if (root.right.operator == Keyword.LBRACESY)
        arrayInitializer(root.right, root.left.type, true, true);
       else
       ...
       if (root.left.type.type == Keyword.NONESY || root.left.type.dim > 0)
        {
         ...
         //if (modify >= 0)
         // root.code += (root.left.name == null?"DUP ":"") +
         //              (member == null || member.name.string.compareTo("~destructor") != 0?
       ...
       if (root.left.name != null)
       ...
       else
        {
         ...
         //pass.warning((leftcode.compareTo(rightcode) == 0) + ":::" + leftcode + ":::" + rightcode);
        }
     ...
     else if (root.operator == Keyword.PLUSSY)
      {
       ...
       if (left != null && right != null && (dest.type != Keyword.NONESY || dest.ident.string.compareTo("String") == 0))
       ...
       else if (root.left.type.dim == 0 && root.left.type.type == Keyword.NONESY && root.left.type.ident.string.endsWith("String"))
        {
         ...
         root.code += incrementReference("DUP") + " SWAP " + Find.buildQualifiedName(m[0].scope, m[0], false, "") + (!rhs?toLocal(((MethodType)m[0]).type, true):"");
        }
       else
     ...
     else if (root.operator == Keyword.QUESTIONMARKSY)
      {
       ...
       root.left.code += "\nIF\n";
       int a = traverse(root.right.left, false, rhs, false, preorder, false, norhs);
       int b = traverse(root.right.right, false, rhs, false, preorder, false, norhs);
     ...
     else if (root.operator == Keyword.ALLOCATESY)
      {
       ...
       root.code = (theClass.scope.offset + Code3color.area) +
                   " MALLOC DROP DUP " + toVariable("§this"); // allocate class storage
       root.code += "OVER IF OVER CELL+ @ " + Find.addOffset(Code3color.son) + "OVER OVER ! DROP ENDIF\n" + // store $this as son in superclass
                    "CELL+ @ OVER\n" +
                    "OVER " + Find.addOffset(Code3color.father) + "! " + "NIP\n"; // store father in object
       if (Find.theScope.get("§outer").length != 0)
        root.code += "§outer " + incrementReference("DUP") + "OVER " + Find.addOffset(Code3color.outer) + "!\n"; // store outer in object
       root.code += Find.buildQualifiedName(theClass.scope.prev, theClass, false, "").trim() + ".table OVER " + Find.addOffset(Code3color.table) + "!\n";
       root.code += Find.crc(member.type) + " OVER " + Find.addOffset(Code3color.coded) + "!\n"; // store code
       root.code += "\" " + theClass.name.string.substring(theClass.name.string.lastIndexOf('.') + 1) + " \" OVER " + Find.addOffset(Code3color.name) + "!\n"; // store class name
       root.code += theClass.scope.refcount + " OVER " + Find.addOffset(Code3color.refcount) + "! DROP\n"; // store code
      }
     else if (root.operator == Keyword.RETURNSY)
      {
       if (root.left != null)
        {
         if ((traverse(root.left, false, true, false, preorder, false, false) & (ISTHIS | LOADED)) != 0)
          if (root.left.type.dim != 0 || root.left.type.type == Keyword.NONESY)
    ...
    *         bit 30 a variable read out
    */
   private static int primarySelector(Operation root, boolean lhs, boolean rhs, Operation preorder, boolean norhs) throws Exception
    {
     ...
     if (ptr.operator == Keyword.LPARSY)
       ...
       if (type.type == Keyword.NONESY && type.ident.string.compareTo("String") == 0)
        {
         ...
         if (!rhsm || norhs)
          {
           ptr.code += toLocal(new Type(type), true);
          }
      ...
      { //new
       ...
       if (ptr.left.operator == Keyword.LBRACESY)
        { // array initializer
         arrayInitializer(ptr.left, type, rhsm, true);
        }
     ...
     else if (ptr.operator == Keyword.SUPERSY)
      {
       ...
       if (ptr.left != null && ptr.left.operator == Keyword.LPARSY)
        ...
        {
         trailer += "§this CELL+ " + Find.addOffset(Code3color.father);
         isSuper = true;
        }
           ...
           if (s.length == 1 && Find.theScope.buildPath("").indexOf(qualified) >= 0)
            {
             ...
             for(String fetch = ""; y != null && (y.prefix == null || tail.compareTo(y.prefix) != 0); y = y.prev)
              if (y.prefix != null && y.prefix.length() > 0)
               {
                trailer += fetch + "CELL+ @ " + Find.addOffset(Code3color.outer);
                fetch = "@ ";
               }
        ...
        if (type.type == Keyword.NONESY && myClass.extend != null && myClass.extend.name != null)
         {
          ...
          trailer += "CELL+ " + Find.addOffset(Code3color.father);
          loaded = false;
          qualified = "";
          isSuper = true;
         }
       ...
       else
        {
         ...
         if (ptr.left != null && ptr.left.operator == Keyword.LPARSY)
          { // method
           ...
           MethodType z = methodCall(ptr, c[0], pushed, trailer, isSuper, rhsm, preorder, norhs);
       ...
       if (ptr.operator == Keyword.IDENTSY)
        {
         ...
         if (ptr.left != null && ptr.left.operator == Keyword.LPARSY)
          { // method
           ...
           MethodType z = methodCall(ptr, myClass, pushed, trailer, isSuper, rhsm, preorder, norhs);
         ...
         if (preorder != null && l)
          {
           ...
           op.code += toLocal(new Type(op.type), false);
           ptr.left.name = null;
  ...
  private static void arrayCreator(Operation ptr, boolean rhs) throws Exception
   {
    ...
    String dup = toLocal(new Type(ptr.type), true), di = "A:DI ";
    ...
    for (Operation n = ptr.left; n != null; n = n.right)
     {
      ...
      if (n.right != null)
       {
        n.left.code += di + "A:R@ " + Find.crc(t) + routine + dup + "R> 0\nDO\n";
    ...
    if (!rhs)
     {
      ptr.code += toLocal(new Type(ptr.type), true);
     }
    if (di.length() == 0)
     ptr.code += "A:EI ";
   }
    ...
    if (p.length == 0)
     ...
     { // new class instance
      ...
      for(int i = 0; i < z.parameter.length && q != null; i++, q = q.right)
       //if (i != 0 || z.parameter[i].name.string.compareTo("§this") != 0)
   ...
   * @throws CodeException
   */
  private static MethodType methodCall(Operation ptr, ClassType c, Type pushed, String trailer, boolean isSuper,
                                       boolean rhs, Operation preorder, boolean norhs) throws Exception
   {
    ...
    while(c != null)
     {
      ...
      a[0] = null;
      trailer += "CELL+ @ " + Find.addOffset(Code3color.outer) + "@ ";
    ...
    else
     {
      ...
      if (!rhs || norhs)
       if (z.type.dim != 0 || z.type.type == Keyword.NONESY)
        {
         ptr.code += toLocal(new Type(z.type), true);
        }
     }
  ...
  private static Type [] arguments(Operation root, boolean skip, Operation preorder) throws Exception
   {
    ...
    for(Operation y = root.left; y != null; y = y.right)
     {
      if ((traverse(y.left, false, true, false, preorder, false, true) & LOADED) != 0 && !skip && (y.left.type.dim > 0 || y.left.type.type == Keyword.NONESY))
    ...
    while(source.prev != null)
     {
      ...
      if (outer.length() > 0)
       return x + Find.addOffset(Code3color.father) + "@ " + outer;
      x += Find.addOffset(Code3color.outer) + "@ ";
      source = source.prev;
      while(source.prev != null && (source.prefix == null || source.prefix.length() == 0))
       source = source.prev;
     }
   ...
   */
  private static void arrayInitializer(Operation root, Type t, boolean rhs, boolean norhs) throws Exception
   {
    ClassType [] c;
    MethodType [] m = Find.getMethod("JavaArray", null, null, c = findClass("JavaArray", 0));
    String routine = ' ' + Find.buildQualifiedName(m[1].scope.prev.prev, c[0], false, "").trim() + ".table " + (m[1].version & -4) + " EXECUTE-NEW ";
    ...
    if (t1.dim > 0 || t1.type == Keyword.NONESY)
     store = toLocal(t1, true) + " ! ";
    else if (t1.type == Keyword.BYTESY)
      {
       store = "C! ";
      }
    ...
    if (!rhs || norhs)
     {
      root.left.code += toLocal(new Type(t), true);
     }
    ...
    for(Operation ptr = root.right; ptr != null; ptr = ptr.right, i++)
     {
      ...
      if (ptr.left.operator == Keyword.LBRACESY)
       arrayInitializer(ptr.left, t1, true, false);
      else
    ...
    if (dest.dim - source.dim == 1 && (source.dim > 0 || source.type == Keyword.NONESY))
     ...
     if (source.type == Keyword.VOIDSY ||
     ...
     else if (dest.ident.string.compareTo("String") == 0)
      {
       ...
       String append = toLocal(new Type(dest), true);
...
class CodeException extends Exception
 {
  ...
  CodeException(String x){super(x);}
 }
