...
# Run in the directory you wish to have Java(tm) Web Start installed.
...
createCFGForJS() {
    ...
    echo "javaws.cfg.jre.0.platform=1.3" >> ${INSTALLDIR}/javaws.cfg
    echo "javaws.cfg.jre.0.product=1.3.0" >> ${INSTALLDIR}/javaws.cfg
    echo "javaws.cfg.jre.0.path=/usr/local/java/jdk1.3/$SOLARIS_DIR/bin/java" >> ${INSTALLDIR}/javaws.cfg

    if [ -f /usr/local/java/jdk1.2.2/$SOLARIS_DIR/bin/java ]; then
      echo "javaws.cfg.jre.1.platform=1.2" >> ${INSTALLDIR}/javaws.cfg
      echo "javaws.cfg.jre.1.product=1.2.2" >> ${INSTALLDIR}/javaws.cfg
      echo "javaws.cfg.jre.1.path=/usr/local/java/jdk1.2.2/$SOLARIS_DIR/bin/java" >>\
           ${INSTALLDIR}/javaws.cfg
    fi
}
...
EOF
    ...
    echo 'Creating javaws.cfg...'
    echo "javaws.cfg.jre.0.platform=${version}" > ${INSTALLDIR}/javaws.cfg
    echo "javaws.cfg.jre.0.product=${version}" >> ${INSTALLDIR}/javaws.cfg
    echo "javaws.cfg.jre.0.path=${javaPath}/bin/java" >> ${INSTALLDIR}/javaws.cfg
    echo "javaws.cfg.jre.default = 0" >> ${INSTALLDIR}/javaws.cfg
}

# Create the actions for dt, creating a link in the users home directory
createDTActions() {
    echo 'Updating action...'
    sed "s@EXEC_STRING@EXEC_STRING ${INSTALLDIR}/javaws %Arg_1%@" \
	${INSTALLDIR}/javaws_launcher.dt > ${INSTALLDIR}/temp
    rm -f ${INSTALLDIR}/javaws_launcher.dt
    mv ${INSTALLDIR}/temp ${INSTALLDIR}/javaws_launcher.dt
    if [ ! -f ${HOME}/.dt/types/javaws_launcher.dt -a ! -h ${HOME}/.dt/types/javaws_launcher.dt ]; then
      if [ ! -d ${HOME}/.dt/types ]; then
	mkdir -p ${HOME}/.dt/types
      fi
      ln -s ${INSTALLDIR}/javaws_launcher.dt ${HOME}/.dt/types
    fi
}
...
updateMailcap() {
    ...
    if [ -f ${HOME}/.mailcap ]; then
      ...
      echo "\n# Java Web Start\n${MIME_TYPE}; $INSTALLDIR/javaws %s" >> \
       ${HOME}/.mailcap
    else
      # .mailcap doesn't exist, create it.
      echo "# Java Web Start\n${MIME_TYPE}; $INSTALLDIR/javaws %s" > ${HOME}/.mailcap
    fi
}
...
if [ -d /usr/local/java/jdk1.3/$SOLARIS_DIR ]; then
  createCFGForJS
else
  createCFGFromUser
fi
