...
static jmethodID g_ojiplugin_getjavaobject;
...
initialize_globals(JNIEnv* m_env) {
  ...
  if(getenv("JAVA_PLUGIN_TRACE")) {
    ...
    g_ojiplugin_getjavaobject = 
      wrapGetStaticMethodID(m_env, g_ojiplugin_class,
			    "getJavaObject", "(I)Ljava/lang/Object;");

    g_jclass_Class      = wrapFindClassGlobal(m_env,"java/lang/Class");
    ...
    g_jclass_Void       = wrapFindClassGlobal(m_env,"java/lang/Void");
...
#define DEFINE_NEWSCALARARRAY(Result, tag)				\
    ...
    case JAVA_PLUGIN_GET_JAVA_OBJECT:
      {
	jint plugin_number = (jint) get_bits32(pipe);
	jobject res =
	  (*env)->CallStaticObjectMethod(env, g_ojiplugin_class, 
					 g_ojiplugin_getjavaobject,
					 plugin_number); 
	jvalue val;
	val.l = res;
	send_OK_val_of_type(env, pipe, jobject_type, &val);
	break;
      }
    default:
...
/* Should use a union, but this is simpler to debug for now */
typedef struct ReturnValue_Struct {
    jobject o;
    jstring s;
    int i;
} ReturnValue;
...
/*
  ...
  8] a ctx
  */
SendJNIJSMessage(JNIEnv* env, 
		 int code, int index,  int slotindex,
		 int use_utfstr, jstring jstrAsUTF, 
		 int use_charstr, jstring jstrAsChars, 
		 jobjectArray jarray,
		 jobject value, 
		 jboolean ctx,
		 ReturnType ret_type,
		 ReturnValue* retval)
{
    ...
    const jchar* strAsChars = NULL;
    int strAsChars_len, strAsChars_sz;
    int header_code = JAVA_PLUGIN_REQUEST;
    const char* strAsUTF = NULL;
    int strAsUTF_len;
    int args_len;
    int value_sz, code_sz, index_sz, slotindex_sz, var_sz, arr_sz, ctx_sz;
    int mesg_sz;
    int mesg_size_sz;
    int non_header_sz;
    char* mesg;
    ...
    if (use_charstr) {
	strAsChars = (*env)->GetStringChars(env, jstrAsChars, NULL);
	strAsChars_len = (*env)->GetStringLength(env, jstrAsChars);
	strAsChars_sz = sizeof(jchar) * strAsChars_len;
#ifndef NO_DEBUG	
      {
	int i;
	const char *debugstrAsUTFChars = 
	  (*env)->GetStringUTFChars(env, jstrAsChars, NULL);
	native_trace("NORMAL STR AS UTF str=\n\t%s\n", 
		debugstrAsUTFChars);
	/*
	for(i = 0; i < strAsChars_len; i++) 
	  fprintf(stderr," server: char %d is %d\n", i, (int) strAsChars[i]);
	*/
      }
#endif       
    ...
    if (use_utfstr) {
	strAsUTF = (*env)->GetStringUTFChars(env, jstrAsUTF, NULL);
	strAsUTF_len = (*env)->GetStringUTFLength(env, jstrAsUTF);
    ...
    /* First the code indicating this request type */
    memcpy(mesg + ci, &code, code_sz );  ci += code_sz;

    /* Then the actual message */
    memcpy(mesg + ci, &index, index_sz); ci += index_sz;
    memcpy(mesg + ci, &slotindex, index_sz); ci += index_sz;
    memcpy(mesg + ci, &strAsUTF_len, var_sz); ci += var_sz;
    if (strAsUTF_len > 0) {
	memcpy(mesg + ci, strAsUTF, strAsUTF_len );    
	ci += strAsUTF_len;
    }
    memcpy(mesg + ci, &strAsChars_len, var_sz); ci += var_sz;
    memcpy(mesg + ci, &strAsChars_sz, var_sz);  ci += var_sz;
    if (strAsChars_sz > 0) {
	memcpy(mesg + ci, strAsChars, strAsChars_sz);   ci += strAsChars_sz;
    }
    memcpy(mesg + ci, &jarray, arr_sz);   ci += arr_sz;
    memcpy(mesg + ci, &value, value_sz);   ci += value_sz;
    ...
    native_trace(">>>>>|||||SendJNIJSMessage(): size=%d code=%X\n index=%d"
	    " slotindex=%d strAsUTF_len=%d\n"
	    " strAsChars_sz=%d value=%d\n"
	    " jarray=%d ctx=%d\n",
	    mesg_sz, code, index,
	    slotindex, strAsUTF_len,
	    strAsChars_sz, (int) value,
	    (int) jarray, (int) ctx);

    /* Wait for a response */
    handleJNIJSResponse(env, pipe, ret_type, retval);
    ...
    if (use_utfstr) (*env)->ReleaseStringUTFChars(env, jstrAsUTF, strAsUTF);
    if (use_charstr) (*env)->ReleaseStringChars(env, jstrAsChars, strAsChars);
}
...
Java_sun_plugin_navig_motif_OJIPlugin_convertToGlobalRef
 ...
 * Signature: (ILjava/lang/String;[[B[IILjava/security/AccessControlContext;)I
 */
JNIEXPORT jint JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSGetNativeJSObject
(JNIEnv *env, jclass jsClass, jint plugin_index, jstring jurl,
 jboolean ctx)
{
    ReturnValue res;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_GET_NATIVE, plugin_index, 0,
		     1, jurl,
		     0, NULL,
		     dummyArrArg,
		     dummyObjArg,
...
Java_sun_plugin_javascript_navig5_JSObject_JSObjectToString
(JNIEnv *env, jclass clazz, jint jnsILiveConnect, jint nativeJSObject)
{
    /* Convert the JSObject to its string representation */
    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_TOSTRING, nativeJSObject, 0,
		     0, NULL,
		     0, NULL,
		     dummyArrArg,
		     dummyObjArg,
 ...
 */
JNIEXPORT void JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSFinalize
(JNIEnv *env, jclass jsClass, jint jnsILiveConnect, jint nativeJSObject)
{

    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_FINALIZE, nativeJSObject, 0,
		     0, NULL,
		     0, NULL,
		     dummyArrArg,
		     dummyObjArg,
 ...
 * Class:     sun_plugin_javascript_navig5_JSObject
 * Method:    JSObjectCall
 * Signature: (IILjava/lang/String;Ljava/lang/String;[Ljava/lang/Object;Z)Ljava/lang/Object;
 */
JNIEXPORT jobject JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSObjectCall
(JNIEnv *env, jclass jsClass, jint jnsILiveConnect, 
 jint nativeJSObject, jstring jurl, 
 jstring methodName, jobjectArray args, jboolean ctx)
{
    jobject dummy;
    ReturnValue retval;
    
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_CALL, nativeJSObject, 0,
		     1, jurl,
		     1, methodName,
		     args,
		     dummy,
 ...
 * Class:     sun_plugin_javascript_navig5_JSObject
 * Method:    JSObjectEval
 * Signature: (IILjava/lang/String;[[B[IILjava/lang/String;
 *   Ljava/security/AccessControlContext;)Ljava/lang/Object;
 */
JNIEXPORT jobject JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSObjectEval
  (JNIEnv *env, jclass jsClass, jint jnsILiveConnect, 
   jint nativeJSObject, 
   jstring jurl, 
   jstring jscript, 
   jboolean ctx)
{
    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_EVAL, nativeJSObject, 0,
		     1, jurl,
		     1, jscript,
		     dummyArrArg,
		     dummyObjArg,
 ...
 *     [[B[IILjava/lang/String;Ljava/security/AccessControlContext;)
 *  Ljava/lang/Object;
 */
JNIEXPORT jobject JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSObjectGetMember
  (JNIEnv *env, jclass jsClass, jint jnsILiveConnect, jint nativeJSObject, 
   jstring jurl, 
   jstring jname, jboolean ctx)
{
    jobject dummy;
    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_GETMEMBER, nativeJSObject, 0,
		     1, jurl,
		     1, jname,
		     dummyArrArg,
		     dummyObjArg,
 ...
 * Signature: (IILjava/lang/String;
 *  [[B[IILjava/lang/String;Ljava/lang/Object;
 *  Ljava/security/AccessControlContext;)V
 */
JNIEXPORT void JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSObjectSetMember
(JNIEnv *env, jclass jsClass, jint jnsILiveConnect, jint nativeJSObject, 
 jstring jurl, 
 jstring jname, 
 jobject value, jboolean ctx)
{
    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_SETMEMBER, nativeJSObject, 0, 
		     1, jurl,
		     1, jname,
		     dummyArrArg,
		     value,
 ...
 * Signature: (IILjava/lang/String;[[B[IILjava/lang/String;Ljava/security/AccessControlContext;)V
 */
JNIEXPORT void JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSObjectRemoveMember
 ...
 jstring jname, jboolean ctx)
{
    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_REMOVEMEMBER, nativeJSObject, 0,
		     1, jurl,
		     1, jname,
		     dummyArrArg,
		     dummyObjArg,
 ...
 * Signature: (IILjava/lang/String;
 *  [[B[IIILjava/security/AccessControlContext;)Ljava/lang/Object;
 */
JNIEXPORT jobject JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSObjectGetSlot
(JNIEnv *env, jclass jsClass, jint jsTrheadId, jint nativeJSObject, 
 jstring jurl, 
 jint jindex, jboolean ctx)
{
    
    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_GETMEMBER, nativeJSObject, (int) index,
		     1, jurl,
		     0, NULL,
		     dummyArrArg,
		     dummyObjArg,
 ...
 * Signature: (IILjava/lang/String;[[B[IIILjava/lang/Object;Ljava/security/AccessControlContext;)V
 */
JNIEXPORT void JNICALL 
Java_sun_plugin_javascript_navig5_JSObject_JSObjectSetSlot
  (JNIEnv *env, jclass jsClass, jint jnsILiveConnect, jint nativeJSObject, 
   jstring jurl, 
   jint jindex, jobject value, 
   jboolean ctx)
{

    ReturnValue retval;
    SendJNIJSMessage(env,
		     JAVA_PLUGIN_JNIJS_SETSLOT, nativeJSObject, (int) index,
		     1, jurl,
		     0, NULL,
		     dummyArrArg,
		     value,
