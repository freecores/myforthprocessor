/*
 * @(#)DefaultPluginAppletContext.java	1.37 03/01/23
 *
...
{    
	...
	ActivatorAppletAudioClipRef ref = null;
	...
	synchronized (audioClipStore)
	{
	    ...
	    ref = (ActivatorAppletAudioClipRef) audioClips.get(url);

	    if (ref == null)
	    {
		ref = new ActivatorAppletAudioClipRef(url);
	...
	ActivatorAppletImageRef ref = null;

	synchronized (imageRefs) 
	{
	    ref = (ActivatorAppletImageRef) imageRefs.get(url);

	    if (ref == null) 
	    {
		ref = new ActivatorAppletImageRef(url);
...
//	stopAudioClips();
	...
	try
	{
	    synchronized(audioClipStore)
	    {
		// Obtain audioClips for a given URL
		HashMap audioClips = (HashMap) audioClipStore.get(appletPanel.getCodeBase());

		if (audioClips != null)
		{
		    Collection collection = audioClips.values();
		    Iterator iter = collection.iterator();
		    
		    while (iter.hasNext())
		    {
			ActivatorAppletAudioClipRef ref = (ActivatorAppletAudioClipRef) iter.next();
			ref.setDoDownload(false);
			AudioClip ac = (AudioClip) ref.get();
			ref.setDoDownload(true);
			if (ac != null)
			    ac.stop();	    
		    }	    
		}
