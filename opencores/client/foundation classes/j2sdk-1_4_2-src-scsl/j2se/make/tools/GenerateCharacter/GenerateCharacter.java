...
/*
 * @(#)GenerateCharacter.java	1.7 03/01/23
 */
...
public class GenerateCharacter {
    ...
    */
    ...
    static long[] buildMap(UnicodeSpec[] data, SpecialCaseMap[] specialMaps) {
        ...
        nonCharSpec.setBidiCategory(Character.DIRECTIONALITY_UNDEFINED);
        nonCharSpec.setGeneralCategory(Character.UNASSIGNED);
        nonCharSpec.setMirrored(false);
        nonCharSpec.setTitleMap('\uFFFF');
        nonCharSpec.setUpperMap('\uFFFF');
        nonCharSpec.setLowerMap('\uFFFF');
        nonCharSpec.setDecimalValue(-1);
        nonCharSpec.setDigitValue(-1);
        nonCharSpec.setNumericValue("");

        for (int j = 0; j < data.length && k < result.length; j++) {
            if (data[j].codePoint == (char) k) {
                result[k] = buildOne((char)k, data[j], specialMaps);
                ++k;
            }
            else if(data[j].codePoint > (char)k) {
                if (data[j].name.endsWith("Last>")) {
                    // build map data for all chars except last in range
                    while (k < data[j].codePoint && k < result.length) {
                        result[k] = buildOne((char)k, data[j], specialMaps);
                        ++k;
                    }
                }
                    ...
                    while (k < data[j].codePoint && k < result.length) {
                        result[k] = buildOne((char)k, nonCharSpec, specialMaps);
                        ++k;
                    }
                }
                k = data[j].codePoint;
     ...
     */
    static boolean isInvalidJavaWhiteSpace(char c) {
        char[] exceptions = {'\u00A0', '\u2007', '\u202F', '\uFEFF'};
    ...
    */
    ...
    static long buildOne(char c, UnicodeSpec us, SpecialCaseMap[] specialMaps) {
        long resultA = 0;
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            UnicodeSpec[] data = UnicodeSpec.readSpecFile(new File(UnicodeSpecFileName));

            specialCaseMaps = SpecialCaseMap.readSpecFile(new File(SpecialCasingFileName));
            if (verbose) {
