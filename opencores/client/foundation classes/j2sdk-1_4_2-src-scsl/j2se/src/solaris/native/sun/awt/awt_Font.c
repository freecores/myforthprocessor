  ...
  (JNIEnv *env, jclass cls)
{
    ...
    platformFontIDs.props =
      (*env)->GetFieldID(env,cls, "props",
			 "Lsun/awt/FontProperties;");
...
JNIEXPORT jstring JNICALL Java_sun_awt_X11GraphicsEnvironment_getNativeFonts 
	(JNIEnv *env, jclass obj) 
{
    int32_t i = 0, retFonts = 0;
    char *pattern = "-*-*-*-*-*-*-0-0-*-*-*-*-*-*";
    char **fonts = NULL;
    char **tmpFonts = NULL;
    jclass stringClass = NULL;
    jobjectArray fontArray = NULL;
    int32_t sFonts = 0;
    ...
    fonts = XListFonts (awt_display, pattern, 5000, &retFonts);
    if (retFonts == 0) {
	return NULL;
    }

    tmpFonts = (char **) calloc (1, sizeof (char *) * retFonts);
    if (tmpFonts == NULL) {
	XFreeFontNames (fonts);
	return NULL;
    }

    /* Only return the scalable fonts (will have the string 
     * "-0-0-0-0" in them. Also, get rid of Bitstream fonts since 
     * we can't get their outlines.
     */

    for (i = 0; i < retFonts; i++) {
	if ((strstr (fonts [i], "-0-0-0-0-") != 0) &&
	    (strstr (fonts [i], "bitstream-charter") == 0) &&
	    (strstr (fonts [i], "bitstream-courier") == 0)) 
	{
            tmpFonts [sFonts++] = fonts [i];
	}
    }

    /* We will be creating (sFonts + 2 ) local refs. The '+2' is
     * for the stringClass object, and the fontArray object. 
     */
    if ((*env)->PushLocalFrame (env, (sFonts + 2)) < 0) {
	free (tmpFonts);
	XFreeFontNames (fonts);
        return NULL;
    }
    ...
    fontArray = (*env)->NewObjectArray (env, sFonts, stringClass, NULL);
    if (fontArray == NULL) {
	(*env)->PopLocalFrame (env, NULL);
	free (tmpFonts);
	XFreeFontNames (fonts);
    ...
    for (i = 0; i < sFonts ; i++) {
	(*env)->SetObjectArrayElement (env, fontArray, i,
    				(*env)->NewStringUTF (env, tmpFonts [i]));
    }

    free (tmpFonts);
    XFreeFontNames (fonts);

    return (*env)->PopLocalFrame (env, fontArray);
#else
    return NULL;
#endif
}
