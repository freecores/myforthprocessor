...
void awt_initialize_Xm_DnD(Display* dpy) {
    ...
    clazz = (*env)->FindClass(env, "sun/awt/motif/X11CustomCursor");
...
Java_sun_awt_dnd_SunDragSourceContextPeer_setNativeCursor(JNIEnv *env, 
                                                          jobject this, 
                                                          jlong nativeCtxt, 
                                                          jobject cursor, 
                                                          jint type) {
    Widget   w = (Widget)jlong_to_ptr(nativeCtxt);
    Display* dpy        = NULL;
    ...
    setCursor(env, dpy, cursor, type, CurrentTime);
}
