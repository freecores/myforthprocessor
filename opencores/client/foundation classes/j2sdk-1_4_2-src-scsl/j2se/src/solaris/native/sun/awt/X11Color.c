...
awt_allocate_colors(AwtGraphicsConfigDataPtr awt_data)
{
	...
	awt_data->awtImage->clrdata.bitsperpixel = 8;
#ifdef NEED_IMAGE_CONVERT        
    ...
    if (awt_data->awt_num_colors > 256) {
	free (awt_data->awtImage);
	return 0;
    }
    ...
    awt_data->color_data->awt_Colors = (ColorEntry *) calloc 
					(256, sizeof (ColorEntry));
	...
	awt_data->color_data->img_grays = (unsigned char *) calloc 
	...
	if (awt_data->color_data->awt_Colors[i].flags == ALLOCATED_COLOR) {
	    reds[j] = awt_data->color_data->awt_Colors[i].r;
	    greens[j] = awt_data->color_data->awt_Colors[i].g;
	    blues[j] = awt_data->color_data->awt_Colors[i].b;
	    indices[j] = i;
	    j++;
    ...
    awt_data->color_data->img_clr_tbl = (unsigned char *) calloc 
		(LOOKUPSIZE * LOOKUPSIZE * LOOKUPSIZE, sizeof (unsigned char));
    img_makePalette(cmapsize, k, LOOKUPSIZE, 50, 250,
		    j, TRUE, reds, greens, blues, 
    ...
    awt_data->color_data->awt_icmLUT2Colors = (unsigned char *) calloc 
					(256, sizeof (unsigned char));
    awt_data->color_data->awt_icmLUT = (int *) calloc (256, sizeof (int));
    for (i=0; i < 256; i++) {
        if (awt_data->color_data->awt_Colors[i].flags == ALLOCATED_COLOR) {
...
jobject awtJNI_GetColorModel(JNIEnv *env, AwtGraphicsConfigDataPtr aData)
{
    ...
    {
	jint rgb[256];
	jbyte valid[256 / 8], *pValid;
	jintArray hArray;
	jobject validBits = NULL;
        ColorEntry *c;
	int i, allocAllGray, b, allvalid;
        jlong pData;
	...
	if (!allvalid) {
        ...
        hArray = (*env)->NewIntArray(env, 256);
        if (hArray == NULL)
        {
	    (*env)->PopLocalFrame(env, 0);
            return NULL;
        ...
        {
            (*env)->SetIntArrayRegion(env, hArray, 0, 256, rgb);
        }
...
awtJNI_CreateColorData (JNIEnv *env, AwtGraphicsConfigDataPtr adata,
    ...
    screen = adata->awt_visInfo.screen;
    if (adata->awt_visInfo.visual == DefaultVisual (awt_display, screen))
	adata->awt_cmap = DefaultColormap (awt_display, screen);
    else {
	int share = 0;
	...
	if (!share) {
            adata->awt_cmap = XCreateColormap (awt_display, root, 
					       adata->awt_visInfo.visual, 
					       AllocNone);
        }  
    }   

    awt_allocate_colors (adata);
