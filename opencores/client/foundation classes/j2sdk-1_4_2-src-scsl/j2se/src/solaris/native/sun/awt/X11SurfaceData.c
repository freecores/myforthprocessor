/*
 * @(#)X11SurfaceData.c	1.47 03/04/24
 *
				   ...
				   jint width, jint height)
{
    ...
    if (getXShmAttachFailed() == JNI_TRUE) {
       shmctl(shminfo->shmid, IPC_RMID, 0);
#ifdef DEBUG
     ...
     * server has attached to the shared memory segment and so now
     * we can get rid of the id so that this segment does not stick
...
#ifdef MITSHM
	if (xsdo->isPixmap && xsdo->shmPMData.usingShmPixmap) {
