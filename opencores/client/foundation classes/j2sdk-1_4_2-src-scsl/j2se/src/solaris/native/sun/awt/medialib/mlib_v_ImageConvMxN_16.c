/*
 * @(#)mlib_v_ImageConvMxN_16.c	1.9 03/01/23
 *
 * Copyright 2003 Sun Microsystems, Inc. All rights reserved.
 * SUN PROPRIETARY/CONFIDENTIAL. Use is subject to license terms.
 */

#pragma ident  "@(#)mlib_v_ImageConvMxN_16.c	1.13  98/10/20 SMI"


/*
 * FUNCTION
 *      mlib_v_convMxN_16nw - convolve a 16-bit image, MxN kernel,
 *                            edge = no write
 *
 * SYNOPSIS
 *      mlib_status
 *      mlib_v_convMxN_16nw( mlib_image *srcimg, mlib_image *dstimg,
...
/*-----------------------------------------------------------------------*/
mlib_status
mlib_v_convMxN_16nw( mlib_image *dstimg,    /* destination image */
                     mlib_image *srcimg,    /* source image */
                     mlib_s32 kwid,         /* kernel width */
                     mlib_s32 khgt,         /* kernel height */
                     mlib_s32 khw,
                     mlib_s32 khh,
                     ...
                     mlib_s32 cmask )
{
    ...
    for ( chan = 0; chan < nchannel; chan++) {
      ...
      for (i = 0; i < khgt-1; i++) {
        (*p_proc_load)(sa, (mlib_s16*)ibuffp[i] , swid, testchan);  /* load khdt-1 (1 channel) rows from src */
        sa += spxl_stride;
      }
      ...
      for (j = 0; j < dhgt; j++) {
        (*p_proc_load)(sa, (mlib_s16*)ibuffp[khgt-1], swid, testchan); /* load next row */
