/*
 * @(#)HAE_API_LinuxOS_Capture.c	1.13 03/05/01
 *
...
// #define HAE_LINUX_FRAMES_PER_BUFFER		1024;		// how many frames do we read (capture) at a time?
...
{
    //sol: audio_info_t	sunAudioHeader;
    ...
    int				totalBytesRead;
    long			currentPos, lastPos;
    long			count;
    
    long			framesToRead;			// number of sample frames to read per callback
    long			bytesToRead;			// number of bytes to read per callback
    ...
    //sol: ioctl(g_captureSound, AUDIO_GETINFO, &sunAudioHeader);
    //sol: deviceByteBufferSize = sunAudioHeader.record.buffer_size;
    ...
    //sol: framesToRead = bytesToRead / (g_channels * g_bitSize / 8);
    // $$ay:
    framesToRead = bytesToRead;

    // flush anything we might have accumulated in the capture queue before starting the capture thread
    
    //sol: ioctl(g_captureSound, I_FLUSH, FLUSHR);
    //sol currentPos = sunAudioHeader.record.samples;
    
    // $$ay:
    HAE_FlushAudioCapture();
    ioctl(g_waveDevice, SNDCTL_DSP_GETIPTR, &audio_info);
	...
	for (count = 0; count < buffersToRead; count++) {
	    ...
	    do {
		ioctl(g_waveDevice, SNDCTL_DSP_GETISPACE, &info);
		if (info.bytes < 1)
		    HAE_SleepFrameThread(context, HAE_LINUX_SOUND_CAPTURE_PERIOD);
	    } while (info.bytes < 1 && g_captureShutdown == FALSE);
	}
...
void HAE_FlushAudioCapture() 
{
	...
	avail = abinfo.bytes;
	while (sizeof(dummyBuffer) < avail) {
