...
XmFontList getMotifFontList() {
    ...
    return XmFontListAppendEntry(NULL, motifFontListEntry);
}
...
#ifndef NOMODALFIX
    /*
     * If we have a modal dialog up, and the event is not
     * in the dialog hierarchy, we just discard the event
     * without calling XtDispatchEvent().
     */
    if (awt_isModal() && !awt_isWidgetModal(widget)) {
        if (xev->type == ButtonRelease || xev->type == KeyRelease) {
             XBell(awt_display, 50);
        }
        return True;
    }
#endif NOMODALFIX
...
processOneEvent(XtInputMask iMask) {
		...
		if (awt_get_next_put_back_event(&xev) == 0) {
		    ...
		    /* TODO: manager selections, xsettings &c */
                }
		...
		if ((xev.type == ButtonPress				||
		  ...
		  */
		  ...
		  XSync(awt_display, False);
			...
			if (winAttr.override_redirect == TRUE && isFocusableWindow(xev.xbutton.window)) {
			    /*
			       Bug 4505899:
			       Call XSetInputFocus on the window of the 
			       button press event, when the shell is an 
			       override_redirect window. 
			    */
			    XSetInputFocus(awt_display, xev.xbutton.window,
					   RevertToParent,
					   CurrentTime);
			}
                    }
		  ...
		  if (xev.type == FocusIn) {
		      ...
		      printf("FocusIn on window %d, mode %d, detail %d, "
			     "send_event %d\n", xev.xfocus.window,
			     xev.xfocus.mode, xev.xfocus.detail,
			     xev.xfocus.send_event);
		  } else {
		      printf("FocusOut on window %d, mode %d, detail %d, "
			     "send_event %d\n", xev.xfocus.window,
			     xev.xfocus.mode, xev.xfocus.detail,
			     xev.xfocus.send_event);
		  }
					   ...
					   jboolean status)
{
    ...
    if (windowCls == NULL) {
        jclass windowClsLocal = (*env)->FindClass(env, "java/awt/Window");
        DASSERT(windowClsLocal != NULL);
        if (windowClsLocal == NULL) {
            return;
        }
        windowCls = (jclass)(*env)->NewGlobalRef(env, windowClsLocal);
        (*env)->DeleteLocalRef(env, windowClsLocal);
        lwRequestStatus = (*env)->GetFieldID(env, windowCls, "syncLWRequests", "Z");
    }
    (*env)->SetBooleanField(env, win, lwRequestStatus, status);
}
                                  ...
                                  jobject cont, jobject comp, jint index)
{
    ...
    if ( JNU_IsNull(env, setZOrderMID) ) {
        jclass containerCls = (*env)->FindClass(env, "java/awt/Container");
        DASSERT(!JNU_IsNull(env, containerCls));
        if ( JNU_IsNull(env, containerCls) ) {
            return;
        }
        setZOrderMID = (*env)->GetMethodID(env, containerCls, "setZOrder",
    ...
    (*env)->CallVoidMethod(env, cont, setZOrderMID, comp, index);
}
...
Java_sun_awt_SunToolkit_getPrivateKey(JNIEnv *env, jclass cls, jobject obj)
{
    ...
    } else if ((*env)->IsInstanceOf(env, obj, menuComponentCls)) {
	key = (*env)->GetObjectField(env, obj, menuComponentIDs.privateKey);
    }
    return key;
}
    ...
    jstring mainClassName)
{
...

    {
        ...
        pfClass = (*env)->FindClass(env, "sun/awt/motif/MFontPeer");
        methID = (*env)->GetStaticMethodID(env, pfClass,
                                           "getDefaultMotifFontSet",
                                           "()Ljava/lang/String;");
	jFontList = (*env)->CallStaticObjectMethod(env, pfClass, methID);
	if (jFontList == NULL) {
	    motifFontList =
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_sun_awt_motif_MToolkit_sync
  (JNIEnv *env, jobject this) 
{
    AWT_LOCK();
    XSync(awt_display, False);
...
Java_sun_awt_motif_MToolkit_loadXSettings(JNIEnv *env, jobject this)
{
    ...
    screen = DefaultScreen(awt_display);
    xSettingsAtom = XInternAtom(awt_display, "_XSETTINGS_SETTINGS", False);
    if (xSettingsAtom == None) {
	DTRACE_PRINTLN("XS: Unable to intern _XSETTINGS_SETTINGS");
	AWT_UNLOCK();
	return;
    }

    /* NOTE: We should populate this based on the screen. */
    sprintf(buffer, "_XSETTINGS_S%d", screen);
    selectionAtom = XInternAtom(awt_display, buffer, False);
    if (selectionAtom == None) {
	DTRACE_PRINTLN1("XS: Unable to intern %s", buffer);
	AWT_UNLOCK();
	return;
    }

    selectionWindow = XGetSelectionOwner(awt_display, selectionAtom);
    if (selectionWindow == None) {
	DTRACE_PRINTLN1("XS: Selection %s is not owned", buffer);
	AWT_UNLOCK();
	return;
    }
    DTRACE_PRINTLN2("XS: %s is owned by %08x", buffer, selectionWindow);

    XGrabServer(awt_display);

#if 0 /* not yet */
    /* Monitor selection owner for property changes and for being destroyed */
    XSelectInput(awt_display, selectionWindow,
		 StructureNotifyMask | PropertyChangeMask);
#endif

    if (XGetWindowProperty(awt_display, selectionWindow, xSettingsAtom, 0,
			   0xffffff, False, xSettingsAtom, &returnType,
			   &returnFormat, &returnCount, &remainCount,
			   &data) != Success
	|| returnType != xSettingsAtom)
    {
	XUngrabServer(awt_display);
	AWT_UNLOCK();
	DTRACE_PRINTLN("XS: Unable to get selection property");
	return;
    }
    XUngrabServer(awt_display);
    ...
    mid = (*env)->GetMethodID(env, cls,"parseXSettings", "([B)V");
    array = (*env)->NewByteArray(env, returnCount);
    (*env)->SetByteArrayRegion(env, array, 0, returnCount, (jbyte *)data);
    XFree(data);

    AWT_UNLOCK();

    (*env)->CallVoidMethod(env, this, mid, array);
