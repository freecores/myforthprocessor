...
{
    static int32_t topGuess    = 0;
    static int32_t leftGuess   = 0;
    static int32_t bottomGuess = 0;
    static int32_t rightGuess  = 0;
	...
	if (wdata->top >= 0) {
	    ...
	    topGuess    = wdata->top;
	    leftGuess   = wdata->left;
	    bottomGuess = wdata->bottom;
	    rightGuess  = wdata->right;
	}
	...
	if ((insets_env = getenv("AWT_INSETS")) != NULL) {
	    ...
	    topGuess    = (guess & 0xff00) >> 8;
	    leftGuess   = guess & 0x00ff;
	    bottomGuess = wdata->leftGuess;
	    rightGuess  = wdata->leftGuess;
	}

	/* don't allow bizarly large insets */
	if ((topGuess > 64) || (topGuess < 0))
	    topGuess = 28;
	if ((leftGuess > 32) || (leftGuess < 0))
	    leftGuess = 6;
	if ((bottomGuess > 32) || (bottomGuess < 0))
	    bottomGuess = 6;
	if ((rightGuess > 32) || (rightGuess < 0))
	    rightGuess = 6;
    }

    wdata->top    = wdata->topGuess    = topGuess;
    wdata->left   = wdata->leftGuess   = leftGuess;
    wdata->bottom = wdata->bottomGuess = bottomGuess;
    wdata->right  = wdata->rightGuess  = rightGuess;
}
...
/**
 * Callback procedure fot XCheckIfEvent
 * Searches X event queue for the FocusIn events corresponding
 * to the FocusOut event supplied in parameters and retrieves
 * the information about the opposite window for focus transfer
 * between top-levels
 */
static Bool
check_for_focus_in(Display * dpy, XEvent * event, XPointer data) {
                ...
                if (wpeer != NULL) {
                    jobject peerComp =
                        (*env)->GetObjectField(env,
                                               ...
                                               mComponentPeerIDs.target);
                    if (peerComp != NULL) {
                        prev->inChild = event->xfocus.window;
                        if (prev->childComp != NULL) {
                            (*env)->DeleteLocalRef(env, prev->childComp);
                        }
                        prev->childComp = peerComp;
                    }
                }
                return False;
            } else {
                Widget child = NULL;
                prev->inWin = event->xfocus.window;
                if (prev->defChild != NULL) return False;
                child = XmGetFocusWidget(w);
                if (child != NULL) {
                    wpeer = findPeer(&child);
                    if (wpeer != NULL) {
                        jobject peerComp = (*env)->GetObjectField(env, wpeer, mComponentPeerIDs.target);
                        if (peerComp != NULL) {
                            prev->defChild = child;
...
shellEH(Widget w, XtPointer data, XEvent *event, Boolean *continueToDispatch)
{
    ...
    case FocusOut: {
		...
		break;
            ...
            if (focus_info.inWin != None) {                
                ...
                jobject wpeer = NULL;
                inWidget = XtWindowToWidget(awt_display, focus_info.inWin);                    
	...
	if (wdata->decor == AWT_NO_DECOR) {
	    ...
	    int32_t		correctWMTop;
	    int32_t		correctWMLeft;
    ...
    jboolean isModal)
{
    ...
    if (!wdata->isShowing) {
        ...
        if ((wdata->decor != AWT_FULL_DECOR) && (wdata->decor != AWT_NO_DECOR)) {
            /* At this point, this code only runs for InputMethodWindow */
    ...
    target = (*env)->GetObjectField(env, this, mComponentPeerIDs.target);
    if ((*env)->CallBooleanMethod(env, target, windowIDs.focusableWindowMID) == JNI_TRUE) {
 ...
 jint iconWidth, jint iconHeight)
{
    ...
    if (wdata->iconPixmap &&
	(wdata->iconWidth != iconWidth || wdata->iconHeight != iconHeight)) {
	XFreePixmap(awt_display, wdata->iconPixmap);
    ...
    jboolean resizable)
{
    ...
    if (wdata->decor != AWT_FULL_DECOR) {
	AWT_UNLOCK();
        return; /* resizability only applies to decorated frames and dialogs */
    }
...
JNIEXPORT void JNICALL
Java_sun_awt_motif_MEmbeddedFramePeer_pShow(JNIEnv *env, jobject this)
{
    struct FrameData *wdata;
