/*
 * @(#)mlib_v_ImageConvMxN_8ext.c	1.11 03/01/23
 *
...
#pragma ident	"@(#)mlib_v_ImageConvMxN_8ext.c	1.9	00/02/07 SMI"
 ...
 * SYNOPSIS
 *      mlib_status
 *      mlib_v_convMxN_8ext( mlib_image *srcimg, mlib_image *dstimg,
...
/*-----------------------------------------------------------------------*/
mlib_status
mlib_v_convMxN_8ext( mlib_image *dstimg,              /* destination image */
                     ...
                     mlib_s32 cmask )
{
  ...
  for ( chan = 0; chan < nchannel; chan++) {
    ...
    for (i = 0; i < khgt-1; i++) {
      (*p_proc_load)(sa, (s_buf + dx_l), swid, testchan);  /* load khgt-1 (1 channel) rows src */
      for (j = 0; j < dx_l; j++) {
        s_buf[j] = s_buf[dx_l];                          /* extend edges */
      }
      for (j = 0; j < dx_r; j++) {
        s_buf[swid + dx_l + j] = s_buf[swid + dx_l - 1];  /* extend edges */
      }
      s_buf += buf_slb;
    ...
    for (j = 0; j < dhgt - dy_b; j++) {
      (*p_proc_load)(sa, (s_buf + dx_l), swid, testchan); /* load next row */
      for (i = 0; i < dx_l; i++) {
        s_buf[i] = s_buf[dx_l];                          /* extend edges */
      }
      for (i = 0; i < dx_r; i++) {
        s_buf[swid + dx_l + i] = s_buf[swid + dx_l - 1];
      }
      ...
      for (i = 0; i < dwid; i += 8 ) {
        *ddst++ = vis_freg_pair(vis_fpack16(ibuffh[0]), vis_fpack16(ibuffh[1]));
        ibuffh += 2;
      }
      ...
      for (i = 0; i < buf_slb*(khgt-1); i += 8) {
        *(mlib_d64 *)(sbuf + i) = *(mlib_d64*)(sbuf + i +  buf_slb);
      }
