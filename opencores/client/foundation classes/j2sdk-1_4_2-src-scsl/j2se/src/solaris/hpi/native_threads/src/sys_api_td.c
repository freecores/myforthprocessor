 ...
 * setjmp/lngjmp to avoid io races.  Look at green_threads/io_md.c for
 ...
 * value and errno to EBADF instead of EINTRT. The high level
 * routine will interpret it as IOException instead of 
 * InterruptedIOException. No other change is needed.
 * The underlying mechanism is using the sigusr1 to wake up
 * the blocking thread, it may has severe conflict with any
 * other libraries that also use the sigusr1.
 */
#ifdef __linux__
