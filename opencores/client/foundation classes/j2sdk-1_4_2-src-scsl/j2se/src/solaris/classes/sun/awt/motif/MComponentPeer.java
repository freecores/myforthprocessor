/*
 * @(#)MComponentPeer.java	1.169 03/02/12
 *
...
abstract class MComponentPeer implements ComponentPeer, DropTargetPeer {
    ...
    /*
     * For all practical purposes, a Pixmap could be used to implement
     * the pixel storage for an offscreen image, but there would be a
     * performance impact for some cases.  While standard 1.1 rendering
     * and blits might be as fast or faster on a Pixmap than using a
     * BufferedImage, doing alpha composites or other imaging operations
     * other than homogenous blits from other Volatile images would be
     * much, much slower if there is no direct access to the pixels in
     * the pixmap (such as a remote server or a local server with no
     * direct graphics access).  For cases where an application is doing
     * simple rendering to an offscreen mostly for double-buffering
     * purposes we provide a property for them to request storage of
     * their pixels on the server, but the default will be to not use
     * the remote storage.
     * REMIND: In the future, we might want to have a more dynamic default
     *     pmoffscreen policy determined by the runtime configuration.
     */
	...
	surfaceData = X11SurfaceData.createData(this);
    }
...
*/
	...
	if (X11SurfaceData.isAccelerationEnabled()) {
	    return new X11RemoteOffScreenImage((Component)target, model, wr,
					       model.isAlphaPremultiplied());
	} else {
	    return new OffScreenImage((Component)target, model, wr,
				      model.isAlphaPremultiplied());
	}
    }
    public VolatileImage createVolatileImage(int width, int height) {
	return new X11VolatileImage((Component)target, width, height);
    }
    ...
    protected void disposeImpl() {
        X11SurfaceData oldData = surfaceData;
        surfaceData = null;
        oldData.invalidate();
    ...
    */
    ...
    private native void destroyBackBuffer(long backBuffer);
    ...
    public void createBuffers(int numBuffers, BufferCapabilities caps)
        ...
        BufferCapabilities configCaps = graphicsConfig.getBufferCapabilities();
        if (!configCaps.isPageFlipping()) {
            throw new AWTException("Page flipping is not supported");
	}
        long window = getWindow(pData);
        int swapAction = getSwapAction(caps.getFlipContents());
        backBuffer = createBackBuffer(window, swapAction);
        xBackBuffer = new X11VolatileImage((Component)target,
            target.getWidth(), target.getHeight(), backBuffer);
    }
