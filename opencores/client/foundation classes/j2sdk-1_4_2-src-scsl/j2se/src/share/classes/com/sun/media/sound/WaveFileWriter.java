/*
 * @(#)WaveFileWriter.java	1.18 03/01/23
 *
...
public class WaveFileWriter extends SunFileWriter {
	...
	if(audioStream instanceof AudioInputStream) {
	    ...
	    if( ((encoding==AudioFormat.Encoding.PCM_SIGNED) && audioStreamFormat.isBigEndian()) ||
		((encoding==AudioFormat.Encoding.PCM_UNSIGNED) && !audioStreamFormat.isBigEndian()) ||
		((encoding==AudioFormat.Encoding.PCM_UNSIGNED) && audioStreamFormat.isBigEndian()) ) {
		if( sampleSizeInBits!=8) {
		    wav_type = WaveFileFormat.WAVE_FORMAT_PCM;
