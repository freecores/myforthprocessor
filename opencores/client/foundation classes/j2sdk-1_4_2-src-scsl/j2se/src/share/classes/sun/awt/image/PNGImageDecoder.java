/*
 * @(#)PNGImageDecoder.java	1.8 03/01/23
 *
...
public class PNGImageDecoder extends ImageDecoder
{
  ...
  public void replayConsumer(ImageConsumer ic) {
    ...
    public synchronized void makeStore(int width, int height) throws IOException {
	if (cancatchup) {
  ...
  public void produceImage() throws IOException, ImageFormatException {
    ...
    try {
	    ...
	    makeStore(width, height);
            setColorModel(cm);
	    store.setColorModel(cm);
	    int flags = (interlaceMethod !=0
                       ? ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES
                       : ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES |
                         ImageConsumer.SINGLEPASS | ImageConsumer.SINGLEFRAME);
	    setHints(flags);
            store.setHints(flags);
	    headerComplete();
	    // end of adding
   ...
   /* Here, the function "visit(row,column,height,width)" obtains the
      ...
      and column are measured from 0,0 at the upper left corner. */
		...
		if(properties!=null) store.setProperties(properties);
		store.imageComplete();
	...
	if (count > 0) {
	    store.setPixels(x, y, w, h, pixels, offset, pixlength);
	} else {
	    aborted = true;
	}
	return !aborted;
