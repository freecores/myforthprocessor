...
public class MixerClip extends AbstractDataLine implements Clip, AutoClosingClip {
    ...
    int position = 0;				// current position in frames
	...
	if (Printer.debug) Printer.debug("    getFramePosition(): " + getFramePosition());
	...
	*/
	...
	int newId = nSetup(waveformId, position, loopStart, loopEnd, currentGain, (currentPan * -1), currentRate);
    ...
    */
	...
	return (id != 0) ? (position + (int)nGetPosition(id)) : position;
    }
    ...
    //		sendEvents(new ChannelEvent(this, ChannelEvent.Type.EOM, getFramePosition()));	
    //	}  
