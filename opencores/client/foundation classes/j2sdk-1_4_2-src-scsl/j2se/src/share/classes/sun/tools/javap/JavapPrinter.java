/*
 * @(#)JavapPrinter.java	1.10 03/01/23
 *
...
public class JavapPrinter implements RuntimeConstants {
	...
	out.println("Compiled from " + javaclassname(cls.getSourceName()));
	if(cls.isInterface())	{
	    out.print("interface "+ javaclassname(cls.getClassName()));
	}
	else if(cls.isClass()) {
	...
	if (exc_index_table != null) {
	    ...
	    for (k=0; k<l; k++) {
		out.print(cls.getClassName(exc_index_table[k]));
		if (k<l-1) out.print(", ");
	    }
	    out.println();
	}
