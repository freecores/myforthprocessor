/*
 * @(#)ReflectionFactory.java	1.14 03/01/23
 *
...
public class ReflectionFactory {
    ...
    /** Note: this routine can cause the declaring class for the field
        to be initialized and therefore must not be called until the
        first get/set of this field. */
    public FieldAccessor newFieldAccessor(Field field) {
        checkInitted();

        if (   useUnsafeFieldAccessors
               && !Modifier.isVolatile(field.getModifiers())) {
            return UnsafeFieldAccessorFactory.newFieldAccessor(field);
        }

        return new FieldAccessorGenerator().generate(field.getDeclaringClass(),
                                                     field.getName(),
                                                     field.getType(),
                                                     field.getModifiers());
    }
    ...
    private static void checkInitted() {
        ...
        AccessController.doPrivileged(new PrivilegedAction() {
                public Object run() {
                    ...
                    String val =
                        System.getProperty("sun.reflect.useGeneratedFieldAccessors");
                    if (val != null && val.equals("true")) {
                        useUnsafeFieldAccessors = false;
                    }

                    val = System.getProperty("sun.reflect.noInflation");
                    if (val != null && val.equals("true")) {
                        noInflation = true;
                    }
