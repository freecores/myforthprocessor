/*
 * @(#)FontDesignMetrics.java	1.53 03/01/23
 *
...
public final class FontDesignMetrics extends FontMetrics {
   ...
   */
    ...
    private void initMatrixAndMetrics() {
        ...
        fontTx.scale(ptSize, ptSize);

        fontTx.getMatrix(matrix);

        float[] metrics = new float[4];
        NativeFontWrapper.getFontMetrics(font, 
                                         matrix,
                                         isAntiAliased,
    ...
    private float handleCharWidth(char ch) {
	float[] result = new float[2];
	NativeFontWrapper.getCharMetrics(font, ch, matrix, devmatrix, isAntiAliased, usesFractionalMetrics, result);
	return result[0]; // x-component of result only
    }
