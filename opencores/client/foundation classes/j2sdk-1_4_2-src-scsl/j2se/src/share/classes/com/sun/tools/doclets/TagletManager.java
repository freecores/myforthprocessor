/*
 * @(#)TagletManager.java	1.27 03/01/23
 *
...
public class TagletManager {
    ...
    public void addCustomTag(String classname, String tagletPath) {
        try {
            ...
            int previousSize = customTags.size();
            meth.invoke(null, new Object[] {customTags});
            list = customTags.values().toArray();
            Taglet newLastTag = (list != null&& list.length > 0)
                ? (Taglet) list[list.length-1] : null;
            if (lastTag != newLastTag) {
                ...
                if (newLastTag != null) {
                    checkTagName(newLastTag.getName());
                }
            }
        } catch (Exception exc) {
    ...
    /**
     * Given a name of a tag, add it to the set of tags it belongs to.
     */
    ...
    private void printTagMisuseWarn(Taglet taglet, Tag tag, String holderType) {
        ...
        String combined_locations = "";
        for (int i = 0; i < locations.length; i++) {
            if (i > 0) {
                combined_locations += ", ";
            }
            combined_locations +=  locations[i];
        }
    ...
    private void initCustomTagArrays() {
        ...
        Vector pTags = new Vector(customTags.size());
        Vector tTags = new Vector(customTags.size());
        Vector fTags = new Vector(customTags.size());
        Vector cTags = new Vector(customTags.size());
        Vector mTags = new Vector(customTags.size());
        Vector iTags = new Vector(customTags.size());
        Vector oTags = new Vector(customTags.size());
        Taglet current;
        while (it.hasNext()) {
    ...
    private void initStandardTags() {
        ...
        customTags.put((temp = new SimpleTaglet("exception",
                                                null, SimpleTaglet.METHOD +
                                                    SimpleTaglet.CONSTRUCTOR)).getName(), temp);
        if (!nosince) {
