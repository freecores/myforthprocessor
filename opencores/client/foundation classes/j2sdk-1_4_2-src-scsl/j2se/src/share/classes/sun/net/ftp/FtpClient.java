...
public class FtpClient extends TransferProtocolClient {
	...
	try {
	    ...
	    // Oh well, let's fall back to the good old "PORT"
	}
	...
	// Some FTP servers (like the one on Solaris) are bugged, they will
	// accept the EPRT command but then, the subsequent command (e.g. RETR)
	// will fail, so we have to check BOTH results (the EPRT cmd then the
	// actual command) to decide wether we should fall back on the older
	// PORT command.
	portCmd = "EPRT |" + 
