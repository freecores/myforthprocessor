/*
 * @(#)StandardMidiFileWriter.java	1.19 03/01/23
 *
...
public class StandardMidiFileWriter extends SunMidiFileWriter {
    ...
    /**
     * Obtains a file stream from the sequence provided.
     * @param type type of file stream to get from sequence, or null for default
     * @param sequence sequence containing events to be written to a file
     * @return stream from which file data may be read, or null if a file stream 
     * cannot be obtained from the sequence provided.
	...
	for(int i=0; i<tracks.length; i++) {
	    try {
		trackStreams[i] = writeTrack( tracks[i], type );
	    } catch (InvalidMidiDataException e) {
				// $$jb: 04.08.99: What to do here?
	...
	if( tracks.length == 1 ) {
	    trackStream = (InputStream) trackStreams[0];
	} else if( tracks.length > 1 ){
	    trackStream = (InputStream) trackStreams[0];
	    for(int i=1; i<tracks.length; i++) {
		trackStream = (InputStream) (new SequenceInputStream( trackStream, trackStreams[i]));
	    }
	}
    ...
    /**
     * Returns ONE_BYTE, TWO_BYTE, SYSEX, or META
     */
