...
final public class LdapCtx extends ComponentDirContext 
    ...
    // 	used for StartTLS extended operation
    //  Use reflection to avoid dependency on StartTlsResponse
    //  and on com.sun.jndi.ldap.ext.StartTlsResponseImpl. The latter might
    //  not be in the same jar as LdapCtx due to dependencies on
    //  JSSE packaging in the J2SDK.
	...
	currentParsedDN = new LdapName(currentDN);
	...
	boolean directUpdate) throws NamingException {
	    ...
	    Vector rdns = (new LdapName.DnParser(dn, true)).getDn();

	    // Get leaf RDN
	    LdapName.Rdn rdn = (LdapName.Rdn) rdns.get(rdns.size()-1);
	    Attributes nameAttrs = rdn.toAttributes();

	    // Add attributes of RDN to attrs if not already there
	    NamingEnumeration enum = nameAttrs.getAll();
	    Attribute nameAttr;
	    while (enum.hasMore()) {
		nameAttr = (Attribute) enum.next();
...
*/
   ...
   // ------------ Return code and Error messages  -----------------------
	...
	try {
	    ...
	    loadStartTlsImpl();
	    if (startTlsImplClass != null && startTlsImplClass.isInstance(er)) {

		// Pass the connection handle to StartTlsResponseImpl
		// ((StartTlsResponseImpl)er).setConnection(clnt.conn, domain);
		try {
		    String domainName = (String)
			(envprops != null ? envprops.get(DOMAIN_NAME) : null);
		    startTlsInitMethod.invoke(er,
			new Object[]{clnt.conn, domainName});
		} catch (InvocationTargetException ie) {
		    Throwable realException = ie.getTargetException();
		    // realException.printStackTrace();
		    ConfigurationException ce = new ConfigurationException(
		"Invocation problem with Start TLS response implementation");
		    ce.setRootCause(realException);
		    throw ce;
		} catch (IllegalAccessException ie) {
		    ConfigurationException ce = new ConfigurationException(
			"Problem initializing Start TLS response implementation");
		    ce.setRootCause(ie);
		    throw ce;
		}
	    }
    ...
    // --------------- Start TLS Reflection code
    /**
     * Loads the com.sun.jndi.ldap.ext.StartTlsResponseImpl implementation.
     * Sets the static fields: startTlsImplClass, startTlsInitMethod
     */
    private synchronized static void loadStartTlsImpl() {
	if (startTlsImplClass == null) {
	    try {
		/*
		 * Use the Obj.helper.loadClass method to load the 
		 * StartTlsResponse implemention class.
		 * The application's class loader is used.
		 */
		startTlsImplClass = Obj.helper.loadClass(
		    "com.sun.jndi.ldap.ext.StartTlsResponseImpl");

		startTlsInitMethod = startTlsImplClass.getMethod("setConnection", 
