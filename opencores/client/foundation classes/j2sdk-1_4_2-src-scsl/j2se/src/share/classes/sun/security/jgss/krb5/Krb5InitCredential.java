/*
 * @(#)Krb5InitCredential.java	1.18 03/01/23
 *
...
public class Krb5InitCredential
	...
	try {
	    TICKET_CLASS =
		Class.forName("javax.security.auth.kerberos.KerberosTicket");
	...
	Object retVal = AccessController.doPrivileged(
	      new SubjectComber(acc, tgsPrincipal, clientPrincipal, KerberosTicket.class));
        if (retVal == null) {
            if (!LoginUtility.useSubjectCredsOnly()) {
		Subject subject = null;
                try {
                    subject = (Subject) AccessController.doPrivileged(
	                new LoginUtility(LoginUtility.GSS_INITIATE_ENTRY));
		...
		retVal = AccessController.doPrivileged(
		    new SubjectComber(subject, tgsPrincipal, clientPrincipal,KerberosTicket.class));
            }
        }
	return (retVal == null? null : (KerberosTicket) retVal);
    }
}
