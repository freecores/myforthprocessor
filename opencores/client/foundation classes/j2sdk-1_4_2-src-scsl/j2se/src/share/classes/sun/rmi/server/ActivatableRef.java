/*
 * @(#)ActivatableRef.java	1.19 03/01/23
 *
...
public class ActivatableRef implements RemoteRef {
     ...
     */
    public static RemoteStub getStub(ActivationDesc desc, ActivationID id)
	...
	try {
	    ...
	    Class remoteCl = RemoteProxy.getRemoteClass(cl);
	    return RemoteProxy.getStub(remoteCl.getName(), remoteCl,
				       new ActivatableRef(id, null));
	} catch (Exception e) {
	...
	try {
	     ...
	     */
	    ActivatableRef newRef = (ActivatableRef)
		((RemoteStub)id.activate(force)).getRef();
	    ref = newRef.ref;
	    return ref;
	} catch (RemoteException e) {
	    throw e;
	} catch (UnknownObjectException e) {
	    throw new NoSuchObjectException("activatable object no longer " +
					    "registered");
	} catch (Exception e) {
	    throw new ActivateFailedException("failed to activate object", e);
	} 
    }
