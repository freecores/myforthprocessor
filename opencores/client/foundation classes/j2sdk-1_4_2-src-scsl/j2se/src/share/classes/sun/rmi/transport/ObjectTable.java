/*
 * @(#)ObjectTable.java	1.28 03/01/23
 *
...
public final class ObjectTable {
	...
	synchronized (tableLock) {
	    return (Target)implTable.get(new WeakRef(impl));
	}
    }
   ...
   public static boolean unexportObject(Remote obj, boolean force)
	...
	if (DGCImpl.dgcLog.isLoggable(Log.VERBOSE)) {
	    DGCImpl.dgcLog.log(Log.VERBOSE, "add object " + id);
	}
	...
	if (DGCImpl.dgcLog.isLoggable(Log.VERBOSE)) {
	    DGCImpl.dgcLog.log(Log.VERBOSE, "remove object " + id);
	}
	...
	synchronized (tableLock) {
	    Target target = (Target) objTable.get(id);
	    if (target != null)
