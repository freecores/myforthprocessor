...
class Scanner
{
      ...
      try
      {
        ...
        if ((data.ch >= '0' && data.ch <= '9') || data.ch == '.') {
          ...
          switch (data.ch)
          {
            ...
            case '"':
              readChar ();
              token = new Token (Token.StringLiteral, getUntil ('"'));
              break;
            case '\\':
              readChar ();
  ...
  String getUntil (char c) throws IOException
  {
    String string = "";
    while (data.ch != c)
      string = appendToString (string, false, false);
    return string;
  } // getUntil
   ...
   *
   **/
  private String appendToString (String string, boolean quote, boolean charLit) throws IOException
  {
    // Ignore any comments
    if (data.ch == '/')
    {
    ...
    else
    {
      if (!charLit && data.ch == '"')
      {
        ...
        string = string + '"';
        while (data.ch != '"')
          string = appendToString (string, true, false);
      }
      else if (!quote && !charLit && data.ch == '(')
      {
        readChar ();
        string = string + '(';
        while (data.ch != ')')
      ...
      else if (!quote && data.ch == '\'')
      {
        readChar ();
        string = string + "'";
        while (data.ch != '\'')
          string = appendToString (string, false, true);
      }
      string = string + data.ch;
      readChar ();
    }
