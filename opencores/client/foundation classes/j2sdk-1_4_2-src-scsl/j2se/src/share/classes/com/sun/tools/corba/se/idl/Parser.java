...
class Parser
{
      ...
      throws IOException, ParseException
  {
	...
	repIDStack.push (((IDLID)repIDStack.peek ()).clone ());
	InterfaceEntry entry = stFactory.interfaceEntry (module, (IDLID)repIDStack.peek ());
  ...
  private SymtabEntry typeSpec (SymtabEntry entry) throws IOException, ParseException
  {
    return ((token.type == Token.Struct) ||
        ...
        : simpleTypeSpec (entry);
  } // typeSpec

  /**
   *
   **/
  private SymtabEntry simpleTypeSpec (SymtabEntry entry) throws IOException, ParseException
  {
    // <f46082.40>
  ...
  {
    match (Token.Struct);
    StructEntry structEntry = stFactory.structEntry (entry, (IDLID)repIDStack.peek ());
    repIDStack.push (((IDLID)repIDStack.peek ()).clone ());
    ((IDLID)repIDStack.peek ()).appendToName(token.name);
    structEntry.sourceFile (scanner.fileEntry ());
    // comment must immediately precede "struct" keyword
    structEntry.comment (tokenHistory.lookBack (1).comment);
    structEntry.name (token.name);
    match (Token.Identifier);
  ...
  {
    match (Token.Union);
    UnionEntry unionEntry = stFactory.unionEntry (entry, (IDLID)repIDStack.peek ());
    repIDStack.push (((IDLID)repIDStack.peek ()).clone ());
    ((IDLID)repIDStack.peek ()).appendToName (token.name);
    unionEntry.sourceFile (scanner.fileEntry ());
    // comment must immediately precede "union" keyword
    unionEntry.comment (tokenHistory.lookBack (1).comment);
    unionEntry.name (token.name);
    match (Token.Identifier);
  ...
  private SequenceEntry sequenceType (SymtabEntry entry) throws IOException, ParseException
  {
    ...
    newEntry.type (simpleTypeSpec (newEntry));
    if (token.type == Token.Comma)
    {
      match (Token.Comma);
...
());
  ...
  private void pigeonhole (SymtabEntry container, SymtabEntry entry)
  {
    ...
    if (overrideNames.get (fullName) == null)
    {
      ...
      // Check to see if this name already exists in the symbol table.
      // If it does, it might be an interface replacing a forward delcaration.
      // Those are OK.  Anything else is not.
      // (Reopening of a module is OK, too, but that's handled in
      // newModule; if that is the situation, then this code should
      // never be encountered.)
      SymtabEntry oldEntry = (SymtabEntry) symbolTable.get (fullName);
      if (oldEntry == null)
      {
        // Make sure the key doesn't exist in the symbol table of lowercase
        // names.  In IDL, identifiers which differ only in case are
        // collisions.
      ...
      else if (oldEntry instanceof ForwardEntry && entry instanceof InterfaceEntry)
      {
        ...
        {
          // <D56351> Allow InterfaceEntry having same Repository ID prefix
          // as its ForwardEntry (CORBA 2.3).
          String lcFullName = fullName.toLowerCase();
          symbolTable.put (fullName, entry);
          lcSymbolTable.put (lcFullName, entry);
        }
        else
          ParseException.badRepIDPrefix (scanner, fullName, oldRepIDPrefix, repIDPrefix);
      }
      else if (entry instanceof ForwardEntry &&
         (oldEntry instanceof InterfaceEntry || oldEntry instanceof ForwardEntry))
      {
        // <61743> Test for this case only.
                ...
                oldEntry.repositoryID () instanceof IDLID))
        {
          ...
          if (!(repIDPrefix.equals (oldRepIDPrefix)))
          {
            // <D56351> Disallow multiple ForwardEntry's having same Repository
            // ID prefixes (CORBA 2.3).
            ParseException.badRepIDPrefix (scanner, fullName, oldRepIDPrefix, repIDPrefix);
          }
        }
      }
      else if (cppModule && entry instanceof ModuleEntry && oldEntry instanceof ModuleEntry)
      {
        // <46082.46.01> Allow multiple ModuleEntry's when user submits
        // the -cppModule flag.
      }
      else if (fullName.startsWith ("org/omg/CORBA") || fullName.startsWith ("CORBA"))
      {
        // <55699> Ignore CORBA PIDL types entered at preParse() by generator.
      }
      else
        ParseException.alreadyDeclared (scanner, fullName);
    }
  ...
  SymtabEntry partlyQualifiedEntry (String typeName, SymtabEntry container)
  {
    ...
    if (typeName != null)
    {
      ...
      if (container != null)
        type = searchModuleScope (typeName.substring (0, typeName.lastIndexOf ('/')), container);
      if (type == null)
        //qualifiedEntry (typeName); // <d59809> What... Discard the result?  baah, humBUG!
