...
public class Group {
    ...
    public boolean checkPackageGroups(String groupname, 
        ...
        if (groupList.contains(groupname)) {
            reporter.printError(
                msg().getText("doclet.Groupname_already_used", groupname));
            return false;
        }
        ...
        while (strtok.hasMoreTokens()) {
            ...
            if (id.length() == 0) {
                reporter.printError(
                    msg().getText("doclet.Error_in_packagelist", 
    ...
    boolean foundGroupFormat(Map map, String pkgFormat,
        ...
        if (map.containsKey(pkgFormat)) { 
            reporter.printError(
                msg().getText("doclet.Same_package_name_used", pkgFormat));
            return true;
        }
        return false;
    }
    ...
    public Map groupPackages(PackageDoc[] packages) {
        ...
        String defaultGroupName = 
            (pkgNameGroupMap.isEmpty() && regExpGroupMap.isEmpty())? 
                msg().getText("doclet.Packages") :
                msg().getText("doclet.Other_Packages");
