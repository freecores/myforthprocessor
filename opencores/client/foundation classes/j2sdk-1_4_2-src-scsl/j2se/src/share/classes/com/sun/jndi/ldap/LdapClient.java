...
public final class LdapClient implements PooledConnection {
	...
	if (authMechanism.equalsIgnoreCase("none") ||
	    ...
	    // SASL authentication
	    try {
		res = saslBind(conn.host, name, pw, authMechanism, env, ctls);
		if (res.status == LdapClient.LDAP_SUCCESS) {
		    conn.setBound();
		}
	    } catch (IOException e) {
    ...
    /**
     * Performs SASL bind.
     * Create a SaslClient by using the specified callback handler, or the
     ...
     * @exception AuthenticationNotSupportedException If SASL or 
     * 	   mechanism not supported
     */
    private LdapResult saslBind(String server, String dn, Object pw, 
	String authMech, Hashtable env, Control[] bindCtls) 
	throws IOException, NamingException {

	    Method sbind = getSaslBindMethod();

	    if (sbind != null) {
		try {
		    return (LdapResult) sbind.invoke(null,
			new Object[]{this, conn, server, dn, pw, authMech, 
					 env, bindCtls});
		} catch (InvocationTargetException e) {
		    Throwable realException = e.getTargetException();
		    // realException.printStackTrace();

		    if (realException instanceof NamingException) {
			throw (NamingException)realException;
			...
			ne.setRootCause(realException);
			throw ne;
		    }
		} catch (IllegalAccessException e) {
			NamingException ne = 
    new AuthenticationNotSupportedException("Cannot access saslBind method");
			ne.setRootCause(e);
			throw ne;
		}
	    }
		...
		"SASL support not available: " + authMech);
    }
	...
	try {
	    Class saslClass = Obj.helper.loadClass(
		"com.sun.jndi.ldap.sasl.SaslInputStream");
	    return saslClass.isInstance(conn.inStream);
	} catch (Exception e) {
    ...
    /**
     * Trieds to load saslBind method. Sets 'triedSasl' to true always
     * after this method has been invoked.
     */
    private static Method getSaslBindMethod() {
	try {
	    Class saslClass = 
		Obj.helper.loadClass("com.sun.jndi.ldap.sasl.LdapSasl");
	    return saslClass.getMethod("saslBind",
		new Class[]{LdapClient.class, Connection.class, 
				String.class, String.class, 
				Object.class, String.class, Hashtable.class,
				Control[].class});
	} catch (Exception e) {
	} catch (NoClassDefFoundError e) {
	} 
	...
	Attribute attr;
		...
		ber.beginSeq(LDAP_REQ_ADD);
		    ...
		    ber.beginSeq(Ber.ASN_SEQUENCE | Ber.ASN_CONSTRUCTOR);
			NamingEnumeration enum = entry.attributes.getAll();
			while (enum.hasMore()) {
