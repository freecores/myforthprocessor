/*
 * @(#)ExtendedTextSourceLabel.java	1.25 03/01/23
 *
...
/*public*/ class ExtendedTextSourceLabel extends ExtendedTextLabel 
  ...
  protected Rectangle2D createAlignBounds() {
    LineMetrics lm = source.getLineMetrics();

    float[] info = getCharinfo();

    float al = 0f;
    float at = -lm.getAscent();
    float aw = 0f;
    float ah = lm.getAscent() + lm.getDescent();
  ...
  protected StandardGlyphVector createGV() {
    ...
    StandardGlyphVector gv = new StandardGlyphVector(f, context, start, length, flags, frc);
  ...
  public float getItalicAngle() {
    return source.getFont().getItalicAngle();
  }

  public LineMetrics getLineMetrics() {
    return source.getLineMetrics();
  }
...
*/
  protected float[] createCharinfo() {
    ...
    float[] glyphinfo = gv.getGlyphInfo();
