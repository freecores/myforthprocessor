...
public class ClassData implements RuntimeConstants {
    ...
    fullname: { // xxx/yyy/zzz
		...
		if (cc=='/') {
		    if (!Character.isJavaIdentifierStart(c)) break fullname;
		} else if (c!='/') {
		    if (!Character.isJavaIdentifierPart(c)) break fullname;
		}
		cc=c;
	    }
	    return name;
