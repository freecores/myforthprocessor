/*
 * @(#)HostnameChecker.java	1.3 03/01/23
 *
...
public class HostnameChecker {
	...
	// IP addresses always end with a (hex) digit
	if (isHexDigit(name.charAt(length - 1)) == false) {
	    return false;
	}

	// literal IPv6 addresses contain at least two colons ':'
	int firstIndex = name.indexOf(':');
	if ((firstIndex == -1) || (name.lastIndexOf(':') == firstIndex)) {
	    return false;
	}

	// hostnames cannot contain colons, must be IPv6 address
	return true;
    }
