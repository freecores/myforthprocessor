...
static JVMDI_capabilities cachedJvmdiCapabilities;
static jboolean           haveCachedJvmdiCapabilities = JNI_FALSE;
...
validateAssignment(jclass enclosingClass, jobject sourceObject, char *destSignature)
{
    ...
    if (match == NULL) {
         *
         * TO DO: This rc is not entirely accurate
         *
...
jint
jvmdiGetCapabilities(JVMDI_capabilities *caps)
{
    if (!haveCachedJvmdiCapabilities) {
        jint error = jvmdi->GetCapabilities(&cachedJvmdiCapabilities);
        if (error != JVMDI_ERROR_NONE) {
            return error;
        }

        /*
         * HotSpot did not initialize the unused capability fields
         * before JVMDI_VERSION_1_3. This means that a newer JPDA
         * backend will see random values when used with an older
         * HotSpot VM. We explicitly initialize newer capability
         * fields to false in JVM/DI versions before they were
         * introduced in order to allow a new JPDA backend to be
         * used with an older VM.
         */
        switch (jvmdiVersion()) {
        case JVMDI_VERSION_1:
        case JVMDI_VERSION_1_1:
        case JVMDI_VERSION_1_2:
            cachedJvmdiCapabilities.can_suspend_resume_thread_lists =
                JNI_FALSE;
            break;
        }
        haveCachedJvmdiCapabilities = JNI_TRUE;
    }

    if (caps != NULL) {
        *caps = cachedJvmdiCapabilities;
    }
...
{
    /*
     * We take advantage of the fact that cachedJvmdiCapabilities
     * is defined in this file. By calling the wrapper with NULL,
     * the cached copy is setup (if not already done before) and
     * then we access it directly.
     */
    jint error = jvmdiGetCapabilities(NULL);
...
classSignature(jclass clazz)
{
    ...
    /*
     * Workaround for classic VM bug 4232335. Translate bogus 
