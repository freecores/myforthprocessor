...
JVM_OnLoad(JavaVM *vm, char *options, void *reserved) 
{
     ...
     */
    jvm = vm;
    error = (*vm)->GetEnv(vm, (void **)&jvmdi, JVMDI_VERSION_1);
    if (error != JNI_OK) {   
        if (error == JNI_EVERSION) {
            /* We get this error both in the case where -Xdebug wasn't
             * specified, and when the VM doesn't support JVMDI_VERSION_1.
             */
            fprintf(stderr, "JDWP unable to access JVMDI Version 1.\nMaybe you need to start the VM with the -Xdebug option\n");
        } else {
            fprintf(stderr, "JDWP unable to initialize: Error %d from JNI GetEnv\n", error);
        }
        return -1;
    }
