 ...
 * If the eventKind field is non-zero, then one of the possible
...
resumeThreadByNode(ThreadNode *node) 
{
    ...
    if (node->suspendCount > 0) {
        ...
        if ((node->suspendCount == 0) && node->toBeResumed && 
            ...
            if (error == JVMDI_ERROR_INVALID_THREAD && !node->isStarted) {
                /*
 ...
 *    entire thread list from JVMDI so we use the runningThreads
...
commonResumeList()
{
    ...
    reqList = jdwpAlloc(reqCnt * sizeof(jthread));
    if (reqList == NULL) {
        ERROR_MESSAGE_EXIT("Unable to allocate resume request list");
    }
    results = jdwpAlloc(reqCnt * sizeof(jvmdiError));
    if (results == NULL) {
        ERROR_MESSAGE_EXIT("Unable to allocate resume list results");
    }
    ...
    error = jvmdi->ResumeThreadList(reqCnt, reqList, results);
    for (i = 0; i < reqCnt; i++) {
        ThreadNode *node = findThread(env, &runningThreads, reqList[i]);
        if (node == NULL) {
            ERROR_MESSAGE_EXIT("Unable to find entry in thread table");
        }
...
commonSuspendList(jint initCount, jthread *initList) 
{
    ...
    reqList = jdwpAlloc(initCount * sizeof(jthread));
    if (reqList == NULL) {
        ERROR_MESSAGE_EXIT("Unable to allocate suspend request list");
    }
    ...
    if (reqCnt > 0) {
        jvmdiError *results = jdwpAlloc(reqCnt * sizeof(jvmdiError));

        if (results == NULL) {
            ERROR_MESSAGE_EXIT("Unable to allocate suspend list results");
        }

        /*
         * We have something to suspend so try to do it.
         */
        error = jvmdi->SuspendThreadList(reqCnt, reqList, results);
        for (i = 0; i < reqCnt; i++) {
            ...
            if (results[i] == JVMDI_ERROR_NONE) {
                /* thread was suspended as requested */
                node->toBeResumed = JNI_TRUE;
            } else if (results[i] == JVMDI_ERROR_THREAD_SUSPENDED) {
                /*
                 * If the thread was suspended by another app thread,
                 * do nothing and report no error (we won't resume it later).
                 */
                results[i] = JVMDI_ERROR_NONE;
            } else if (results[i] == JVMDI_ERROR_INVALID_THREAD) {
                /*
...
threadControl_suspendAll() 
{
    ...
    jint count;
    int i;
    jboolean foundNewThread = JNI_FALSE;
...
threadControl_clearCLEInfo(JNIEnv *env, jthread thread)
{
    ...
    node = findThread(env, &runningThreads, thread);
    if (node != NULL) {
        node->cleInfo.eventKind = 0;
        if (node->cleInfo.clazz != 0) {
            (*env)->DeleteGlobalRef(env, node->cleInfo.clazz);
            node->cleInfo.clazz = 0;
        }
    }
                         ...
                         jmethodID method, jlocation location)
{
    ...
    node = findThread(env, &runningThreads, thread);
    if (node != NULL && node->cleInfo.eventKind != 0 &&
        node->cleInfo.method == method &&
        node->cleInfo.location == location &&
        (*env)->IsSameObject(env, node->cleInfo.clazz, clazz)) {
...
void
threadControl_saveCLEInfo(JNIEnv *env, jthread thread, jint kind,
 ...
 * bogus. In that case, we get the location through other means.
 */
...
jboolean 
threadControl_getPopFrameEvent(jthread thread)
{
    JNIEnv *env = getEnv();
    ThreadNode *node = findAnyThread(env, thread);
    ...
    return node->popFrameEvent;
}

void
threadControl_setPopFrameEvent(jthread thread, jint value)
{
    JNIEnv *env = getEnv();
    ThreadNode *node = findAnyThread(env, thread);
...
jboolean 
threadControl_getPopFrameProceed(jthread thread)
{
    JNIEnv *env = getEnv();
    ThreadNode *node = findAnyThread(env, thread);
    ...
    return node->popFrameProceed;
}

void
threadControl_setPopFrameProceed(jthread thread, jint value)
{
    JNIEnv *env = getEnv();
    ThreadNode *node = findAnyThread(env, thread);
