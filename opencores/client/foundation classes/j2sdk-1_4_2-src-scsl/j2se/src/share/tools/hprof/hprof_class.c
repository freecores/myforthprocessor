...
static UINTPTR_T hash_class(void *_class)
{
    hprof_class_t *class = _class;
    return ((UINTPTR_T)class->class_id >> HASH_OBJ_SHIFT) %
...
hprof_class_t * hprof_intern_class(jobjectID class_id)
{
    hprof_class_t *class;
    hprof_class_t class_tmp;
    ...
    class_tmp.class_id = objmap;
    objmap->is_class = TRUE;
    class = hprof_hash_lookup(&hprof_class_table, &class_tmp);
    if (class == NULL) {
       /* Add a placeholder which should be filled during class_load event */
       class = hprof_hash_put(&hprof_class_table, &class_tmp);
       class->serial_num = 0;
       class->super = NULL;
       class->name = NULL;
       class->src_name = NULL;
       class->num_interfaces = 0;
       class->num_statics = 0;
       class->statics = NULL;
			    ...
			    int requested)
{
    ...
    class_tmp.class_id = objmap;
    if ((result = hprof_hash_lookup(&hprof_class_table, &class_tmp)) != NULL) {
        /* A class can only be created before its class load event 
         * is due to an unknown class ID in an OBJECT_ALLOC event.
         * It must have serial_num = 0.
         */
        if (!objmap->is_class || result->serial_num != 0) {
			      ...
			      jobjectID class_id)
{
  ...
  CALL(RawMonitorEnter)(data_access_lock); 
  objmap = hprof_fetch_unloaded_class_object_info(class_id);
  if (objmap == NULL) {
      fprintf(stderr, "HPROF ERROR: fail to fetch object info in class_unload\n");
  } else {
      class = hprof_lookup_class_objmap(objmap);
      if (class == NULL) {
      ...
      hprof_free_unloaded_class_objmap(objmap);
  }
  CALL(RawMonitorExit)(data_access_lock);
}
