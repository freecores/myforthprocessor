...
INT32 total_live_bytes = 0;
INT32 total_live_instances = 0;

/*
 * PRESERVE THIS LOCK ORDER TO AVOID DEADLOCKS!!!
 *
 * thread_list_lock => data_access_lock
 * thread_list_lock => DisableGC/EnableGC => data_access_lock
 *
 */
...
/*
 * The hprof_dump_lock is used to control modifications to the
 * dump_in_process flag and the jvm_shut_down flag. hprof data is
 * dumped in response to two different events:
 * 1) JVMPI_EVENT_JVM_SHUT_DOWN, if dump_on_exit option is true
 * 2) JVMPI_EVENT_DUMP_DATA_REQUEST (typically sent when VM receives
 *    a SIGQUIT)
 *
 * The dump_in_process flag prevents races between the two events
 * and is also used by the "HPROF CPU profiler" thread to determine
 * if a pause in sampling is needed.
 *
 * The jvm_shut_down flag prevents multiple calls to
 * hprof_jvm_shut_down_event().
 */
JVMPI_RawMonitor hprof_dump_lock;
int dump_in_process = FALSE;
int jvm_shut_down   = FALSE;
...
static void *_hprof_calloc(SIZE_T size);
static char *alloc_type_name(unsigned int tag);

void *hprof_calloc(SIZE_T size)
{
    return hprof_calloc_tag(ALLOC_TYPE_OTHER, size);
}

void *hprof_calloc_tag(unsigned int tag, SIZE_T size)
{
    unsigned int *buf;
...
/*
 * Flag that determines if we can use JVM/PI SuspendThreadList() and
 * ResumeThreadList().
 */
int can_use_suspend_resume_thread_lists;
...
JNIEXPORT jint JNICALL JVM_OnLoad(JavaVM *vm, char *options, void *reserved)
{
    ...
    if (hprof_jvm_interface->version >= JVMPI_VERSION_1_2) {
	can_use_suspend_resume_thread_lists = 1;
    } else {
	can_use_suspend_resume_thread_lists = 0;
    }
...
{
    if (cpu_sampling || cpu_timing || monitor_tracing) {
...
{
    /*
     * Use local flag to minimize hprof_dump_lock hold time.
     */
    int need_to_dump = FALSE;

    CALL(RawMonitorEnter)(hprof_dump_lock);

    if (jvm_shut_down) {
	/* we already got this event once so bail this instance */
        CALL(RawMonitorExit)(hprof_dump_lock);
	return;
    }
    jvm_shut_down = TRUE;

    if (hprof_is_on && !dump_in_process) {
        need_to_dump    = TRUE;
        dump_in_process = TRUE;
        /*
         * Setting dump_in_process will cause cpu sampling to pause
         * (if we are sampling). We don't resume sampling after the
         * hprof_dump_data() call below because the VM is shutting
         * down.
         */
    }
    CALL(RawMonitorExit)(hprof_dump_lock);
    ...
    if (hprof_socket_p) {
         ...
         */
        shutdown(hprof_fd, 2 /* disallow sends and receives */);
    }
    close(hprof_fd);
}  
    ...
    case JVMPI_EVENT_DUMP_DATA_REQUEST:
        {
            /*
             * Use local flag to minimize hprof_dump_lock hold time.
             * Note: the original code didn't check hprof_is_on so
             * this rewrite doesn't either. That may be a bug.
             */
            int need_to_dump = FALSE;
            ...
            if (need_to_dump) {
                ...
                CALL(RawMonitorEnter)(hprof_dump_lock);
                dump_in_process = FALSE;
                CALL(RawMonitorExit)(hprof_dump_lock);

                if (cpu_sampling && !jvm_shut_down) {
                    hprof_cpu_sample_on(NULL); /* resume sampling */
                }
            }
            return;
        }
