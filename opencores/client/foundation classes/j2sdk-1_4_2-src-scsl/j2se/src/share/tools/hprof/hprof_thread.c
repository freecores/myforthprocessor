...
static void hprof_free_thread_local_info(JNIEnv *env_id, hprof_thread_local_t *info);
static void hprof_do_bill_frames(JNIEnv *env_id, hprof_thread_local_t* info);

static unsigned int thread_serial_number = 1;

static hprof_hash_t hprof_thread_table;

/* list of active threads protected by thread_list_lock */
JVMPI_RawMonitor thread_list_lock;
live_thread_t *live_thread_list = NULL;
int num_live_threads = 0;

static UINTPTR_T hash_thread(void *_thread)
{
    hprof_thread_t *thread = _thread;
    return (UINTPTR_T)thread->env_id % hprof_thread_table.size;
}
...
static int compare_thread(void *_thread1, void *_thread2)
{
    ...
    return (int)((UINTPTR_T)thread1->env_id - 
		 (UINTPTR_T)thread2->env_id);
}
...
void hprof_thread_end_event(JNIEnv *env_id)
{
    ...
    info = NULL;
    if (cpu_timing || monitor_tracing) {
        /* Get the thread local for this thread before acquiring the lock */
...
#ifdef XXX_HASH_STATS
{
    ...
    if (thread == NULL) {
        ...
        if (cpu_timing) {
          ...
          hprof_do_bill_frames(env_id, info);
        }
        if (cpu_timing || monitor_tracing) {
          /* free the thread local table if we allocated one */
          hprof_free_thread_local_info(env_id, info);
          /* reset thread local storage pointer */
...
{
    jlong self_time = fc->self_time / (jlong)(1000000);/* convert to ms */
    jlong cost = 0;
	...
	cost = (fc->total_time) / (jlong)(1000000);    /* convert to ms */
	bill_it = 1;
    } else if ((timing_format == NEW_PROF_OUTPUT_FORMAT) && (self_time > 0)) {
        cost = self_time;
	bill_it = 1;
    }
...
static void * hprof_free_thread_table_helper(void *_thread, void *_arg)
{
    ...
    info = (hprof_thread_local_t *)(CALL(GetThreadLocalStorage)(thread->env_id));
    if (info == NULL) {
        fprintf(stderr, 
		"HPROF ERROR: thread local table NULL for env_id = %p," 
		" cannot free it\n", thread->env_id);
    ...
    hprof_free_thread_local_info(thread->env_id, info);
    return _arg;
}
