...
/* rehash a bucket in the hash table:
 * - (*f)(void*, void*) returns non-zero if the bucket's content 
 *   is the one to be rehashed;
 * - arg is the argument to pass as the second argument to f.
 * Note that the element (the content in a bucket) shall be updated
 * before f returns.
 */ 
void
hprof_rehash(hprof_hash_t *table, 
	     int (*f)(void *, void *),
	     void *arg)
{
    int i;
	...
	while ((bucket = *p)) {
	    if (f(bucket->content, arg)) {
	        *p = bucket->next;
                index = table->hash_f(bucket->content);
                bucket->next = table->entries[index];
                table->entries[index] = bucket;
	        return;
	    }
	    p = &(bucket->next);
	}
...
hprof_hash_lookup(hprof_hash_t *table, 
		  void *new)
{
    int index = (int)(table->hash_f(new) % (UINTPTR_T)table->size);
...
hprof_hash_put(hprof_hash_t *table, 
	       void *new)
{
	unsigned int   a_type = ALLOC_TYPE_OTHER;
