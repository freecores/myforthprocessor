...
static char *SECT_PATTERN      = "%d\t%ld\t%ld\t\t%ld\n";
static char *DELIM             = " \t";
...
static Bool read_line(void) {
    ...
    while (ch != '\n') {
        if (tmp - line >= MAX_JCOV_LINE_LEN) {
            sprintf(buf, "line #%d is too long", line_number);
            ERROR_AND_EXIT(buf);
        }
        *(tmp++) = (char)ch;
        ch = getc(result_file);
...
#define CHK_STATE(old_state, new_state) \
    if (state != old_state) { \
        jcov_close(&temp_file); \
        jcov_remove(temp_file_name); \
        sprintf(buf, "malformed jcov file (line %d)", line_number); \
        jcov_error_stop(buf); \
    } state = new_state
#define CHK_TOKEN \
    if (token == NULL) { \
    ...
    while (read_line()) {
        ...
        if (EQUALS(token, KEYWORD_CLASS)) {
            ...
            if (strlen(token) > 1 || 
                ...
                sprintf(buf, "bad DATA section (line %d)", line_number);
                ERROR_AND_EXIT(buf);
            }
            ...
            if (filters_total > filters_max) {
                ...
                for (i = 0; i < filters_total; tmp_filters[i] = filters[i++]);
                jcov_free(filters);
                filters = tmp_filters;
            }
            ...
            if (jcov_item_ind > ct_size - 1) {
                sprintf(buf, "numbers of jcov items don't match (line %d)", line_number);
                ERROR_AND_EXIT(buf);
            }
            do {
            ...
            if (jcov_item.type != ji->type || jcov_item.where_line != ji->where_line ||
                ...
                sprintf(buf, "jcov items don't match (line %d)", line_number);
                ERROR_AND_EXIT(buf);
            }
...
void save_jcov_data(char *filename) {
    ...
    if (access(filename, F_OK) == -1 || overwrite_jcov_file) {
