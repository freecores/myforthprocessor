...
#undef BASE
    ...
    case JVMPI_EVENT_RAW_MONITOR_CONTENDED_EXIT:
        sprintf(info,
                "%s %s [0x" PTR_FORMAT "], thread " PTR_FORMAT,
                get_event_name(event->event_type),
                event->u.raw_monitor.name,
                (intptr_t)event->u.raw_monitor.id,
                (intptr_t)event->env_id);
        jcov_info(info);
        return;
    ...
    case JVMPI_EVENT_MONITOR_CONTENDED_EXIT:
        sprintf(info, "%s [0x" PTR_FORMAT "], thread: " PTR_FORMAT,
                get_event_name(event->event_type), 
                (intptr_t)event->u.monitor.object, 
                (intptr_t)event->env_id);
        jcov_info(info);
        return;
    ...
    case JVMPI_EVENT_MONITOR_WAITED:
        sprintf(info, "%s [0x" PTR_FORMAT "], thread: " PTR_FORMAT,
                get_event_name(event->event_type), 
                (intptr_t)event->u.monitor_wait.object, 
                (intptr_t)event->env_id);
        jcov_info(info);
        return;
    default:
...
JNIEXPORT jint JNICALL JVM_OnLoad(JavaVM *vm, char *options, void *reserved) {
    ...
    jvm = vm;
    res = (*jvm)->GetEnv(jvm, (void**)&jvmpi_interface, JVMPI_VERSION_1_1);
    if (res < 0) {
