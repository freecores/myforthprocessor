...
static JVMPI_RawMonitor hprof_monitor_lock;
...
static UINTPTR_T hash_raw_monitor(void *_rmon)
{
    hprof_raw_monitor_t *rmon = _rmon;

    SANITY_CHECK("hash_raw_monitor");
    return ((UINTPTR_T)rmon->id + 37*(UINTPTR_T)rmon->name) %
...
static int compare_raw_monitor(void *_rmon1, void *_rmon2)
{
    ...
    result = (int)((UINTPTR_T)rmon1->name - (UINTPTR_T)rmon2->name);
    if (result) {
        return result;
    }

    result = (int)((UINTPTR_T)rmon1->id - (UINTPTR_T)rmon2->id);
    return result;
}
...
static UINTPTR_T hash_contended_monitor(void *_cmon)
{
    hprof_contended_monitor_t *cmon = _cmon;
    UINTPTR_T hash;

    SANITY_CHECK("hash_contended_monitor");
    hash = (UINTPTR_T)cmon->type + 37*(UINTPTR_T)cmon->mon_info;
    return (37*hash + (UINTPTR_T)cmon->trace_serial_num) %
...
static int compare_contended_monitor(void *_cmon1, void *_cmon2)
{
    ...
    result = (int)((UINTPTR_T)cmon1->mon_info - (UINTPTR_T)cmon2->mon_info);
    return result;
}
...
void hprof_output_cmon_times(float cutoff)
{
    ...
    iterate.total_time = iterate.index = 0;
    hprof_hash_iterate(&hprof_contended_monitor_table, 
	...
	percent = (float)cmon->time / (float)iterate.total_time;
	if (percent < cutoff) {
	    break;
	}
	n_items++;
    ...
    if (output_format == 'a') {
        time_t t = time(0);
	int total_time = (int)(iterate.total_time/(jlong)1000000);
	...
	if (total_time > 0) {
		...
		percent = (float)cmon->time / (float)iterate.total_time * 100.0;
		accum += percent;
					    ...
					    jlong cur_time)
{
    ...
    info = info = (hprof_thread_local_t *)(CALL(GetThreadLocalStorage)(env_id));
    if (info == NULL) {
        fprintf(stderr, 
		"HPROF ERROR: thread local table NULL in contended monitor " 
		"entered %p\n", env_id);
    ...
    if (imon == NULL) {
        mon->time = cur_time - mon->time;
	mon->num_hits = 1;
	hprof_hash_put(&hprof_contended_monitor_table, mon);
    } else {
        imon->time += (cur_time - mon->time);
	imon->num_hits++;
    }
			     ...
			     JVMPI_RawMonitor mid)
{
    ...
    if (mid == data_access_lock) return;
