...
static hprof_objmap_t *unloaded_class_objmap_list = NULL;
...
#define HASH_OBJ(obj) (((UINTPTR_T)obj >> HASH_OBJ_SHIFT) % table_size)
...
static void hprof_objmap_del(jobjectID obj_id)
{
    ...
    while ((bucket = *p)) {
	    ...
	    *p = bucket->next;
            if (bucket->is_class) {
                bucket->next = unloaded_class_objmap_list;
                unloaded_class_objmap_list = bucket;
            } else {
...
void hprof_objmap_print(hprof_objmap_t *objmap)
{
    ...
    case JVMPI_NORMAL_OBJECT:
        hprof_printf("%s", (hclass && hclass->name != NULL) ? hclass->name->name : "<Unknown_class>");
	break;
    case JVMPI_CLASS:
        hprof_printf("[L%s;", (hclass && hclass->name != NULL) ? hclass->name->name : "<Unknown_class>");
	break;
    case JVMPI_BOOLEAN:
        hprof_printf("[Z");
	break;
...
{ 
    ...
    objmap = hprof_objmap_lookup(obj_id);
    if (requested) {
        if (objmap) {
            /* objmap has been created due to another OBJECT_ALLOC event */
	    goto done;
	}
        trace_num = 0;
    } else {
    ...
    if (objmap) {
        /* If an objmap has been created and this OBJECT_ALLOC must be
         * a normal OBJECT_ALLOC event (i.e. not requested), update
         * the trace num to the real allocation site for this object.
         * This objmap was created on a requested OBJECT_ALLOC.  
         * For example, a class ID for an object is unknown when
         * handling OBJECT_ALLOC event for that object.  So an objmap
         * with trace=0 is temporarily created for that class ID.
         */
        if (objmap->site->trace_serial_num == 0) {
            /* Now update the real allocation site for this objmap. */ 
            hprof_site_update_trace_serial_num(objmap->site, trace_num);
        } else if (objmap->site->trace_serial_num != trace_num) {
	    fprintf(stderr, "HPROF ERROR: duplicate obj_id in object_alloc\n");
        }
    } else {
...
hprof_objmap_t* hprof_fetch_unloaded_class_object_info(jobjectID obj_id)
{
    hprof_objmap_t *objmap = unloaded_class_objmap_list;
    for (; objmap != NULL; objmap = objmap->next) {
        if (objmap->obj_id == obj_id) {
            return objmap;
        }
    }
    /* If not in unloaded_class_objmap_list, CLASS_UNLOAD event is posted 
     * before OBJECT_FREE event for the same object.
     */
    return hprof_fetch_object_info(obj_id);
}

void hprof_free_unloaded_class_objmap(hprof_objmap_t *objmap)
{
    hprof_objmap_t **p = &unloaded_class_objmap_list;
    hprof_objmap_t *bucket;
    while ((bucket = *p)) {
        if (bucket == objmap) {
	    *p = bucket->next;
    ...
    /* If not in unloaded_class_objmap_list, clear is_class flag so that
     * the objmap will be removed when receiving OBJECT_FREE event.
     */
    objmap->is_class = FALSE;
    return;
}
