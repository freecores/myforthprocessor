/*
 * @(#)hprof_heapdump.c	1.14 03/01/23
 *
				      ...
				      char *end_objects)
{
	...
	case HPROF_GC_ROOT_STICKY_CLASS: {
		...
		const char *class_name = (class == NULL || class->name == NULL) ? "<Unknown>" : class->name->name;
	...
	case HPROF_GC_CLASS_DUMP: {
	    ...
	    if (class == NULL) {
			...
			class_id, class_map);
		return;
	    } else if (class->serial_num == 0) {
	        fprintf(stderr, "HPROF ERROR: no class_load event for class_id in heap dump %p %p\n",
			class_id, class_map);
		return;
            }
	...
	case HPROF_GC_INSTANCE_DUMP: {
	    ...
	    class_name = (class == NULL || class->name == NULL) ? "<Unknown>" : class->name->name;
