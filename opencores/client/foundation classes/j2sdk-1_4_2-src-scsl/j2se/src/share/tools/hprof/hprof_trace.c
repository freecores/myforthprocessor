...
static UINTPTR_T hash_trace(void * _trace)
{
    hprof_trace_t *trace = _trace;
    UINTPTR_T hash = (UINTPTR_T)trace->thread_serial_num + 37 * (UINTPTR_T)trace->n_frames;
    int i;
    for (i = 0; i < trace->n_frames; i++) {
        hash = hash * 37 + ((UINTPTR_T)trace->frames[i] >> HASH_OBJ_SHIFT);
    }
...
static int compare_trace(void *_trace1, void *_trace2)
{
    ...
    for (i = 0; i < trace1->n_frames; i++) {
        result = (int)((UINTPTR_T)trace1->frames[i] - (UINTPTR_T)trace2->frames[i]);
	if (result) {
	    return result;
	}
    }
...
static UINTPTR_T hash_frame(void *_frame)
{
    hprof_frame_t *frame = _frame;
    return (((UINTPTR_T)frame->method >> HASH_OBJ_SHIFT) +
	31 * (UINTPTR_T)frame->lineno) % hprof_frame_table.size;
}
...
static int compare_frame(void *_frame1, void *_frame2)
{
    ...
    return (int)((UINTPTR_T)frame1->method - (UINTPTR_T)frame2->method);
}
...
static void hprof_output_trace(hprof_trace_t *trace)
{
	...
	for (i = 0; i < trace->n_frames; i++) {
	    ...
	    hprof_method_t *method = frame->method;
	    hprof_class_t *class = method->class;
	    const char *srcname = (class->src_name ? class->src_name->name : "Unknown_class");
	    const char *classname = (class->name ? class->name->name : "Unknown_class");
...
static int hprof_trace_compare_cost(const void *p_trace1, const void *p_trace2)
{
    ...
    return trace2->cost - trace1->cost;
}
...
hprof_output_trace_cost(float cutoff, char *record_name)
{
	...
	percent = (float)trace->cost / (float)iterate.total_count;
	if (percent < cutoff) {
	    break;
	}
	n_items++;
	...
	for (i = 0; i < n_items; i++) {
	    ...
	    percent = (float)trace->cost / (float)iterate.total_count * 100.0;
	    accum += percent;
		...
		hprof_method_t *method = frame->method;
		hprof_printf(" %s.%s\n", (method->class->name ? method->class->name->name : "Unknown_class"), 
	...
	for (i = 0; i < n_items; i++) {
	    hprof_trace_t *trace = iterate.traces[i];
	    hprof_write_u4((unsigned int)trace->cost);
	    hprof_write_u4(trace->serial_num);	    
	}
    }
...
void hprof_output_trace_cost_in_prof_format(void) 
{
	...
	if (num_frames >= 1) {
	    ...
	    hprof_printf("%s.%s%s ", 
			 (callee->class->name ? callee->class->name->name : "Unknown_class"), 
	...
	if (num_frames > 1) {
	    ...
	    hprof_printf("%s.%s%s ", 
			 (caller->class->name ? caller->class->name->name : "Unknown_class"), 
