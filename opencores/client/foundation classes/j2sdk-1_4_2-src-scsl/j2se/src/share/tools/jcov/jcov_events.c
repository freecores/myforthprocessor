...
static Bool jcov_parse_class_data(JNIEnv *env_id, bin_class_context_t *context) {
    ...
    if (!non_abstract_met) {  /* interface encountered - skip it */
        jcov_free_hooked_class_full(hooked_class);
        jcov_free_constant_pool(ctx.cp, ctx.cp_size);
        return FALSE;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (!strcmp(cp_entry->u.Utf8.bytes, ABS_SRC_PATH_ATTR_NAME)) {
            ...
            sprintf(hooked_class->timestamp,SIZE_FORMAT, tstamp.timestamp);
...
void jcov_class_load_event(JVMPI_Event *event) {
    ...
    for (i = 0; i < class->num_methods; i++) {
        ...
        CHK(ind == -1, "jcov_class_load_event: hooked class method not found");
        last_matched = ind;
...
void jcov_object_move_event(JVMPI_Event *event) {
    ...
    if (found_class) {
         ...
         */
        found_class->id_sav = found_class->id;
        found_class->id = event->u.obj_move.new_obj_id;
        if (verbose_mode > 1) {
    ...
    if (found_id) {
        found_id->id_sav = found_id->id;
        found_id->id = event->u.obj_move.new_obj_id;
    }
}
...
static void *store_moved_class(void *class_, void *arg) {
...
static void *store_moved_id(void *id_, void *arg) {
...
void jcov_gc_finish_event(void) {
    ...
    jcov_hash_iterate(class_id_table, store_moved_class, (void*)(&moved_classes_list));
    ...
    for (l = moved_classes_list; l != NULL; l = l->next) {
        ...
        remove_class_by_id(class_id_table, class->id_sav);
        class->id_sav = NULL;
    }
    for (l = moved_classes_list; l != NULL; l = l->next) {
        jcov_class_t *class = (jcov_class_t*)l->elem;
    ...
    if (load_early_classes) {
        jcov_hash_iterate(class_filt_table, store_moved_id, (void*)(&moved_id_list));
        for (l = moved_id_list; l != NULL; l = l->next) {
            jcov_class_id_t *id = (jcov_class_id_t*)l->elem;
            remove_classID(class_filt_table, id->id_sav);
            id->id_sav = NULL;
        }
        for (l = moved_id_list; l != NULL; l = l->next) {
            jcov_class_id_t *id = (jcov_class_id_t*)l->elem;
...
void jcov_thread_start_event(JVMPI_Event *event) {
    ...
    if (verbose_mode > 0) {
        ...
        sprintf(info, "THREAD_START : %s [" PTR_FORMAT "]", name, (intptr_t)env_id);
        jcov_info(info);
    }
...
void jcov_thread_end_event(JVMPI_Event *event) {
    ...
    if (verbose_mode > 0) {
        sprintf(info, "THREAD_END : " PTR_FORMAT, (intptr_t)event->env_id);
        jcov_info(info);
    }
    LOCK(threads);
    ...
    if (thread != NULL) {
        ...
        sprintf(info, "jcov_thread_end_event: thread not found: " PTR_FORMAT, (intptr_t)event->env_id);
        jcov_warn(info);
    }
}
    ...
    if (m == NULL && load_early_classes && jcov_java_init_done) {
        ...
        *((char*)mem) = state_sav;
        ...
        if (res != JVMPI_SUCCESS && verbose_mode > 1) {
            sprintf(info, "could not request CLASS_LOAD for 0x" PTR_FORMAT "\n", (intptr_t)class_id);
            jcov_warn(info);
            return;
        }
        LOCK(methods);
