 ...
 *------------------------------------------------------------------------------
 */
void calcGtbl3 (fut_gtbldat_p tables[], KpInt32_t gridSizes[], 
				double_p rows[], double offset)
{
double			input[3], temp, scale;
KpInt32_t		i, j, k, row, col, it;

	scale = (double) FUT_GRD_MAXVAL;
		...
		for (j = 0; j < gridSizes[1]; j++) {
			...
			for (i = 0; i < gridSizes[2]; i++, it++) {
				...
				for (row = 0; row < 3; row++) {
				    ...
				    temp = offset;	 /* include offset for extended range, if required:  */
					for (col = 0; col < 3; col++) {
						temp += rows[row][col] * input[col];
					}

				     /* Quantize for 12-bit grid table and insert:  */
					tables[row][it] = QUANT (temp, scale);
				}
			}
		}
	}
