...
processQuadratic(doeE env, dcPathStorage sp, f32* pnts, i32 level)
{
    ...
    f32            difs[4], mods[2];
    if (arcsQuadraticDifsAndMods(difs, mods, pnts, DEGENERATE_BRANCH)) {
	...
	if (doeError_occurred(env)) return;
        ...
        if (level >= LEVEL_MAX || quadraticHasUAV(difs, tans)) {
...
processCubic(doeE env, dcPathStorage sp, f32* pnts, i32 level)
{
    dcPathConsumer spc = (dcPathConsumer) sp;
    f32            difs[6], mods[3];
    if (arcsCubicDifsAndMods(difs, mods, pnts, DEGENERATE_BRANCH)) {
	...
	if (doeError_occurred(env)) return;
        ...
        if (level >= LEVEL_MAX || cubicHasUAV(difs, tans)) {
