...
#define EVAL_HIGH_MASK (0xfffff000)
...
{ \
KpInt32_t	dividendShifted; \
 \
	dividendShifted = (dividend) << EVAL_FRACBITS;				/* get extra precision */ \
...
KpInt32_t	sPosition, interpolant, tableData, nextTableData, index;
	...
	if (index < tableEntries -1) {
		...
		nextTableData = tableData;
	}

	interpolateDelta (tableData, nextTableData, interpolant, nextTableData)
	nextTableData += ROUND_VALUE(EVAL_FRACBITS - EVAL_EXTENDED_BITS);
	KCP_SHIFT_RIGHT(nextTableData, nextTableData, EVAL_FRACBITS - EVAL_EXTENDED_BITS);
