 ...
 *	@(#)malloc.c	1.2 98/09/22
	...
	Contains:	allocate and free, lock and unlock memory used for
			input, output, and grid tables
...
KpInt32_t	size;
	...
	itbl->tbl = (fut_itbldat_p) fut_malloc (size);
	if (itbl->tbl != NULL) {
	...
	fut_alloc_gtbldat (	fut_gtbl_p	gtbl)
{
	...
	gtbl->tbl = (fut_gtbldat_p) fut_malloc (gtbl->tbl_size);
	if (gtbl->tbl != NULL) {
...
KpInt32_t	size;
	...
	otbl->tbl = (fut_otbldat_p) fut_malloc (size);
	if (otbl->tbl != NULL) {
						...
						KpInt32_t	nEntries)
{
	...
	itbl->refTbl = (mf2_tbldat_p) fut_malloc (nEntries * sizeof (mf2_tbldat_t));
...
KpInt32_t	theSize;
	...
	gtbl->refTbl = (mf2_tbldat_p) fut_malloc (theSize);
	if (gtbl->refTbl != NULL) {
						...
						KpInt32_t	nEntries)
{
	...
	otbl->refTbl = (mf2_tbldat_p) fut_malloc (nEntries * sizeof (mf2_tbldat_t));
	if (otbl->refTbl != NULL) {
