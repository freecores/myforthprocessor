...
static imageIODataPtr initImageioData (JNIEnv *env, 
    ...
    data->imageIOobj = (*env)->NewGlobalRef(env, obj);
    if (data->imageIOobj == NULL) {
        free (data);
        return NULL;
    }
    if (initStreamBuffer(env, &data->streamBuf) == NOT_OK) {
        (*env)->DeleteGlobalRef(env, data->imageIOobj);
        free (data);
        return NULL;
    }
...
Java_com_sun_imageio_plugins_jpeg_JPEGImageReader_resetReader
    ...
    cinfo->src->term_source(cinfo);  /* Currently a no-op, but might not always be */
    cinfo->src->bytes_in_buffer = 0;
    cinfo->src->next_input_byte = NULL;    
}
