/*
 * @(#)GenAudioStreams.c	1.71 03/03/14
 *
...
void GM_SetStreamResample(GM_AudioStream* pStream, XBOOL enable) {
	...
	if (enable) {
...
#ifdef DEBUG
		fprintf(stderr, "GenAudioStreams:SetStreamResample Creating resampleParams for voice %d\n", (int) pStream->playbackReference); fflush(stderr);
#endif
...
#ifdef DEBUG
		fprintf(stderr, "GenAudioStreams:SetStreamResample. Setting pStream=%p voice to %d \n", 
			pStream, (int) pStream->playbackReference); fflush(stderr);
#endif
	    }
	} else {
	    /* resample off */
...
#ifdef DEBUG
		    fprintf(stderr, "GenAudioStreams:SetStreamResample: setting external to NULL on voice %d\n",(int) pStream->resampleVoice); fflush(stderr);
#endif
		...
		SR_exit(params);
#ifdef DEBUG
		fprintf(stderr, "GenAudioStreams:SetStreamResample: Disposing resampleParams\n"); fflush(stderr);
#endif
		XDisposePtr((XPTR)params);
	    }
...
static void PV_AudioBufferCallback(void *context, XPTR pWhichBufferFinished, INT32 *pBufferSize_IN_OUT)
{
    ...
    if (pStream)
	{
		...
		sprintf(text, "Playback Pos %ld Length %ld\r", pStream->streamPlaybackPosition, *pBufferSize_IN_OUT);
		DEBUG_STR(text);
	    }
#endif
#if USE_DEBUG && 0
		...
		sprintf(text, "buffer done %lx size %ld", pWhichBufferFinished, *pBufferSize_IN_OUT);
		DEBUG_STR(text);
	    }
#endif
    ...
    if (pStream->streamShuttingDown == FALSE)
	{
		...
		case STREAM_MODE_START_BUFFER_1:
		    ...
		    if (pStream->streamLength1)
			{
...
#ifdef DEBUG
				    fprintf(stderr, "GenAudioStreams:PV_PrepareThisBufferForPlaying: call to GM_SetSampleResampleExtern. Setting pStream=%p voice to %d \n", 
					    pStream, (int) pStream->playbackReference); fflush(stderr);
#endif
				    GM_SetSampleResampleExtern(pStream->playbackReference, pStream->resampleParams);
				    pStream->resampleVoice = pStream->playbackReference;
				...
				fprintf(stderr, "GenAudioStreams:PV_PrepareThisBufferForPlaying: call to GM_SetupSampleDoubleBuffer failed. playbackReference is NULL\n"); 
				fflush(stderr); 
			    }
#endif
...
static OPErr PV_FileStreamCallback(void *context, GM_StreamMessage message, GM_StreamData *pAS)
{
	...
	case STREAM_GET_DATA:
	    ...
	    if (pAS->pData)
		{
			...
			sprintf(text, "STREAM_GET_DATA::frames %ld outputBufferSize %ld fileSize %ld", pAS->dataLength, outputBufferSize, fileSize);
			DEBUG_STR(text);
		    }
#endif
				     ...
				     char channelSize)		// 1 or 2 channels of date
{
    ...
    reference = DEAD_STREAM;
    theErr = NO_ERR;
    DEBUG_STR("StreamSetup: Starting.\n");
...
#ifdef DEBUG
		    fprintf(stderr, "GenAudioStreams:GM_AudioStreamStop: playbackReference set to NULL\n"); fflush(stderr); 
#endif
		    // stop current voice
		    pStream->playbackReference = DEAD_VOICE;
...
#ifdef DEBUG
		    fprintf(stderr, "GenAudioStreams:GM_AudioStreamFlush. playbackReference set to NULL\n"); fflush(stderr); 
#endif
		    // stop current voice
		    pStream->playbackReference = DEAD_VOICE;
		 ...
		 (pStream->streamUnderflow && (pStream->streamPaused == FALSE)))
		{
		    ...
		    if (theProc)
			{
				...
				case STREAM_MODE_START_BUFFER_2:		// read buffer 1 into memory
					...
					sprintf(text, "B1-> %ld len %ld", pStream->streamPlaybackPosition, pStream->streamLength1);
					DEBUG_STR(text);
				    }
#endif
				    break;
				case STREAM_MODE_START_BUFFER_1:		// read buffer 2 into memory
					...
					sprintf(text, "B2-> %ld len %ld", pStream->streamPlaybackPosition, pStream->streamLength2);
					DEBUG_STR(text);
				    }
#endif
				    break;
...
OPErr GM_StartLinkedStreams(LINKED_STREAM_REFERENCE pTop)
{
    ...
    if (MusicGlobals)
	{
	    ...
	    while (pNext)
		{
		    ...
		    //fprintf(stdout, "GM_StartLinkedStreams, GM_SetSyncAudioStreamReference returned %d\n", err);
	    ...
	    if (err == NO_ERR)
		{
			...
			{
				//fprintf(stdout, "GM_StartLinkedStreams, XWaitMicroseocnds\n");
			    XWaitMicroseocnds(HAE_GetSliceTimeInMicroseconds());
			}
		    pNext = (GM_LinkedStream *)pTop;
		    while (pNext)
			{
				//fprintf(stdout, "GM_StartLinkedStreams, calling GM_SyncAudioStreamStart with %d\n", ((GM_LinkedStream *)pNext->playbackReference));
			    err = GM_SyncAudioStreamStart(((GM_LinkedStream *)pNext)->playbackReference);
				//fprintf(stdout, "GM_StartLinkedStreams, GM_SyncAudioStreamStart returned %d\n", err);
			    pNext = pNext->pNext;
			}
		}
	}
	...
	{
	    err = NOT_SETUP;
	    //fprintf(stdout, "GM_StartLinkedStreams, NOT_SETUP!! err = %d\n", err);
	}
    //fprintf(stdout, "GM_StartLinkedStreams, returning %d\n\n", err);
    return err;
}
