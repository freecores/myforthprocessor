...
GeneralDisposeFunc PathDasher_DisposeOps;
void PathDasher_Dispose(JNIEnv *env, jlong ops);
  ...
  (JNIEnv *env, jobject obj, jobject out)
{
    ...
    Disposer_AddRecord(env, obj, PathDasher_DisposeOps, ptr_to_jlong(cdata));
 ...
 * All the native memory associated with dasher is  cleaned here.
 */
void PathDasher_DisposeOps(JNIEnv *env, jlong ops)
{
    doeObject   cobj;
    PathDasher cdata   = (PathDasher)jlong_to_ptr(ops);
    doeE        cenv;
