/*
 * @(#)mlib_ImageConv_32nw.c	1.7 03/01/23
 *
...
#ifdef __SUNPRO_C
#pragma ident	"@(#)mlib_ImageConv_32nw.c	1.5	00/04/17 SMI"
#endif /* __SUNPRO_C */
...
#define CONV_FUNC(KERN) mlib_conv##KERN##_32nw
...
#pragma pipeloop(0)
      ...
      for (i = 0; i <= (wid - 3); i += 3) {
        ...
        dp[0    ] = CLAMP_S32(d0);
        dp[chan1] = CLAMP_S32(d1);
        dp[chan2] = CLAMP_S32(d2);
...
#pragma pipeloop(0)
          ...
          for (j = 0; j <= (hsize - 2); j += 2) {
            ...
            dp[0  ] = CLAMP_S32(p0*k0 + p1*k1 + p2*k2 + p3*k3 + pbuff[j]);
            dp[dll] = CLAMP_S32(p1*k0 + p2*k1 + p3*k2 + p4*k3 + pbuff[j + 1]);
...
#pragma pipeloop(0)
          ...
          for (j = 0; j <= (hsize - 2); j += 2) {
            ...
            dp[0  ] = CLAMP_S32(p0*k0 + p1*k1 + p2*k2 + pbuff[j]);
            dp[dll] = CLAMP_S32(p1*k0 + p2*k1 + p3*k2 + pbuff[j + 1]);
...
#pragma pipeloop(0)
          ...
          for (j = 0; j <= (hsize - 2); j += 2) {
            ...
            dp[0  ] = CLAMP_S32(p0*k0 + p1*k1 + pbuff[j]);
            dp[dll] = CLAMP_S32(p1*k0 + p2*k1 + pbuff[j + 1]);
