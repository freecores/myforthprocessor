...
#if defined (KPLINUX)
                semun_t dummySemun;
                semctl (semId, 0, IPC_RMID, dummySemun);  
#else
		semctl (semId, 0, IPC_RMID);	
#endif
/*  remove the semaphore since it
											was not initialized
											properly						*/
	}
	return NULL;	
...
semun_t               dummySemun;
...
#if defined(KPLINUX)
        usageCnt = semctl (semSetDataPtr->SemId, semNum, GETVAL, dummySemun);
#else
	...
	if (0 == usageCnt) {
#if defined(KPLINUX)
                sysRet = semctl (semSetDataPtr->SemId, 0, IPC_RMID, dummySemun);
#else
...
char			numStr[10];
	...
	size = (KpUInt32_t)(strlen (baseName) + strlen (numStr) + 1);
	if (size > bufSize) {
		return KCMS_FAIL;
	}
