									...
									KpChar_p entry, KpChar_p theString)
{
	...
	/* write the data out to the registry */
	ret = KpWriteRegistry (hkey, entry, theString, (KpUInt32_t)strlen(theString)+1, REG_SZ);
									...
									KpChar_p stringName, KpChar_p stringBuffer)
{
	...
	/* write the data out to the registry */
	ret = KpWriteRegistry (hkey, stringName, stringBuffer, (KpUInt32_t)strlen(stringBuffer)+1, REG_SZ);
	...
	if (!KpUseRegistryEx()) {
		switch (szType)
		{
			case REG_DWORD:
				...
				dataSize = (KpUInt32_t)strlen(dataBuf);
				break;
...
static void KpconvertSpace (KpChar_p inBuffer, KpChar_p outBuffer)
{
	...
	len = (KpUInt32_t)strlen (inBuffer);
