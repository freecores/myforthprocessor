...
void Add_GlyphClass( GlyphClass **tPtr, GlyphClass *addMe, tt_uint16 flags, tt_int32 arg1, tt_int32 arg2, tt_int32 flip)
{
	register int i, j;
	GlyphClass *t;
	...
	/* The following flip concerns component glyphs which have one component,
		and which flip the one component. */
	if (flip !=0 ) 
	{ 
		pointCount= addMe->pointCount;
		n=pointCount+2;
		if (flip==1)
		{
		  for ( i = 0; i < pointCount; i++ ) 
