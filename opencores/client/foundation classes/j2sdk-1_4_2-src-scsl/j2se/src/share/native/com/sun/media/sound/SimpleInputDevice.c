/*
 * @(#)SimpleInputDevice.c	1.33 03/05/01
 *
...
#define MAX_FORMATS 96
    ...
    jobject newFormatObject;
	
    int maxFormats;
	
    // this sets maxFormats to the maximum number of formats that may be supported
    //int maxFormats = HAE_MaxCaptureFormats(index);


    UINT32 encodings[MAX_FORMATS];
    UINT32 sampleRates[MAX_FORMATS];
    UINT32 channels[MAX_FORMATS];
    UINT32 bits[MAX_FORMATS];
    ...
    //maxFormats = HAE_GetSupportedCaptureFormats(index, encodings, sampleRates, channels, bits);
    maxFormats = HAE_GetSupportedCaptureFormats(index, encodings, sampleRates, channels, bits, MAX_FORMATS);
