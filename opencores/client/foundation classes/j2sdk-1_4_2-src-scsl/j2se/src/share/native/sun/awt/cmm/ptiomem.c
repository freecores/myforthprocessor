 ...
 *	@(#)ptiomem.c	2.60 99/03/05
	...
	The external database is responsible for loading and storing a block of
	memory which contains all of the PT information.  It needs to know only
	the location and size of this block.
...
PTErr_t errnum;
	...
	if ((errnum == KCP_PT_ACTIVE) || (errnum == KCP_SERIAL_PT)) {
		errnum = makeInActive (PTRefNum);	/* deactivate the PT */
	}
