	...
	F16Dot16 params[] )
{
	...
	tt_int32 leftSideIntegerMoveTemp,rightSideIntegerMoveTemp,leftSideIntegerMoveDot6Temp,rightSideIntegerMoveDot6Temp;
  	...
  	if ( (n > 0) && (boldIntegerAdvance != 0) ) {
		...
		for ( ctr = 0; ctr < contourCount; ctr++ ) {
			...
			if (CounterClockWiseWinding)
			{
				...
				leftSideIntegerMoveTemp=leftSideReverseInteger;
				rightSideIntegerMoveTemp=rightSideReverseInteger;
				leftSideIntegerMoveDot6Temp=leftSideIntegerMoveTemp<<6;
				rightSideIntegerMoveDot6Temp=rightSideIntegerMoveTemp<<6;
 			}
			else
			{
				/* It should be an exterior curve, so use full strength */
				leftSideIntegerMoveDot6Temp=leftSideIntegerMoveDot6;
				rightSideIntegerMoveDot6Temp=rightSideIntegerMoveDot6;
