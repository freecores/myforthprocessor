 ...
 *	June 30, 1995
 *------------------------------------------------------------------*/
SpStatus_t KSPAPI SpGetInfo (SpDBInfo *info, KpInt32_t size)
{
	...
	if ((PTStat = PTGetAttribute((PTRefNum_t)1,(int32)KCM_KCP_VERSION,&len,tempBuf)) != KCP_SUCCESS)
	{
		return SpStatusFromPTErr(PTStat);
	}
