/*
 * @(#)mlib_c_ImageConvKernelConvert.c	1.11 03/01/23
 *
...
#ifdef __SUNPRO_C
#pragma ident	"@(#)mlib_c_ImageConvKernelConvert.c	1.15	00/03/09 SMI"
#endif /* __SUNPRO_C */
...
mlib_status mlib_c_ImageConvKernelConvert(mlib_s32 * ikernel,
                                          mlib_s32 * iscale,
                                          mlib_d64 * fkernel,
                                          mlib_s32 m,
                                          mlib_s32 n,
...
#endif
{
  ...
  mlib_s32 *tmp_ikernel;
  mlib_s32 iskernel[256], *idkernel=NULL;
  mlib_s32 ismalloc;
#endif  
  ...
  if ((type == MLIB_BYTE) || (type == MLIB_SHORT)) {
    ...
    norm *= 65536.0;
    for (i = 0; i < m * n; i++) {
      ikernel[i] = CLAMP_S32(fkernel[i] * norm);
    }

    *iscale = scale + 16;
#else
    if (type == MLIB_BYTE) norm *= 256.0;
    ...
    if (type == MLIB_BYTE) {
      if (test == 1) {
        for (i = 0; i < m * n; i++)
          ikernel[i] = (mlib_s32) (fkernel[i] * norm) << 8;
      }
      else {
        for (i = 0; i < m * n; i++)
          ikernel[i] = tmp_ikernel[i] << 8;
      }
    } else {
      if (test == 1) {
        for (i = 0; i < m * n; i++)
    ...
    *iscale = 29 - scale;
    ...
    for (i = 0; i < m * n; i++) {
      if (fkernel[i] > 0)
        ikernel[i] = CLAMP_S32(fkernel[i] * norm + 0.5);
      else
        ikernel[i] = CLAMP_S32(fkernel[i] * norm - 0.5);
    }
