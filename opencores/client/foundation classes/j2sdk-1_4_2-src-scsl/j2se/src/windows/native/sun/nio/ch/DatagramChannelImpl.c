/*
 * @(#)DatagramChannelImpl.c	1.16 03/01/23
 *
                                            ...
                                            jint len, jboolean connected)
{
    ...
    if (!isSenderCached(env, this, &psa)) {
        ...
        if (ia != NULL) {
            ...
            (*env)->SetIntField(env, ia, ia_famID, psa.sin_family);	    // AF_INET
                                            ...
                                            jint len, jobject dest)
{
    ...
    memset((char *)&psa, 0, sizeof(psa));
    psa.sin_port = htons((short) destPort);
    psa.sin_addr.s_addr = (unsigned long) htonl(
        (*env)->GetIntField(env, destAddress, ia_addrID));
