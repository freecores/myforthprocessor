/*
 * @(#)PLATFORM_API_WinOS_Ports.c	1.4 03/01/23
 *
...
void printControl(MIXERCONTROL* control) {
    TRACE3("    %s: dwControlType=%s/%s, ", control->szShortName, getControlClass(control->dwControlType), getControlType(control->dwControlType));
...
INT32 PORT_GetPortName(void* id, INT32 portIndex, char* name, INT32 len) {
	...
	line = info->ports[portIndex];
	if (line) {
	    strncpy(name, line->szShortName, len-1);
	    name[len-1] = 0;
	    return TRUE;
	}
    }
...
void createPortControl(PortInfo* info, PortControlCreator* creator, MIXERCONTROL* mixerControl,
    ...
    char* typeName = mixerControl->szShortName;
    float min;
...
void PORT_GetControls(void* id, INT32 portIndex, PortControlCreator* creator) {
	...
	if (line) {
		...
		addCompoundControl(info, creator, line->szShortName, controls, &controlCount);
	    } else {
		...
		for (i = 0; i < line->cConnections; i++) {
		    ...
		    addCompoundControl(info, creator, srcLine->szShortName, controls, &controlCount);
		}
	    }
	}
    }
...
void PORT_SetIntValue(void* controlIDV, INT32 value) {
	...
	break;
    default:
	ERROR1("PORT_GetFloatValue: wrong controlType=%d !\n", controlID->controlType);
	return;
    }
