/*
 * @(#)awt_Win32GraphicsDevice.cpp	1.34 03/01/23
 *
...
AwtWin32GraphicsDevice::AwtWin32GraphicsDevice(int screen, MTSafeArray *arr)
{
    ...
    pMonitorInfo = new MONITOR_INFO;
    pMonitorInfo->dwSize = sizeof(MONITOR_INFO);
...
/*
 * Class:     sun_awt_Win32GraphicsDevice
 * Method:    isDirectDrawUsed
 * Signature: ()Z
 */
JNIEXPORT jboolean JNICALL
Java_sun_awt_Win32GraphicsDevice_isDirectDrawUsed(
    JNIEnv* env, jclass graphicsDevice) {
    return DDInit();
}
...
Java_sun_awt_Win32GraphicsDevice_enterFullScreenExclusive(
    ...
    // REMIND: It would be nice to do SendMessage instead so that
    // we could catch errors and make the timing of this operation
    // more predictable.  Leaving it as PostMessage for now due
    // to deadlock concerns.  The whole architecture of directx
    // threading needs an overhaul anyway.
    ::PostMessage(hWnd, WM_AWT_DD_ENTER_FULLSCREEN, (WPARAM)monitor, NULL);
...
Java_sun_awt_Win32GraphicsDevice_exitFullScreenExclusive(
    ...
    // to deadlock concerns.  The whole architecture of directx
    // threading needs an overhaul anyway.
    ::PostMessage(hWnd, WM_AWT_DD_EXIT_FULLSCREEN, (WPARAM)monitor, NULL);
...
(JNIEnv* env, jobject graphicsDevice, jint screen, jboolean useDD) {
    ...
    if (useDD) {
        ...
        DEVMODE dm;
        char* pName = NULL; // REMIND : Extend for multi-monitor support
	 ...
	 jboolean useDD)
{
	...
	// processing code since we may return from the PostMessage
	// before that object has been used.
	::PostMessage(hWnd, WM_AWT_DD_SET_DISPLAY_MODE, (WPARAM)monitor,
		      (LPARAM)dm);
    } else {
        DEVMODE dm;
...
JNIEXPORT void JNICALL Java_sun_awt_Win32GraphicsDevice_enumDisplayModes
    (JNIEnv* env, jobject graphicsDevice, jint screen, jobject arrayList, jboolean useDD) {
