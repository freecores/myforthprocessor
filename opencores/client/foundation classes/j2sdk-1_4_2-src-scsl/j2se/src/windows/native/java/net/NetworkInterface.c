/*
 * @(#)NetworkInterface.c	1.18 03/01/23 
 *
...
static jclass ni_iacls;		    /* InetAddress */
static jfieldID ni_iaAddr;	    /* InetAddress.address */

static jclass ni_ia4cls;	    /* Inet4Address */
static jmethodID ni_ia4Ctor;	    /* Inet4Address() */
...
jobject createNetworkInterface(JNIEnv *env, netif *ifs, int netaddrCount, netaddr *netaddrP)
{
    ...
    addr_index = 0;
    while (addrs != NULL) {
	jobject iaObj = (*env)->NewObject(env, ni_ia4cls, ni_ia4Ctor);
	if (iaObj == NULL) {
	    free_netaddr(netaddrP);
	    return NULL;
	}
	...
	(*env)->SetIntField(env, iaObj, ni_iaAddr, addrs->addr);
 ...
 */
JNIEXPORT jobject JNICALL Java_NetworkInterface_getByIndex
