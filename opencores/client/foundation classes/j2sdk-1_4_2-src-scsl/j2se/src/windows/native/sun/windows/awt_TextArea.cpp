/*
 * @(#)awt_TextArea.cpp	1.59 03/02/26
 *
...
const char* AwtTextArea::GetClassName() {
    load_rich_edit_library();
    return (IS_WIN95 && !IS_WIN98) ? "RICHEDIT" : RICHEDIT_CLASSA;
}

LPCWSTR AwtTextArea::GetClassNameW() {
    load_rich_edit_library();
    return (IS_WIN95 && !IS_WIN98) ? L"RICHEDIT" : RICHEDIT_CLASSW;
}
...
size_t AwtTextArea::GetALength(JNIEnv* env, jstring jStr, size_t maxlen) {
    size_t alength = 0;
    ...
    WCHAR* wbuffer = new WCHAR[maxlen + 1];
    env->GetStringRegion(jStr, 0, static_cast<jsize>(maxlen), wbuffer);
    wbuffer[maxlen] = L'\0';

    /*
     * Fix for BugTraq Id 4260109.
     * Don't use W2A since it allocates memory on the stack
     * causing stack overflow when the text is very long.
     */
    size_t len = (wcslen(wbuffer)+1)*2;
    char* abuffer = NULL;
    try {
        abuffer = new char[len];
    } catch (std::bad_alloc&) {
        delete[] wbuffer;
        throw;
    }
    alength = strlen(W2AHelper(abuffer, wbuffer, static_cast<int>(len)));
...
AwtTextArea::EditProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    if (mr == mrDoDefault) {
        ...
        retValue = (*lpFuncCallWindowProc)(sm_pDefWindowProc, 
...
AwtTextArea::WmContextMenu(HWND hCtrl, UINT xPos, UINT yPos) {
    ...
    if (m_hEditCtrl == NULL) {
        ...
        DWORD dwExStyle = 0;
	if (IS_NT) {
            m_hEditCtrl = ::CreateWindowExW(dwExStyle,
                                       L"EDIT",
                                       L"TEXT",
                                       dwStyle,
                                       0, 0, 0, 0,
            ...
            m_hEditCtrl = ::CreateWindowExA(dwExStyle,
                                       "EDIT",
                                       "TEXT",
                                       dwStyle,
                                       0, 0, 0, 0,
        ...
        ::SetLastError(0);
        INT_PTR ret = (*lpFuncSetWindowLongPtr)(m_hEditCtrl, GWLP_WNDPROC, 
     ...
     */
    ::SetWindowText(m_hEditCtrl, "TEXT");
...
AwtTextArea::HandleEvent(MSG *msg, BOOL synthetic)
{
    ...
    if (msg->message == WM_LBUTTONDOWN || msg->message == WM_LBUTTONDBLCLK) {
        ...
        LONG lCurPos = static_cast<LONG>(SendMessage(EM_CHARFROMPOS, 0,
            reinterpret_cast<LPARAM>(&msg->pt)));
	 ...
	 * the mouse pointer is not over the selected region.
        ...
        if (lCurPos < cr.cpMin || cr.cpMax <= lCurPos) {
            if (msg->message == WM_LBUTTONDBLCLK) {
                SetStartSelectionPos(static_cast<LONG>(SendMessage(
                    EM_FINDWORDBREAK, WB_LEFT, lCurPos)));
                SetEndSelectionPos(static_cast<LONG>(SendMessage(
                    EM_FINDWORDBREAK, WB_RIGHT, lCurPos)));
            } else {
        ...
        if (GetStartSelectionPos() != -1 && 
            ...
            cr.cpMin = min(GetStartSelectionPos(), GetLastSelectionPos());
            cr.cpMax = max(GetEndSelectionPos(), GetLastSelectionPos());
            /*
             * Hide the selection in a rich edit control to prevent it
             * from scrolling the end of selection into view.
             */
...
LONG AwtTextArea::getJavaSelPos(LONG orgPos)
{
    ...
    if ((mblen = GetTextLength()) == 0)
        return 0;
    mbbuf = new char[mblen + 1];
    GetText(mbbuf, mblen + 1);
    if (m_isLFonly == TRUE) {
        mblen = RemoveCR(mbbuf);
    }

    while (cur < pos && cur < mblen) {
					       ...
					       jint start, jint end)
{
	 ...
	 * the right positions.
	 */
        start = static_cast<jint>(
            AwtTextArea::GetALength(env, targetText, start) + cr_start);
        end = static_cast<jint>(
            AwtTextArea::GetALength(env, targetText, end) + cr_end);

        /* Copy Java string into a buffer. */
	int length = env->GetStringLength(text);
	char *buffer = new char[2*length + 1];
        AwtFont::javaString2multibyte(env, text, buffer, length);
        c->CheckLineSeparator(buffer);
        c->RemoveCR(buffer);
