/*
 * @(#)awt_MenuItem.cpp	1.62 03/01/23
 *
...
AwtMenuItem::~AwtMenuItem() {
    if (m_freeId && strcmp(GetClassName(), "SunAwtMenuItem") == 0) {
...
LPCWSTR AwtMenuItem::GetClassNameW() {
  return L"SunAwtMenuItem";
}
...
void AwtMenuItem::SetLabel(const char *sb)
{
    AwtMenu* menu = GetMenuContainer();
    /* Fix for bug 4257944 by ssi@sparc.spb.su
    * check parent
...
LRESULT AwtMenuItem::WinThreadExecProc(ExecuteArgs * args)
{
	...
	{
	    const char * sb = (const char *)args->param1;
					      ...
					      jstring label)
{
    ...
    if (JNU_IsNull (env, label)) {
        label = JNU_NewStringPlatform(env, "");
    }
    const char *labelPtr = JNU_GetStringPlatformChars(env, label, 0);
