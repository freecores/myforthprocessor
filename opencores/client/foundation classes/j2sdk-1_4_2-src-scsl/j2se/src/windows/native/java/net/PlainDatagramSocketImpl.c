/*
 * @(#)PlainDatagramSocketImpl.c	1.80 03/01/23
 *
...
Java_java_net_PlainDatagramSocketImpl_peekData(JNIEnv *env, jobject this,
	...
	/* receive the packet */
	n = recvfrom(fd, fullPacket, packetBufferLen, MSG_PEEK,
			 (struct sockaddr *)&remote_addr, &remote_addrsize);
	...
	if (packetAddress != NULL) {
	    int currAddress;
	    int currFamily;
	    currAddress = (*env)->GetIntField(env, packetAddress, ia_addressID);
	...
	if (packetAddress == NULL) {
	    packetAddress = (*env)->NewObject(env, ia4_clazz, ia4_ctor);
	    CHECK_NULL_RETURN(packetAddress, -1); 
	    (*env)->SetIntField(env, packetAddress, ia_addressID, newAddress);
...
Java_java_net_PlainDatagramSocketImpl_receive(JNIEnv *env, jobject this,
	...
	/* receive the packet */
	n = recvfrom(fd, fullPacket, packetBufferLen, 0,
			 (struct sockaddr *)&remote_addr, &remote_addrsize);
...
Java_java_net_PlainDatagramSocketImpl_leave(JNIEnv *env, jobject this,
	...
	addressAddress = (*env)->GetIntField(env, iaObj, ia_addressID);
    }
    /*
     * Set the multicast group address in the ip_mreq field
     * eventually this check should be done by the security manager
