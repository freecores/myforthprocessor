...
extern jboolean IsMultiFont(JNIEnv *env, jobject obj) 
{
    ...
    jobject props = env->GetObjectField(peer, AwtFont::propsID);
    jboolean result = props != NULL;
    env->DeleteLocalRef(peer);
    env->DeleteLocalRef(props);
    return result;
}

extern jstring GetFontProperty(JNIEnv *env, jobject font, jstring key) 
{
    ...
    jobject props = env->GetObjectField(peer, AwtFont::propsID);
    DASSERT(props != NULL);
    jstring result = (jstring)JNU_CallMethodByName(
	env, NULL, props, "getProperty", 
	"(Ljava/lang/String;)Ljava/lang/String;", key).l;
    env->DeleteLocalRef(peer);
    env->DeleteLocalRef(props);
    return result;
}
...
AwtFont* AwtFont::Create(JNIEnv *env, jobject font, jint angle, jfloat awScale)
{
	...
	if (cfnum > 0) {
	    // Ask java.awt.Font for the input charset property.
	    jstring key = env->NewStringUTF("inputtextcharset");
	    jstring jtextin = GetFontProperty(env, font, key);
	    WCHAR* textin = TO_WSTRING(jtextin);

	    env->DeleteLocalRef(key);
	    env->DeleteLocalRef(jtextin);
...
{
    if(strstr((char *)lParam, lpelfe->elfLogFont.lfFaceName)) {
        strcpy((char *)lParam, lpelfe->elfLogFont.lfFaceName);
        return 0;
    } else {
        return 1;
    }
			     ...
			     int angle=0, float awScale=1.0f)
{
	...
	LPCWSTR lpszTmp = logFont.lfFaceName;
        size_t tmplen = wcslen(lpszTmp);

        size_t mblen = WideCharToMultiByte(CP_ACP, 0,
	    lpszTmp, static_cast<int>(tmplen), NULL, 0, NULL, NULL);
        char* chartmp = new char[mblen+1];
        chartmp[mblen] = '\0';
        WideCharToMultiByte(CP_ACP, 0,
	        lpszTmp, static_cast<int>(tmplen), chartmp,
            static_cast<int>(mblen), NULL, NULL);
        strcpy(&logFontA.lfFaceName[0], chartmp);
	HDC hdc = ::GetDC(NULL);
        ::EnumFontFamiliesEx(hdc, &logFontA, (FONTENUMPROC)FindFamilyName, 
                             (LPARAM)chartmp, 0L); 
	::ReleaseDC(NULL, hdc);
        strcpy(&logFontA.lfFaceName[0], chartmp);
        delete [] chartmp;
	...
	if (awScale != 1.0f) {
	    ...
	    avgWidth = tm.tmAveCharWidth;
	    logFontA.lfWidth = (LONG)((fabs)(avgWidth*awScale));
	    hFont = CreateFontIndirectA(&logFontA);
	    DASSERT(hFont != NULL);
	    VERIFY(::ReleaseDC(0, hDC));
	}
...
char* AwtFont::javaString2multibyte(JNIEnv *env, jstring jstr, char buffer[], int length)
{
    if (length == 0) {
        buffer[0] = '\0';
    } else {
        const unsigned short *ustr = NULL;
	int cchMultiByte = 0;
	try {
	    ustr = env->GetStringCritical(jstr, 0);
	    if (ustr == 0)
	        return 0;
	    cchMultiByte = WideCharToMultiByte(CP_ACP, 0, ustr, length, buffer,
					       length * 2 + 1, NULL, NULL);
	} catch (...) {
	    env->ReleaseStringCritical(jstr, ustr);
	    throw;
	}
	env->ReleaseStringCritical(jstr, ustr);
        DASSERT(cchMultiByte);
        buffer[cchMultiByte] = '\0';
    }

    return buffer;
}

jstring AwtFont::multibyte2javaString(JNIEnv *env, char * str)
{
    size_t str_len = strlen(str);
    if (str_len == 0) {
	return (jstring)env->NewStringUTF("");
    }
    jchar buffer[256];
    size_t jstr_len = MultiByteToWideChar(CP_ACP, 0, str,
        static_cast<int>(str_len), buffer, 0);
    jchar *jstr;
    size_t buf_size;
    if (jstr_len < 255) {
	DASSERT(jstr_len > 0);
	buf_size = sizeof(buffer);
        ...
        buf_size = (jstr_len + 1) * sizeof(jchar);
        jstr = (jchar *)safe_Malloc(buf_size);
    }
    int n = MultiByteToWideChar(CP_ACP, 0, str,
        static_cast<int>(str_len), jstr, static_cast<int>(buf_size));
    DASSERT(n == static_cast<int>(jstr_len));
    jstring result = env->NewString(jstr, static_cast<jsize>(jstr_len));
    if (jstr != buffer)
        free(jstr);
    return result;
}
...
Java_java_awt_Font_pDispose(JNIEnv *env, jobject self) 
{
	...
	AwtToolkit::GetInstance().SendMessage(WM_AWT_DISPOSE, (WPARAM)awtFont,
					      0);
    }
...
Java_sun_awt_PlatformFont_initIDs(JNIEnv *env, jclass cls) 
{
    ...
    AwtFont::propsID = env->GetFieldID(cls, "props", "Lsun/awt/FontProperties;");
