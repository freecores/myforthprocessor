/*
 * @(#)awt_TextComponent.cpp	1.47 03/02/26
 *
...
const char* AwtTextComponent::GetClassName() {
    return "EDIT";  /* System provided edit control class */
}

LPCWSTR AwtTextComponent::GetClassNameW() {
    return L"EDIT";  /* System provided edit control class */
}
...
int AwtTextComponent::RemoveCR(char *pStr)
{
    ...
    if (pStr) {
        ...
        if (strchr(pStr, '\r') == NULL) {
            return static_cast<int>(strlen(pStr));
        }
...
void AwtTextComponent::CheckLineSeparator(char *pStr)
{
    if (pStr == NULL) {
        return;
    }
    ...
    // check to see if there are any LF's
    if (m_EOLchecked == TRUE || strchr(pStr, '\n') == NULL) {
        return;
    }
...
Java_sun_awt_windows_WTextComponentPeer_getText(JNIEnv *env, jobject self)
{
    ...
    BOOL isNT = IS_NT;
    int len;
    /* before wrapper function of GetWindowTextLengthW is made, 
     * we have to distinguish the running environment
     */
    if (isNT) {
	len = ::GetWindowTextLengthW(c->GetHWnd());
    } else {
        len = ::GetWindowTextLengthA(c->GetHWnd());
    }
	...
	/*
	 * before wrapper function of GetWindowText is made so that 2
	 * types of AwtTextComponent::GetText are unified, we have to
...
#endif
	    /*
	     * Text in a TextComponent is written in platform-specific
	     * codeset because it may be output from IME, which
	     * doesn't emit unicode.  
	     */
            char* buffer = new char[len + 1];	/* Including null terminater */
            c->GetText(buffer, len + 1);
            c->RemoveCR(buffer);
	    js = AwtFont::multibyte2javaString(env, buffer);/* convert to unicode. */
            delete[] buffer;
        }
    }
    return js;
						...
						jstring text)
{
    ...
    AwtTextComponent* c = (AwtTextComponent*)pData;
    /*
     * before wrapper function of SetWindowText is made, two versions
     * of SetText will be unified.  Before that we have to distinguish
