...
#pragma ident "@(#)ciTypeFlow.cpp	1.25 03/02/28 09:18:04 JVM"
#endif
/*
...
void ciTypeFlow::JsrSet::apply_control(ciTypeFlow* analyzer,
  ...
  if (code == Bytecodes::_jsr) {
    ...
    if (size() == 0) {
      ...
      // Present this fact to the next phase as an immediate uncommon trap
      // at the entry point of the OSR method.  This must be special-cased
      // as a failure to compile, not simply an uncommon trap, since the
      // compiled code cannot represent the interpreter's state.
...
ciType* ciTypeFlow::StateVector::type_meet(ciType* t1, ciType* t2) {
  ...
  if (t1->equals(top_type())) {
    ...
    ciKlass* object_klass = ciEnv::current()->Object_klass();
	...
	ciKlass* elem  = type_meet(elem1, elem2)->as_klass();
...
const ciTypeFlow::StateVector* ciTypeFlow::get_start_state() {
  ...
  if (is_osr_flow()) {
    ...
    assert(non_osr_block != NULL, "bad OSR point"); // or bottom out the state
...
void ciTypeFlow::StateVector::do_aaload(ciByteCodeStream* str) {
  ...
  if (!array_klass->is_loaded()) {
    ...
    trap(str, array_klass, Deoptimization::Deopt_unloaded_array_class);
    return;
  }
  ...
  if (!element_klass->is_loaded() && element_klass->is_instance_klass()) {
    ...
    trap(str, element_klass, Deoptimization::Deopt_unloaded_array_class);
  } else {
...
bool ciTypeFlow::StateVector::do_ldc(ciByteCodeStream* str) {
...
void ciTypeFlow::StateVector::trap(ciByteCodeStream* str, ciKlass* klass, int index) {
  ...
  if (log != NULL) {
    ...
    const char* deopt_kind = Deoptimization::deopt_info_name(index);
    if (deopt_kind != NULL) {
      log->print(" kind='%s'", deopt_kind);
    } else {
      log->print(" kind='unloaded' index='%d'", index);
    }
    if (kid >= 0)
...
bool ciTypeFlow::StateVector::apply_one_bytecode(ciByteCodeStream* str, bool* bailed_out) {
  _trap_bci = -1;
  _trap_index = 0;
  ...
  case Bytecodes::_fast_iaccess_0: ShouldNotReachHere(); break;
...
void ciTypeFlow::flow_block(ciTypeFlow::Block* block,
  ...
  while ((code = str.next()) != ciByteCodeStream::EOBC &&
    ...
    bool res = state->apply_one_bytecode(&str, &bailed_out);
    // Bailing out takes precedence
    if (bailed_out) {
      int start_bci = this->start_bci();
      Block* start_block = existing_block_at(start_bci, new JsrSet(NULL, 16));
      guarantee(start_block != NULL && start_block->is_start(), "start block must exist");
      start_block->set_trap(start_bci, Deoptimization::Deopt_unhandled_bytecode);
    }
    if (res) {
  ...
  if (control != ciTypeFlow::Range::fall_through_bci) {
    ...
    state->apply_one_bytecode(&str, NULL);
  } else {
...
#ifdef COMPILER2
    if (_next_pre_order >= NodeCountCutoff) {
      // Too many basic blocks.  Bail out.
      //
      // NodeCountCutoff is the number of nodes at which the parser
      // will bail out.  Probably if we already have that many BBs,
      // the parser will generate at least that many nodes and bail out.
      ...
      // as a failure to compile, not simply an uncommon trap, since the
      // compiled code cannot represent the interpreter's state.
      if (PrintOptoBailouts || (PrintOpto && (Verbose || WizardMode))) {
	tty->print_cr("Bailing out: Too many basic blocks (%d) in ", _next_pre_order);
	method()->print();
	tty->cr();
      }
      int start_bci = this->start_bci();
      Block* block = existing_block_at(start_bci, new JsrSet(NULL, 16));
      guarantee(block != NULL && block->is_start(), "start block must exist");
      block->set_trap(start_bci, Deoptimization::Deopt_unhandled_bytecode);
      return;
    }
#endif // COMPILER2
