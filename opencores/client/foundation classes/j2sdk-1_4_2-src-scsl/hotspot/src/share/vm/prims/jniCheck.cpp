...
#pragma ident "@(#)jniCheck.cpp	1.22 03/05/15 13:47:13 JVM"
#endif
/*
 ...
 */
static klassOop ValidateClass (JavaThread*, jclass,
                               bool allow_primitive = false);
static oop      ValidateObject(JavaThread* thr, jobject);
...
{
  // Nothing to do - checks moved to JNI_ENTRY_CHECKED macro above.
}
...
checkStaticFieldID(JavaThread* thr, jfieldID fid, jclass cls, int ftype)
{
  ...
  ASSERT_OOPS_ALLOWED;
  klassOop k_oop = ValidateClass(thr, cls, false);
  ...
  if (!instanceKlass::cast(f_oop)->jni_find_local_field_from_offset(
          id->offset(), true, &fd))
...
checkInstanceFieldID(JavaThread* thr, jfieldID fid, jobject obj, int ftype)
{
  ...
  ASSERT_OOPS_ALLOWED;
  oop oopObj = ValidateObject(thr, obj);
  if (!oopObj)
  ...
  /* check for proper field type */
  if (!instanceKlass::cast(k_oop)->jni_find_field_from_offset(offset,
							      false, &fd))
...
checkArray(JavaThread* thr, jarray jArray, int elementType)
{
  ...
  aOop = (arrayOopDesc *)ValidateObject(thr, jArray);
...
static inline oop
ValidateHandle(JavaThread* thr, jobject obj)
{
...
static inline oop
ValidateObject(JavaThread* thr, jobject obj)
{
    if (!obj)
	return NULL;
    ASSERT_OOPS_ALLOWED;
    oop oopObj = ValidateHandle(thr, obj);
    if (!oopObj) {
      ReportJNIFatalError(thr, fatal_bad_ref_to_jni);
    }
    return oopObj;
...
static inline klassOop
ValidateClass(JavaThread* thr, jclass clazz, bool allow_primitive)
{
  ASSERT_OOPS_ALLOWED;
  oop mirror = ValidateHandle(thr, clazz);
  if (!mirror) {
...
static inline void
ValidateCallObject(JavaThread* thr, jobject obj, jmethodID methodID)
{
  JNIid* mid = (JNIid *)methodID;
  ...
  /* check for proper subclass hierarchy */
  klassOop f_oop = mid->holder();
  if (!instanceKlass::cast(k_oop)->is_subtype_of(f_oop))
    ReportJNIFatalError(thr, fatal_wrong_class_or_method);
}

static inline void
ValidateCallClass(JavaThread* thr, jclass jclazz, jmethodID methodID)
{
  JNIid* mid = (JNIid *)methodID;
  /* validate the class being passed */
  ASSERT_OOPS_ALLOWED;
  klassOop k_oop = ValidateClass(thr, jclazz, false);
...
JNI_ENTRY_CHECKED(jboolean,
  checked_jni_IsSameObject(JNIEnv *env,
    ...
    IN_VM(
       ...
       */
      if (obj1 != NULL && ValidateHandle(thr, obj1) != NULL) {
        ValidateObject(thr, obj1);
      }
      if (obj2 != NULL && ValidateHandle(thr, obj2) != NULL) {
        ValidateObject(thr, obj2);
      }
    )
...
JNI_ENTRY_CHECKED(ResultType,  \
  checked_jni_Call##Result##Method(JNIEnv *env, \
    ...
    va_list args; \
    IN_VM( \
      ValidateCallObject(thr, obj, methodID); \
    ) \
    va_start(args,methodID); \
...
JNI_ENTRY_CHECKED(ResultType,  \
  checked_jni_CallNonvirtual##Result##Method(JNIEnv *env, \
    ...
    IN_VM( \
      ValidateCallObject(thr, obj, methodID); \
      ValidateCallClass(thr, clazz, methodID); \
    ) \
    va_start(args,methodID); \
