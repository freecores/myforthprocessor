...
#pragma ident "@(#)postaloc.cpp	1.65 03/02/28 16:49:38 JVM"
#endif
/*
...
bool PhaseChaitin::is_copy_of_callee( Node *def ) const {
  ...
  return false;                 // Some random computation, not callee-save
}
...
int PhaseChaitin::use_prior_register( Node *n, uint idx, Node *def, Block *current_block, Node_List &value, Node_List &regnd ) {
  ...
  // registers.  
  OptoReg::Name def_reg = lrgs(n2lidx(def)).reg();
  const RegMask &use_mask = n->in_RegMask(idx);
  bool can_use = ( RegMask::can_represent(def_reg) ? use_mask.Member(def_reg)
                                                   : use_mask.is_AllStack() );
  if (!can_use)
    return 0;
  ...
  // miss miss the opportunity.  :-(
  if( _matcher.is_save_on_entry(def_reg) &&
      is_copy_of_callee(def) ) {
    ...
    for( j = 1; j < phi_dex; j++ ) {
      ...
      for( k=1; k<phi->req(); k++ ) {
        ...
        if( phi != x && u != x ) // Found a different input
          u = u ? (Node*)-1 : x; // Capture unique input, or -1 for 2nd input
      }
      if( u != (Node*)-1 ) {    // Junk Phi.  Remove
    ...
    for( j = phi_dex; j < b->_nodes.size(); j++ ) {
      ...
      if( is_single_register(n_ideal_reg) ) {
        ...
        if( value[nreg] != val ) {
          ...
          value.map(nreg,val);
        } else if( !is_copy_of_callee(n) && regnd[nreg]->outcnt() != 0 ) {
          assert( n->is_Copy(), "" );
