/*
 * @(#)Frame.java	1.17 03/01/23 11:44:52
 *
...
public abstract class Frame implements Cloneable {
  ...
  public boolean isJavaFrame() {
    ...
    if (!VM.getVM().isCore()) {
      // FIXME: can not handle deoptimized frames yet
      //      if (isDeoptimizedFrame()) return true;
  ...
  public boolean isOSRAdapterFrame() {
    ...
    if (VM.getVM().isClientCompiler()) {
      return VM.getVM().getRuntime1().returnsToOSRAdapter(getPC());
    } else {
      CodeBlob cb = VM.getVM().getCodeCache().findBlob(getPC());
  ...
  public boolean isGlueFrame() {
    ...
    if (cb == null) {
      return false;
         ...
         if (cb.isI2CAdapter())  return true;
    else if (cb.isC2IAdapter())  return true;
    else if (cb.isRuntimeStub()) return true;
    else return false;
  }
  ...
  public Frame realSender(RegisterMap map) {
    // FIXME: has very different implementation in non-core case
    return sender(map);
  }
  ...
  public Address oopMapRegToLocation(VMReg reg, RegisterMap regMap) {
    VMReg stack0 = VM.getVM().getSharedInfo().getStack0();
    if (reg.lessThan(stack0)) {
      // If it is passed in a register, it got spilled in the stub frame.  
      return regMap.getLocation(reg);
    } else {
      long spOffset = VM.getVM().getAddressSize() * reg.minus(stack0) + getOopMapOffsetAdjustmentPD();
      return getSP().addOffsetTo(spOffset);
    }
  }
  ...
  public void oopsDo(AddressVisitor oopVisitor, RegisterMap map) {
    ...
    } else if (VM.getVM().isClientCompiler() && isOSRAdapterFrame()) {
