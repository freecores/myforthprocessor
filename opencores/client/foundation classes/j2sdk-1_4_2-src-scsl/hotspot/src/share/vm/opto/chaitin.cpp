...
#pragma ident "@(#)chaitin.cpp	1.79 03/02/28 16:49:36 JVM"
#endif
/*
...
void LRG::dump( ) const {
  ...
  if( _def == (Node*)-1 ) tty->print("MultiDef ");
...
void PhaseChaitin::Register_Allocate() {
  ...
  if( must_spill ) {  
    if( !_maxlrg ) return;
    // Bail out if unique gets too large (ie - unique > 64000)
    if( C->unique() + 10*must_spill > 65000 ) {
      C->set_root(NULL);
      return;
    }
    _maxlrg = Split( _maxlrg );        // Split spilling LRG everywhere
    // Bail out if unique gets too large (ie - unique > 63000)
    // or we failed to split
    if( !_maxlrg || C->unique() > 63000 ) {
      C->set_root(NULL);
      return;
    }
#ifdef ASSERT
  ...
  while( spills ) {
    if( _trip_cnt++ > 24 ) {
      assert( true, "failed spill-split-recycle sanity check" );
      prepare_to_bailout("failed spill-split-recycle sanity check");
      return;
    }
  ...
  // Convert CISC spills
  bool fixed_spills = fixup_spills();

  // Bail out if unique gets too large (ie - unique > 65000)
  if( !fixed_spills ) {
    prepare_to_bailout("Spilling requires too many nodes");
    return;
  }
...
//------------------------------record_bailout---------------------------------
void PhaseChaitin::prepare_to_bailout(const char *printed_error_msg) const {
  if( PrintOptoBailouts || PrintOpto || PrintCompilation ) {
    tty->print_cr("Bailout: %s", printed_error_msg);
  }
  C->set_root(NULL);
  return;
}
	...
	if( b->_inner_loop ) {
	  ...
	  if( _spilled_twice.test(n->_idx) ) 
        ...
        case Op_RegP:
          ...
          // IA32     2     1     1          1    1          6           6
          // IA64     1     1     1          1    1         52          42
          // SPARC    2     2     2          2    2         48          52
          // SPARCV9  2     2     2          2    2         48          52
          // -----------------------------------------------------
#ifdef SPARC
        ...
        case Op_RegD:
          ...
          // FOUR registers!
          if( rm.Size() == 2 && !rm.is_aligned_Pairs() ) {
            lrg._fat_proj = 1;
            lrg._is_bound = 1;
          }
          break;
...
uint PhaseChaitin::Select( ) {
  ...
  while( _simplified ) {
    ...
    // Live range is live and no colors available
      ...
      assert( lrg->alive(), "" );
      assert( !lrg->_fat_proj || lrg->_def == (Node*)-1 ||
...
bool PhaseChaitin::fixup_spills() {
