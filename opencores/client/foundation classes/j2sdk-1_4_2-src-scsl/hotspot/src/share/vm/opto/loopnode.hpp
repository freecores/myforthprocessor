...
#pragma ident "@(#)loopnode.hpp	1.111 03/01/23 12:17:13 JVM"
#endif
/*
...
public:
  ...
  int policy_maximally_unroll( PhaseIdealLoop *phase ) const;
...
class PhaseIdealLoop : public PhaseTransform {
  ...
  int is_postvisited( Node *n ) const { return _preorders[n->_idx]&1; }
...
private:
  ...
  int build_loop_tree( Node *n, int pre_order );
  ...
  void build_loop_early( Node *n, VectorSet &visited, Node_List &worklist, const PhaseIdealLoop *verify_me );
  void build_loop_late ( Node *n, VectorSet &visited, Node_List &worklist, const PhaseIdealLoop *verify_me );
...
public:
  ...
  // pass.
  void do_maximally_unroll( IdealLoopTree *loop, Node_List &old_new, int trip_count );
  ...
  void  split_if_with_blocks     ( Node *root, VectorSet &visited );
