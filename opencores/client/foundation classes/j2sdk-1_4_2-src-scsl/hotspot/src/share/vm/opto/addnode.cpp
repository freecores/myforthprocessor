...
#pragma ident "@(#)addnode.cpp	1.117 03/01/23 12:14:48 JVM"
#endif
/*
...
Node *AddFNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  if( !IdealizedNumerics || EnforceStrictFP || phase->C->method()->is_strict() ) {
...
Node *AddPNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  const AddNode *add = in(Offset)->is_Add();
  if( add && (const Node*)add->in(1) != add ) {
...
Node *MinINode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( y->Opcode() == Op_AddI && // Check for "x+c0" and collect constant
      y->in(2)->is_Con() ) {
