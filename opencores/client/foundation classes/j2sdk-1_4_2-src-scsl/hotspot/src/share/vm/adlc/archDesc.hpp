...
#pragma ident "@(#)archDesc.hpp	1.117 03/01/23 11:12:41 JVM"
#endif
/*
...
private:
  ...
  void gen_match(FILE *fp, MatchList &mlist, Dict &operands_chained_from);
  void chain_rule(FILE *fp, const char *indent, const char *ideal, 
		  const char *icost, const char *irule, 
                  Dict &operands_chained_from);
  void chain_rule_c(FILE *fp, char *indent, char *ideal, char *irule);
  void expand_opclass(FILE *fp, const char *indent, const char *cost, 
                      const char *result_type);
  void child_test(FILE *fp, MatchList &mList);
  void calc_cost(FILE *fp, const char *spaces, MatchList &mList);
...
public:
  ...
  void gen_dfa_state_body(FILE *fp, Dict &minmize, Dict &chained, int i);    // Driver for constructing the DFA state bodies
  ...
  const char *machOperEnum(const char *opName);// create dense index names
