...
#pragma ident "@(#)heap.cpp	1.38 03/01/23 12:08:54 JVM"
#endif
/*
...
bool CodeHeap::reserve(size_t reserved_size, size_t committed_size, size_t segment_size) {
  ...
  if (!_memory.initialize(align_to_page_size(reserved_size), align_to_page_size(committed_size))) {
    return false;
  }
...
void CodeHeap::verify() {
  ...
  assert(count <= 10000, "too many free blocks");  
