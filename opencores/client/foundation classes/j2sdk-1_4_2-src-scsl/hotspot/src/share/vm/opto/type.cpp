...
#pragma ident "@(#)type.cpp	1.223 03/01/23 12:19:22 JVM"
#endif
/*
...
void *Type::operator new( size_t x ) {
  Compile* compile = Compile::current();
  compile->set_type_last_size(x);
  void *temp = compile->type_arena()->Amalloc(x);
  compile->set_type_hwm(temp);
  return temp;
}
void Type::operator delete( void *ptr ) {
  Compile* compile = Compile::current();
  if( ptr == compile->type_hwm() )
...
const Type *Type::hashcons(void) {
  ...
  type_dict()->Insert(this,this);  // New Type, insert into Type table
...
void TypeLong::dump2( Dict &d, uint depth ) const {
  if( this == LONG )            // Common case?
    ...
    tty->print("long[%lld]",get_con());
  else {
    Type::dump2(d,depth);
    tty->print("long:%lld-%lld",_lo,_hi);
  }
}
#endif
