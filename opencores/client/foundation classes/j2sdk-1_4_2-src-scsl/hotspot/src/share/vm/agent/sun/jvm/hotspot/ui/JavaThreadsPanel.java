/*
 * @(#)JavaThreadsPanel.java	1.13 03/01/23 11:49:14
 *
 ...
 * threads.
 */
public class JavaThreadsPanel extends JPanel implements ActionListener {
	...
	fireShowThreadStackMemory(dataModel.getJavaThread(i));
    }

    private void fireShowThreadStackMemory(JavaThread t) {
	for (Iterator iter = listeners.iterator(); iter.hasNext(); ) {
	    Listener listener = (Listener) iter.next();
    ...
    private void fireShowJavaStackTrace() {
        ...
        fireShowJavaStackTrace(dataModel.getJavaThread(i));
    }

    private void fireShowJavaStackTrace(JavaThread t) {
	for (Iterator iter = listeners.iterator(); iter.hasNext(); ) {
	    Listener listener = (Listener) iter.next();
	...
	fireShowThreadInfo(dataModel.getJavaThread(i));
    }

    private void fireShowThreadInfo(JavaThread t) {
	for (Iterator iter = listeners.iterator(); iter.hasNext(); ) {
	    Listener listener = (Listener) iter.next();
