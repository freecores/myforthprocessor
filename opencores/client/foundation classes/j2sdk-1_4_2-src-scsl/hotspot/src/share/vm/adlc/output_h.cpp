...
#pragma ident "@(#)output_h.cpp	1.152 03/01/23 11:13:28 JVM"
#endif
/*
...
/*    else if ( instr->_matrule && instr->_matrule->_rChild && 
      ...
      fprintf(fp, " };\n");
             ...
             (!strcmp(instr->_matrule->_rChild->_opType,"CopyP") || 
              !strcmp(instr->_matrule->_lChild->_opType,"stackSlotP")) ) {
      // !!!!!
      // Special hack for ideal CopyP.  Bottom type is oop or not depending on input.
...
void ArchDesc::defineStateClass(FILE *fp) {
  ...
  fprintf(fp,"\n");
  fprintf(fp,"  void preset(MachNode*);           // Prepack the state\n");
  fprintf(fp,"\n");
  fprintf(fp,"  // Methods created by ADLC an invoked by Reduce\n");
  fprintf(fp,"  MachOper *MachOperGenerator( int opcode, MachNode *leaf );\n");
...
public:
  ...
  void closing()     { 
    ...
    if( end_instructions      != -1 )
      fprintf(_cpp, "  _last_Mach_Node  = %d,\n",  end_instructions);
    fprintf(_cpp, "  _PSEUDO_RULE_FOR_PRESET = %d // no trailing comma\n", end_instructions+1);
    OutputMap::closing();
  }
