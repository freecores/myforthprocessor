...
#pragma ident "@(#)cardTableModRefBS.cpp	1.41 03/01/23 12:07:05 JVM"
#endif
// 
  ...
  ModRefBarrierSet(max_covered_regions), _whole_heap(whole_heap)
{
  ...
  _byte_map_size = ReservedSpace::page_align_size_up((heap_size_in_words / 
  ...
  *guard_card = last_card;
  ...
  if (TraceCardTableModRefBS) {
    tty->print_cr("CardTableModRefBS::CardTableModRefBS: ");
    tty->print_cr("  "
...
void CardTableModRefBS::resize_covered_region(MemRegion new_region) {
  ...
  if (TraceCardTableModRefBS) {
    tty->print_cr("CardTableModRefBS::resize_covered_region: ");
    tty->print_cr("  "
...
size_t CardTableModRefBS::chunks_to_cover(MemRegion mr) {
  return (size_t)(addr_to_chunk_index(mr.end()) -
  ...
  for (int i = 0; i < _cur_covered_regions; i++) {
	...
	if (*cur_entry != clean_card) {
	  ...
	  if (clear) {
		...
		if (is_par) {
		  jbyte res =
		    atomic::compare_and_exchange_byte(clean_card,
						      &cur_entry[i],
						      cur_entry_val);
...
process_stride(Space* sp,
  ...
  // Invariant: chunk_mr is the intersection of the chunk corresponding
  // to "chunk_start_card" and "chunk_end_card" and the "used" region.
  MemRegion chunk_mr = MemRegion(addr_for(chunk_card_start),
				 addr_for(chunk_card_end));
  chunk_mr = chunk_mr.intersection(used);

  while (chunk_mr.word_size() > 0) {
    // Process the chunk.
    ...
    chunk_card_end    = chunk_card_start + CardsPerStrideChunk;
    chunk_mr          = MemRegion(addr_for(chunk_card_start),
				  addr_for(chunk_card_end));
    chunk_mr          = chunk_mr.intersection(used);
  }
}
