...
#pragma ident "@(#)mulnode.cpp	1.114 03/01/23 12:17:44 JVM"
#endif
/*
...
Node *LShiftINode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( add1_op == Op_AddI ) {    // Left input is an add?
    ...
    if( t12 && t12->is_con() ){ // Left input is an add of a con?
      ...
      if( t2->get_con() < 16 ) {
        ...
        return new (3) AddINode( lsh, phase->intcon(t12->get_con() << t2->get_con()));
      }
    }
  }
  ...
  if( (add1_op == Op_RShiftI || add1_op == Op_URShiftI ) &&
      add1->in(2) == in(2) ) 
    // Convert to "(x & -(1<<c0))"
    return new (3) AndINode(add1->in(1),phase->intcon( -(1<<t2->get_con())));
...
Node *LShiftLNode::Identity( PhaseTransform *phase ) {
  return ( phase->type( in(2) )->higher_equal( TypeLong::ZERO ) ) ? in(1) : this;
}
...
Node *LShiftLNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( add1_op == Op_AddL ) {    // Left input is an add?
    ...
    if( t12 && t12->is_con() ){ // Left input is an add of a con?
      ...
      // Compute X<<con0 + (con1<<con0)
      return new (3) AddLNode( lsh, phase->makecon(TypeLong::make(t12->get_con() << t2->get_con())));
    }
  }
  ...
  if( (add1_op == Op_RShiftL || add1_op == Op_URShiftL ) &&
      add1->in(2) == in(2) ) 
    // Convert to "(x & -(1<<c0))"
    return new (3) AndLNode(add1->in(1),phase->makecon(TypeLong::make( -(CONST64(1)<<t2->get_con()))));
...
Node *RShiftINode::Identity( PhaseTransform *phase ) {
  const TypeInt *t2 = phase->type(in(2))->isa_int();
  if( !t2 ) return this;
  if( t2->higher_equal( TypeInt::ZERO ) )
    return in(1);
...
Node *URShiftINode::Identity( PhaseTransform *phase ) {
  ...
  if( add->Opcode() == Op_AddI &&
      phase->type(add->in(2)) == TypeInt::make(wordSize-1) &&
      add->in(1)->Opcode() == Op_LShiftI &&
      phase->type(add->in(1)->in(2)) == TypeInt::make(LogBytesPerWord) )
    return add->in(1)->in(1);   
...
Node *URShiftLNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  const TypeInt *t2 = phase->type( in(2) )->isa_int();
  if( !t2 || !t2->is_con() ) return NULL; // Right input is a constant
  const int con = t2->get_con() & 63; // Shift count is always masked
  // We'll be wanting the right-shift amount as a mask of that many bits
