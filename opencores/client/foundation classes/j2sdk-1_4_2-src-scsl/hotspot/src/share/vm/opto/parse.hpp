...
#pragma ident "@(#)parse.hpp	1.240 03/02/28 09:18:11 JVM"
#endif
/*
...
class InlineTree : public ResourceObj {
  ...
  ciScope*    _scope;             // NULL if UseOptoInlining
...
protected:
  InlineTree(ciScope* callee_scope,   // NULL if UseOptoInlining
	     Compile* C,
  ...
  InlineTree *build_inline_tree_for_callee(ciScope* callee_scope,  //NULL
					   ciMethod* callee_method,
					   JVMState* caller_jvms,
					   int caller_bci);
...
public:
  ...
  ciScope    *scope()             const { return _scope;  }
...
class Parse : public GraphKit {
 public:
  // Parser bailout types
  enum BailoutConstants { 
    Bailout_none = 0,            // No bailout
    Bailout_too_many_args,       // Too many arguments to allocate
    Bailout_osr_unsupported,     // Non-empty expression or jsr stack
    Bailout_osr_needs_type_flow, // Missing type flow information for initial block
    Bailout_unhandled_bytecode,  // Unimplemented bytecode (for bootstrapping)
    Bailout_node_limit_exceeded  // Exceeded node construction limit during parsing
  };
 ...
 private:
  ...
  bool          _has_ex_handler;// Possibly covered by ex-handler, recursively
  bool          _wrote_final;   // Did we write a final field?
  bool          _count_invocations; // update and test invocation counter
  bool          _method_data_update; // update method data oop
  BailoutConstants _bailout;    // Record Bailout info during parse
...
#ifndef PRODUCT
  int           _deopt_cause;   // Record of deopt that can cause recompilation
  enum {
    No_deopt_in_method = (int)0x80000000  // No recompilation deopt generated
  };
 ...
 public:
  ...
  bool          has_ex_handler()const { return _has_ex_handler; }
  ...
  BailoutConstants bailout()    const { return _bailout; }
  ...
  bool parse_failed()           { return bailout() != Bailout_none; }
 ...
 private:
  ...
  // Note a bailout.  The current parse fails, and perhaps the current compile.
  void set_parser_bailout(BailoutConstants b);
 ...
 public: // make public for GraphKit, only
  // Record if uncommon trap bailout inserted.
  // No_deopt_in_method indicates no bailouts.
  void record_deopt(int deopt_cause)  PRODUCT_RETURN;
 ...
 private:
  ...
  // Helper function to record dependence on an inlined method for the purposes of
  // dynamic class redefinition that can happen later
  // Does nothing if !HotSwap.
  void record_dependence_for_dynamic_class_redefinition(ciMethod* callee);
  ...
  void    jump_if_true_fork(IfNode *ifNode, int dest_bci_if_true);
  void    jump_if_false_fork(IfNode *ifNode, int dest_bci_if_false);
  ...
  void increment_and_test_invocation_counter();
  void increment_profiling_counter(Node* counter_addr);
  void increment_method_data_at(ciMethodData* md, ciProfileData* data, ByteSize offset);

  void profile_method_entry();
  void profile_taken_branch(int bci);
  void profile_not_taken_branch(int bci);
  void profile_call(int bci);
  void profile_static_call(int bci);
  void profile_virtual_call(int bci);
  void profile_ret(int bci);
  void profile_checkcast(int bci);
  void profile_switch_case(int bci, int table_index);
 ...
 public:
  ...
  // Print cause for bailout.
  void show_bailout_info()   PRODUCT_RETURN;
#ifndef PRODUCT
