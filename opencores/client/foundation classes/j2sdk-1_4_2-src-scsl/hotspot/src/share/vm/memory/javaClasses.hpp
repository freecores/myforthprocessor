...
#pragma ident "@(#)javaClasses.hpp	1.119 03/01/23 12:09:12 JVM"
#endif
/*
...
class java_lang_ThreadGroup : AllStatic {
 private:
  enum {
   ...
   hc_threads_offset           = 2,
   hc_groups_offset            = 3,
   hc_maxPriority_offset       = 4,
   hc_destroyed_offset         = 5,
   hc_daemon_offset            = 6,
   hc_vmAllowSuspension_offset = 7,
   hc_nthreads_offset          = 8,
   hc_ngroups_offset           = 9 
  };
  ...
  static int groups_offset;
  static int maxPriority_offset;
  static int destroyed_offset;
  static int daemon_offset;
  static int vmAllowSuspension_offset; 
  static int nthreads_offset;  
  static int ngroups_offset; 
...
class java_lang_ref_Reference: AllStatic {
 public:
  enum {
   ...
   hc_queue_offset                         =  1,
   hc_next_offset                          =  2,
   hc_discovered_offset			   =  3  // If not last, see SoftRefs.
  };
  enum {
...
class java_lang_ref_SoftReference: public java_lang_ref_Reference {
 public:
  enum {
   hc_first_non_static_offset  = hc_discovered_offset + 1,
   // The timestamp is a long field and may need to be adjusted for
   // alignment per ALIGN_LONG_FIELDS.  For example, when there were
   // 3 oop fields in a java.lang.ref.Reference the definition was.
   // hc_timestamp_offset                     = ALIGN_LONG_FIELDS ? (4) : (3)
   hc_timestamp_offset         = hc_first_non_static_offset +
				 (ALIGN_LONG_FIELDS ? hc_first_non_static_offset % 2 : 0)
  };
  enum {
...
// Interface to java.io.FileDescriptor objects

class java_io_FileDescriptor: AllStatic {
 private:
  enum {
   hc_fd_offset = 0 // There are more fields, but this is the only one we currently use
  };

  static int fd_offset;

 public:
  static long     fd(oop java_filedescriptor);
  static void set_fd(oop java_filedescriptor, long fd);
