...
#pragma ident "@(#)memnode.cpp	1.202 03/01/23 12:17:32 JVM"
#endif
/*
...
Node *MemNode::Ideal_common(PhaseGVN *phase, bool can_reshape) {
  ...
  if( phase->type( mem ) == Type::TOP ) return (Node*)-1; // caller will return NULL
  if( mem == this )                     return (Node*)-1; // caller will return NULL

  Node *address = in(MemNode::Address);
  const Type *t_adr = phase->type( address );
  if( t_adr == Type::TOP )              return (Node*)-1; // caller will return NULL
...
const Type *LoadNode::Value( PhaseTransform *phase ) const {
  ...
  if (tp->base() == Type::AryPtr) {
    ...
    if (t->base() == Type::Long && Opcode() == Op_LoadI)
      t = TypeInt::INT;
