...
#pragma ident "@(#)safepoint.hpp	1.75 03/04/25 10:53:05 JVM"
#endif
/*
...
class SafepointSynchronize : AllStatic {
 ...
 private:
  ...
  static TimeStamp  _timer;
#endif

  static void begin_statistics(int nof_threads) PRODUCT_RETURN;
  static void end_statistics()                  PRODUCT_RETURN;
...
public:
  ...
  static bool can_be_at_safepoint_before_suspend  (JavaThread *thread, JavaThreadState state);
  static bool can_be_at_safepoint_after_suspend   (JavaThread *thread, JavaThreadState state);
  static void save_thread_state_at_safepoint      (JavaThread *thread);
  static void patch_return_instruction_md         (address cb_pc);
  static bool needs_deopt_suspend                 (void);
  static bool needs_deopt_suspend                 (JavaThread *thread);
#ifndef CORE
  static void do_deopt_suspend                    (JavaThread *thread);
#endif
  ...
  static int  handle_illegal_instruction_exception(JavaThread *thread);
  ...
  #ifndef PRODUCT
  static void log_suspension()                             { _num_of_suspends += 1; }   
  #endif
...
class ThreadSafepointState: public CHeapObj {
 public:  
  enum suspend_type {    
    ...
    _at_safepoint_suspended =  2, // Thread at a safepoint and is suspended
    _call_back              =  3, // Keep executing and wait for callback (if thread is in interpreter or vm)
    _compiled_safepoint     =  4  // Compiled safepoint
  };
 private:
...
class SafepointHandler : public CHeapObj {
 ...
 public:
  ...
  virtual int  handle_illegal_instruction_exception()   { ShouldNotReachHere(); return 0;}
...
public:
  ...
  int  handle_illegal_instruction_exception();
