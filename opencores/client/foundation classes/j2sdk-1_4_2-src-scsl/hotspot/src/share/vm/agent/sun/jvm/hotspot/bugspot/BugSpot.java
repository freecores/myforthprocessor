/*
 * @(#)BugSpot.java	1.6 03/01/23 11:22:37
 *
...
public class BugSpot extends JPanel {
  ...
  public void showMemoryDialog() {
    ...
    final MemoryPanel memory = new MemoryPanel(getDebugger(),
    ...
    addressField.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          try {
            memory.makeVisible(getDebugger().parseAddress(addressField.getText()));
          } catch (NumberFormatException ex) {
          }
        }
      });
    memoryDialog.getContentPane().add(addressPanel, BorderLayout.NORTH);

    memoryDialog.getContentPane().add(memory, BorderLayout.CENTER);
