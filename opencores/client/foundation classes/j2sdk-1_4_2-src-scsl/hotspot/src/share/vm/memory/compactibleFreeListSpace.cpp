...
#pragma ident "@(#)compactibleFreeListSpace.cpp	1.82 03/01/23 12:07:33 JVM"
#endif
/*
...
size_t CompactibleFreeListSpace::totalCount() {
  return totalCountInIndexedFreeLists() +
...
void CompactibleFreeListSpace::reportFreeListStatistics() const {
  ...
  if (PrintFLSStatistics > 1) {
    ...
    size_t totalSize = totalSizeInIndexedFreeLists() +
      _dictionary->dictAmountFree();;
    tty->print(" freeblksz=%ld frag=%15.10e\n", totalSize, flsFrag());
  }
}

void CompactibleFreeListSpace::reportIndexedFreeListStatistics() const {
  assert_lock_strong(&_freelistLock);
  printf("Statistics for IndexedFreeLists:\n"
         "--------------------------------\n");
  size_t totalSize = totalSizeInIndexedFreeLists();
...
HeapWord* CompactibleFreeListSpace::allocate(size_t size, 
  ...
  if (res != NULL) {
    ...
    assert(!ALIGN_ALL_OBJECTS || is_aligned((void*)res),
           "alignment check");
...
FreeChunk* CompactibleFreeListSpace::getChunkFromGreater(size_t numWords) {
  ...
  assert(!ALIGN_ALL_OBJECTS || (currSize % 2 == 0),
         "currSize should be a double-word multiple");
...
oop CompactibleFreeListSpace::promote(oop obj, size_t obj_size, oop* ref) {
  ...
  if (_promoInfo.tracking() && !_promoInfo.ensureSpoolingSpace()) {
    return NULL;
  }
  ...
  if (res != NULL) {
    Memory::copy_words_aligned((HeapWord*)obj, res, obj_size);
...
CompactibleFreeListSpace::returnChunkToDictionary(FreeChunk* chunk) {
  ...
  _dictionary->increment_fragmentation_metric_by((double) size * (double) size);
  _dictionary->increment_amount_free_by(size);
...
CompactibleFreeListSpace::removeChunkFromDictionary(FreeChunk* fc) {
  ...
  _bt.verify_single_block((HeapWord*)fc, size);
  _dictionary->decrement_fragmentation_metric_by((double) size * (double) size);
  _dictionary->decrement_amount_free_by(size);
...
FreeChunk* CompactibleFreeListSpace::bestFitSmall(size_t numWords) {
  ...
  if (start < IndexSetSize) {
    ...
    while (hint < IndexSetSize) {
      assert(!ALIGN_ALL_OBJECTS || (hint % 2 == 0),
             "hint should be double-word multiple");
...
CompactibleFreeListSpace::gc_epilogue() {
  assert_locked();
  if (PrintMiscellaneous && Verbose && !_adaptive_freelists) {
...
void PromotionInfo::promoted_oops_iterate##nv_suffix(OopClosureType* cl) {  \
  /* go over the list of promoted objects, removing each from the list,     \
  ...
  for (curObj = _promoHead; curObj != NULL; curObj = nextObj) {             \
    if ((nextObj = curObj->next()) == NULL) {                               \
      /* protect ourselves against additions due to closure appliation below\
     ...
     * we actually forward _promoHead; Note that this is not necessary      \
     * in general (i.e. when we don't need to call PromotionInfo::verify()  \
     * because oop_iterate can only add to the end of _promoTail, and never \
     * needs to look at _promoHead.                                         \
     */                                                                     \
  ...
  /* post-condition: this is really a tautology, given the loop exit        \
   * condition above, but this checks the correctness of the                \
   * "list restart" code above */                                           \
  assert(noPromotions(), "post-condition violation");                       \
}

ALL_SINCE_SAVE_MARKS_CLOSURES(CFLS_OOP_SINCE_SAVE_MARKS_DEFN)
ALL_SINCE_SAVE_MARKS_CLOSURES(PROMOTED_OOPS_ITERATE_DEFN)
...
double CompactibleFreeListSpace::flsFrag() const {
  ...
  {
    double totFree = itabFree + _dictionary->dictAmountFree();
    return ((double)1.0) - 
           ((frag + _dictionary->fragmentation_metric()) / 
	    (totFree * totFree));
  }
}
...
void CompactibleFreeListSpace::verifyIndexedFreeList(size_t size) const {
  for (FreeChunk* fc = _indexedFreeList[size].head(); fc != NULL;
    fc = fc->next()) {
    guarantee(!ALIGN_ALL_OBJECTS || (size % 2 == 0),
              "Odd slots should be empty");
...
void CompactibleFreeListSpace::printFLCensus(int sweepCt) const {
  ...
  int splitBirths = 0;
  int splitDeaths = 0;
  tty->print("end sweep# %d\n", sweepCt);
  tty->print("%4s\t"    "%7s\t"      "%7s\t"      "%7s\t"      "%7s\t"
             "%7s\t"    "%7s\t"      "%7s\t"      "%7s\t"      "%7s\t"
             "%7s\t"    "\n",
  ...
  tty->print_cr("Total free in indexed lists %d words", totalFree);
  tty->print("growth: %8.5f  deficit: %8.5f\n",
...
void PromotionInfo::saveDisplacedHeader(markOop hdr) {
  ...
  if (++_nextIndex == _spoolTail->bufferSize) { // last location in this block
    ...
    if ((_spoolTail->nextSpoolBlock = getSpoolBlock()) != NULL) {
      _spoolTail = _spoolTail->nextSpoolBlock;
      _nextIndex = 1;
    } else if (PrintMiscellaneous && Verbose) {
      warning("Spool area exhausted");
    }
  }
}
...
bool PromotionInfo::ensureSpoolingSpaceWork() {
  assert(!hasSpoolingSpace(), "Only call when there is no spooling space");
  ...
  if (_spoolTail == NULL) {
    ...
    _spoolHead = _spoolTail = newSpool;
...
void PromotionInfo::startTrackingPromotions() {
  assert(_spoolHead == _spoolTail && _firstIndex == _nextIndex, "premature");
  _firstIndex = _nextIndex = 1;
  _tracking = true;
}
...
void PromotionInfo::verify() const {
  ...
  assert(_spoolHead == NULL || _spoolTail != NULL, "list consistency");
  ...
  for (SpoolBlock* curSpool = _spoolHead; curSpool != _spoolTail;
  ...
  if (_spoolTail != NULL) {
    ...
    assert(numDisplacedHdrs == 0, "internal consistency");
  }
...
CFLS_LAB::CFLS_LAB(CompactibleFreeListSpace* cfls) :
  _cfls(cfls),
  _max_alloc_block_start(NULL), _max_alloc_block_end(NULL)
{
...
HeapWord* CFLS_LAB::alloc(size_t word_sz) {
  ...
  HeapWord* hw_res = (HeapWord*)res;
  if (hw_res > _max_alloc_block_start) {
    _max_alloc_block_start = hw_res;
    _max_alloc_block_end = hw_res + word_sz;
  }
  return hw_res;
}
...
void CFLS_LAB::retire() {
  ...
  if (_max_alloc_block_start != NULL) {
    MutexLockerEx x(ParGCRareEvent_lock,
                    Mutex::_no_safepoint_check_flag);
    _cfls->_bt.allocated(_max_alloc_block_start,
			 _max_alloc_block_end);
    _max_alloc_block_start = NULL;
    _max_alloc_block_end = NULL;
  }
}
