...
#pragma ident "@(#)loopTransform.cpp	1.80 03/01/23 12:17:07 JVM"
#endif
//
...
bool IdealLoopTree::policy_peeling( PhaseIdealLoop *phase ) const {
  ...
  if( body_size > 255 /* Prevent overflow for large body_size */
      || (body_size * body_size + uniq > 65000) ) {
...
int IdealLoopTree::policy_maximally_unroll( PhaseIdealLoop *phase ) const {
  ...
  // Non-constant bounds
  if( !init->is_Con() || !limit->is_Con() ) return 0;
  ...
  return 0;               // Do not maximally unroll
}
...
// Actually we do interation-splitting, a more powerfully form of RCE.
bool IdealLoopTree::policy_range_check( PhaseIdealLoop *phase ) const {
  ...
  for( uint i = 0; i < _body.size(); i++ ) {
	...
	if( !add->is_Add() && !add->is_Sub() ) continue; // Odd compare
...
// Insert pre and post loops.  If peel_only is set, the pre-loop cannout have
...
void PhaseIdealLoop::insert_pre_post_loops( IdealLoopTree *loop, Node_List &old_new, bool peel_only ) {
  ...
  // Build the IfNode
  IfNode *zer_iff = new (2) IfNode( new_main_exit, zer_bol, 0.50f, -1.0f );
  ...
  // Build the IfNode
  IfNode *min_iff = new (2) IfNode( new_pre_exit, min_bol, 0.50f, -1.0f );
...
void PhaseIdealLoop::do_unroll( IdealLoopTree *loop, Node_List &old_new, bool adjust_min_trip ) {
  ...
  // Step 3: Find the min-trip test guarenteed before a 'main' loop.
...
void PhaseIdealLoop::do_maximally_unroll( IdealLoopTree *loop, Node_List &old_new, int trip_count ) {
  assert( trip_count > 0, "");
...
void PhaseIdealLoop::do_range_check( IdealLoopTree *loop, Node_List &old_new ) {
  ...
  for( uint i = 0; i < loop->_body.size(); i++ ) {
	...
	if( loop->is_member(get_loop(limit_c) ) ) 
      ...
      if( limit_c == ctrl ) continue;// Bail out for being pinned below zero trip test

      if( offset == trip_counter ) {
	offset = zero;          // Use zero offset
      } else if( offset->is_Add() ) {
        // These pattern matches are not repeated for the is_Sub() below
...
void IdealLoopTree::adjust_loop_exit_prob( PhaseIdealLoop *phase ) {
  ...
  while( test != _head ) {
	...
	float p = iff->_prob;
	if( !phase->is_member( this, ex ) && iff->_fcnt <= -0.9 ) {
	  if( top == Op_IfTrue ) {
	    if( p < 0.501 ) iff->_prob = 0.9f;
	  } else {
	    if( p > 0.499 ) iff->_prob = 0.1f;
	  }
	}
      }
    }
...
void IdealLoopTree::iteration_split_impl( PhaseIdealLoop *phase, Node_List &old_new ) {
  ...
  if( cl->is_normal_loop() ) {
    int trip_count = policy_maximally_unroll(phase);
    if( trip_count > 0 ) {
      // Here we did some unrolling and peeling.  Eventually we will 
      // completely unroll this loop and it will no longer be a loop.
      phase->do_maximally_unroll(this,old_new,trip_count);
      return;
    }
  }
