...
#pragma ident "@(#)output_c.cpp	1.147 03/02/14 11:19:56 JVM"
#endif
/*
...
void ArchDesc::defineExpand(FILE *fp, InstructForm *node) {
  ...
  if( node->expands() ) {
    ...
    unsigned      numi     = node->_exprule->_instrs.count();
    ...
    for(node->_exprule->_instrs.reset();
        (new_id = node->_exprule->_instrs.iter()) != NULL; cnt++) {
      // Build the node for the instruction
      fprintf(fp,"  %sNode *n%d = new %sNode();\n", new_id, cnt, new_id);
      // Add control edge for this node
	...
	fprintf(fp, "  ((MachIfNode*)n%d)->_fcnt = _fcnt;\n",cnt);
      ...
      fprintf(fp,"  n%d->set_opnd_array(0, state->MachOperGenerator( %s, n%d ));\n", 
              cnt, machOperEnum(resultOper), cnt);
      // Grab the instruction's operand NameList
      NameList *lst = (NameList *)node->_exprule->_opers[new_id];
      ...
      for( lst->reset(); (opid = lst->iter()) != NULL; ) {
        ...
        if ((exp_pos = node->_exprule->_newopers.index(opid)) != -1) {
          ...
          // If this operand is a definition
          if((eform) && (is_def(eform->_use_def))) {
            // Check if this instruction defs a locally created operand
            new_pos = new_inst->_components.operand_position(parameter,Component::DEF);
  ...
  if( ! node->expands() && node->needs_projections() ) {
    ...
    while( (comp = node->_components.iter()) != NULL ) {
      if( comp->isa(Component::DEF) || comp->isa(Component::KILL) ) {
        ...
        fprintf(fp,"  kill = ");
        fprintf(fp,"new (1) MachProjNode( %s, MachProjNode::fat_proj, (%s), Op_%s );\n",
...
void ArchDesc::buildMachOperGenerator(FILE *fp_cpp) {
  ...
  fprintf(fp_cpp, "(int opcode, MachNode *mach)");
  fprintf(fp_cpp, "{\n");
  fprintf(fp_cpp, "\n");
...
void ArchDesc::buildMachNode(FILE *fp_cpp, InstructForm *inst, const char *indent) {
  ...
  if ( (inst->num_post_match_opnds() != 0) ) {
	...
	continue;
      ...
      fprintf(fp_cpp, "%s node->_opnd_array[%d] = ", indent, index);
      fprintf(fp_cpp, "MachOperGenerator(%s,%s);\n", opcode, parent);
      }
  } 
