...
#pragma ident "@(#)globalDefinitions_sparcWorks.hpp	1.64 03/01/23 12:28:16 JVM"
#endif
/*
...
// ARENA MALLOC ALIGNMENT

#define ARENA_AMALLOC_ALIGNMENT (sizeof(char *))


// unaligned access
inline void store_unaligned_ushort(jushort *dst, jushort val) {
  jubyte *p = (jubyte *)dst;

  *p     = (val >> 8) & 0x00ff;
  *(p+1) = val & 0x00ff;
}
inline void store_unaligned_uint(juint *dst, juint val)       {
  jubyte *p = (jubyte *)dst;
...
inline jushort load_unaligned_ushort(jushort *dst)            {
  jubyte *p = (jubyte *)dst;

  return (jushort)((*p << 8) | *(p+1));
}
inline juint load_unaligned_uint(juint *dst)                  {
  jubyte *p = (jubyte *)dst;

  return (juint)((*p << 24) | (*(p+1) << 16) | (*(p+2) << 8) | *(p+3));
}
//----------------------------------------------------------------------------------------------------
// ANSI C++ fixes
