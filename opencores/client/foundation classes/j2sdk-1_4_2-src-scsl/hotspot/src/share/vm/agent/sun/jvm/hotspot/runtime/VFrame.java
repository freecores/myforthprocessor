/*
 * @(#)VFrame.java	1.11 03/01/23 11:46:04
 *
...
public class VFrame {
  ...
  public static VFrame newVFrame(Frame f, RegisterMap regMap, JavaThread thread, boolean unsafe, boolean mayBeImprecise) {
    ...
    if (!VM.getVM().isCore()) {
      ...
      if (cb != null) {
        if (cb.isNMethod()) {
          ...
          if (mayBeImprecise || VM.getVM().isDebugging()) {
            ...
            scope = nm.getScopeDescAt(f.getPC(), regMap.isPCAtCall(f.getID()));
          }
      ...
      // Deoptimized frame
      //      if (f.isDeoptimizedFrame()) {
      //        throw new RuntimeException("Can not handle deoptimized frames yet");
        //        return new DeoptimizedVFrame(f, regMap, thread);
      //      }
    }
  ...
  public boolean isDeoptimizedFrame() { return false; }
