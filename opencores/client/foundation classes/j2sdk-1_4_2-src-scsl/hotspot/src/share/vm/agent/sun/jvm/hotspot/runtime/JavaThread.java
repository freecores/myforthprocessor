/*
 * @(#)JavaThread.java	1.26 03/01/23 11:45:09
 *
...
public class JavaThread extends Thread {
  ...
  private static AddressField  notAtCallIDield;
  private static CIntegerField threadStateField;

  private static JavaThreadPDAccess access;		

  // not-at-call bit read from underlying process
  private static long          notAtCallBit;
  ...
  private static int           IN_NATIVE_BLOCKED;
  ...
  private static synchronized void initialize(TypeDataBase db) {
    ...
    notAtCallIDield  = anchorType.getAddressField("_not_at_call_id");
    threadStateField  = type.getCIntegerField("_thread_state");
    
    notAtCallBit      = db.lookupIntConstant("JavaFrameAnchor::not_at_call_bit").intValue();
    ...
    IN_NATIVE_BLOCKED = db.lookupIntConstant("_thread_in_native_blocked").intValue();
  }
  ...
  public Address getLastJavaSP() {
    ...
    if (sp != null) {
      long mask = notAtCallBit;
      mask = ~mask;
      sp = sp.andWithMask(mask);
      // Raw "biased" stack pointer?
      // FIXME: is this equivalent to the underlying C++ code?
      // No way to get the absolute value of an address.
  ...
  public JavaThreadState getThreadState() {
    ...
    } else if (val == IN_NATIVE_BLOCKED) {
      return JavaThreadState.IN_NATIVE_BLOCKED;
    } else {
  ...
  // FIXME: not yet implementable
  //  void  set_pc_not_at_call_for_frame(intptr_t* sp)   { _not_at_call_id = sp; set_bits((intptr_t&)_last_Java_sp, not_at_call_bit); }
  
  public boolean notAtCallFrameExists() {
    Address sp = getLastJavaSP();
    if (sp == null) {
      return false;
    }
    return (sp.andWithMask(notAtCallBit) != null);
  }

  public Address getNotAtCallID() {
    return (notAtCallFrameExists() ? notAtCallIDield.getValue(addr.addOffsetTo(anchorField.getOffset())) : null);
  }
  ...
  private Frame cookLastFrame(Frame fr) {
    ...
    // FIXME: implement this for non-core system
    // #ifndef CORE
    ...
    //     assert(AbstractInterpreter::contains(adjusted_pc) || CodeCache::contains(adjusted_pc) || StubRoutines::contains(adjusted_pc), "invalid pc");
    // #else
    //     // No adjustment needed for interpeter-only system
