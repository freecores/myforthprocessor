...
#pragma ident "@(#)generateOopMap.cpp	1.126 03/01/23 12:12:18 JVM"
#endif
/*
...
void GenerateOopMap::update_basic_blocks(int bci, int delta) {     
  int newWords = binsToHold(method()->code_size() + delta);
  
  uintptr_t * new_bb_hdr_bits = NEW_RESOURCE_ARRAY(uintptr_t, newWords);
    
  BitMap bb_bits(new_bb_hdr_bits, method()->code_size() + delta);
  bb_bits.clear();
...
void GenerateOopMap::interp1(BytecodeStream *itr) {  
  ...
  switch(itr->code()) {
    ...
    // Pair bytecodes for (aload_0, _fast_igetfield)
    case Bytecodes::_fast_iaccess_0:     ppush1(valCTS);            break;
    // Pair bytecodes for (aload_0, _fast_agetfield)
    case Bytecodes::_fast_aaccess_0:     pp_new_ref(rCTS, itr->bci()); break;

    case Bytecodes::_fast_igetfield:     pp(rCTS, vCTS);            break;
    case Bytecodes::_fast_agetfield:     pp_new_ref(rCTS, itr->bci()); break;
...
void GenerateOopMap::do_ldc(int idx, int bci) {
  ...
  CellTypeState cts = (tag.is_string() || tag.is_unresolved_string()) ? CellTypeState::make_line_ref(bci) : valCTS;
  ppush1(cts);
}
...
class RelocCallback : public RelocatorListener {
 ...
 public:
  ...
  // Callback method
  virtual void relocated(int bci, int delta) {
    _gom->update_basic_blocks  (bci, delta);
