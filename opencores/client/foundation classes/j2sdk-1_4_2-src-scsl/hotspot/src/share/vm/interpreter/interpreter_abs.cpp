...
#pragma ident "@(#)interpreter_abs.cpp	1.209 03/04/30 14:12:29 JVM"
#endif
/*
...
#include "incls/_interpreter_abs.cpp.incl"
...
void AbstractInterpreter::initialize() {
...
#ifdef CC_INTERP
  {
    ...
    if ( jvmdi::enabled() || jvmpi::enabled() )
...
NOT_PRODUCT(address  AbstractInterpreter::_bci_histogram_entry              = NULL;)
...
#ifdef PRODUCT
    int codelet_size = 20*K;
#else
    int codelet_size = 40*K;  // extra space for VerifyThread, VerifyFPU, etc..
#endif // PRODUCT
...
void AbstractInterpreterGenerator::generate_all() {
...
#ifndef PRODUCT
  { CodeletMark cm(_masm, "bci histogram support", false);
    Interpreter::_bci_histogram_entry = generate_bci_histogram_entry();
  }
#endif // PRODUCT
  ...
  { CodeletMark cm(_masm, "throw exception entrypoints");
    ...
    Interpreter::_throw_ClassCastException_entry             = generate_exception_handler("java/lang/ClassCastException"            , NULL       );
...
#ifndef PRODUCT
  if (PrintBCIHistogram)                                         histogram_bci();
...
void AbstractInterpreter::ignore_safepoints() {
  if (_notice_safepoints) {
    if ((!jvmdi::enabled()) || (jvmdi::ignore_safepoints())) {
