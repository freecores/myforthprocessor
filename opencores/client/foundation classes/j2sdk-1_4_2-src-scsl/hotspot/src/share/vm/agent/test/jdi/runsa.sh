...
{
    cat <<EOF
    Run sagclient.class using Serviceability Agent to talk to a corefile/pid.
    Usage:  runsa.sh [-jdk <jdk-pathname>] [-gui] [ -jdbx ] [ pid | corefile]

    -jdk means to use that jdk.  Default is 1.4.1/latest.
    -gui means to run the SA gui instead of the SA- JDI client
...
javacp=".:$classesDir:$jdk/lib/tools.jar:$jdk/classes:./workdir"
...
if [ "$do" = jdbx ] ; then
    ...
    $jdk/bin/java -Djava.class.path=$javacp $extraArgs $theClass $args
