...
#pragma ident "@(#)allocation.cpp	1.55 03/01/23 12:06:33 JVM"
#endif
/*
...
static volatile jint trace_lock = 0;

void trace_heap_malloc(size_t size, const char* name, void* p) {
  while(atomic::exchange(1, (jint *)&trace_lock) != 0) os::yield();
  tty->print_cr("Heap malloc " INTPTR_FORMAT " %7d %s", p, size, name == NULL ? "" : name);
  atomic::store(0, &trace_lock);
}
