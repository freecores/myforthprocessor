/*
 * @(#)SPARCFrame.java	1.16 03/01/23 11:46:37
 *
...
public class SPARCFrame extends Frame {
  ...
  // FIXME: move into some sort of VMGlobals.java
  private static boolean useC2CallingConventions;
  ...
  private static void initialize(TypeDataBase db) {
    ...
    useC2CallingConventions = (db.lookupIntConstant("UseC2CallingConventions").intValue() != 0);
  }
  ...
  public SPARCFrame(Address raw_sp, Address raw_youngerSP, long pcAdjustment, boolean youngerFrameIsInterpreted) {
    super();
    if (DEBUG) {
    ...
    if (raw_youngerSP == null) {
      ...
      pc = youngerSP.getAddressAt(SPARCRegisters.I7.spOffsetInSavedWindow()).addOffsetTo(PC_RETURN_OFFSET + pcAdjustment);
      if (Assert.ASSERTS_ENABLED) {
    ...
    if (youngerFrameIsInterpreted) {
      ...
      if (VM.getVM().isClientCompiler()) {
        ...
        interpreterSPAdjustmentOffset = unBiasSP(getYoungerSP().getAddressAt(IsavedSP)).minus(getSP());
      } else if (VM.getVM().isServerCompiler()) {
        // adjustment to this frame's SP was saved by its interpreted callee
        interpreterSPAdjustmentOffset = unBiasSP(getSP().getAddressAt(IsavedSP)).minus(getSP());
      }
    } else {
  ...
  public Frame sender(RegisterMap map, CodeBlob cb) {
    return senderWithPCAdjustment(map, cb, true);
  }
  ...
  public Address getFP() {
    Address sp = getSP();
    if (sp == null) {
      System.err.println("SPARCFrame.getFP(): sp == null");
    }
    Address fpAddr = sp.addOffsetTo(SPARCRegisters.FP.spOffsetInSavedWindow());
    try {
      Address fp = unBiasSP(fpAddr.getAddressAt(0));
      if (fp == null) {
        System.err.println("SPARCFrame.getFP(): fp == null (&fp == " + fpAddr + ")");
      }
      return fp;
    } catch (RuntimeException e) {
      System.err.println("SPARCFrame.getFP(): is bad (&fp == " + fpAddr + " sp = " + sp + ")");
      return null;
    }
  }
  ...
  public int getInterpreterFrameBCI() {
    ...
    // Interpreter native frames have null bcp
  ...
  protected long getOopMapOffsetAdjustmentPD() {
    return interpreterSPAdjustmentOffset;
  }
  ...
  /** This method has been added to enable passing in of a flag which
      indicates whether the pc needs to be adjusted (in case the frame
      is a safepoint patched frame). Adding the method here avoids
      changes to the shared sender method. */
  private Frame senderWithPCAdjustment(RegisterMap regMap, CodeBlob cb, boolean adjustPC) {
    ...
    if (VM.getVM().isDebugging()) {
      ...
      if (isSignalHandlerFrameDbg()) {
        ...
        // System.err.println("SPARCFrame.senderWithPCAdjustment: found signal handler frame");
        ...
        // System.err.println("  start of mcontext: " + fp);
        ...
        // System.err.println("  next frame's SP: " + sp + " PC: " + pc);
    ...
    if (!VM.getVM().isCore()) {
      ...
      if (cb != null) {
        // %%% Re-check this optimized fast path, and clean up the code duplication
        // if it is no longer necessary.
        if (Assert.ASSERTS_ENABLED) {
          Assert.that(!cb.callerMustGCArguments(map.getThread()) && !map.getIncludeArgumentOops(),
        ...
        if (map.getUpdateMap()) {
          if (isDeoptimizedFrame()) {
            ...
            // map.getThread().getVFrameArrayFor(youngerSP).updateRegisterMap(map);
            // map.makeIntegerRegsUnsaved();
          } else if (cb.getOopMaps() != null) {
            OopMapSet.updateRegisterMap(this, cb, map, VM.getVM().isDebugging());
          }
        }
        if (adjustPC) {
          // FIXME: not doing pcAdjustment because we assume we won't
          // currently be at a safepoint
          //          Address senderPC0 = getSenderPC();
          //          Address senderPC = map.getThread().getSafepointState().computeAdjustedPC(senderPC0);
          //          // Adjust the senderPC if it points into a temporary codebuffer.      
          //          pcAdjustment = senderPC.minus(senderPC0);
        }
      } else {
        boolean condition = false;
        if (VM.getVM().isClientCompiler()) {
          if (VM.getVM().getInterpreter().contains(pc) || isOSRAdapterFrame()) {
            isInterpreted = true;
            map.makeIntegerRegsUnsaved();
            map.shiftWindow(sp, youngerSP);
            condition = true;
        ...
        if (!condition) {
          ...
          if (cb != null) {
            // %%% the following code is duplicated from the top of this function
            if (adjustPC) {
              //              Address senderPC0 = getSenderPC();
              //              Address senderPC  = map.getThread().getSafepointState().computeAdjustedPC(senderPC0);
              //              // Adjust the senderPC if it points into a temporary codebuffer.      
    ...
    return new SPARCFrame(biasSP(sp), biasSP(youngerSP), pcAdjustment, isInterpreted);
  }
  ...
  private Frame senderForEntryFrame(RegisterMap regMap) {
    ...
    if (!VM.getVM().isCore()) {
      map.makeIntegerRegsUnsaved();
      map.shiftWindow(lastJavaSP, youngerSP);
    }
    ...
    return new SPARCFrame(biasSP(lastJavaSP), biasSP(youngerSP), 0, false);
  }
  ...
  public long getUContextOffset() {
    ...
    //    System.err.println("  SPARCFrame.isSignalHandlerFrameDbg: I2 = " + i2 + ", fp = " + fp + ", youngerSP = " + youngerSP);
  ...
  public long getMContextAreaOffsetInUContext() {
    ...
    //    System.err.println("SPARCFrame.getMContextAreaOffsetInUContext: offset = " + offset);
