...
#pragma ident "@(#)jni.cpp	1.340 03/04/03 18:30:00 JVM"
#endif
/*
...
// Define a class to lock and unlock the JNIid just like MutexLocker
class JNIidLocker : StackObj {
 private:
  JNIid* _jid;

 public:
  JNIidLocker(JNIid* jid) {
    _jid = jid;
    _jid->lock();
  }
...
// Wrapper to trace JNI functions
  ...
  JNIHistogramElement::JNIHistogramElement(const char* elementName) {
    ...
    while (atomic::compare_and_exchange(1, &JNIHistogram_lock, 0) != 0) {
      while (JNIHistogram_lock != 0) {
        count +=1;
    ...
    atomic::decrement(&JNIHistogram_lock);
  }
...
JNI_ENTRY(jclass, jni_DefineClass(JNIEnv *env, const char *name, jobject loaderRef, const jbyte *buf, jsize bufLen))
  ...
  klassOop k = SystemDictionary::resolve_from_stream(class_name, class_loader, Handle(), &st, CHECK_0);
...
JNI_ENTRY(jclass, jni_FindClass(JNIEnv *env, const char *name))
  ...
  if (k.not_null()) {    
    ...
    // Special handling to make sure JNI_OnLoad and JNI_OnUnload are executed in the correct class context.
    if (loader.is_null() && k->name() == vmSymbols::java_lang_ClassLoader_NativeLibrary()) {      
      ...
      oop mirror = (oop) result._value.h;
      loader = Handle(THREAD, instanceKlass::cast(java_lang_Class::as_klassOop(mirror))->class_loader());
      protection_domain = Handle(THREAD, instanceKlass::cast(java_lang_Class::as_klassOop(mirror))->protection_domain());
    }
  } else {
    // We call ClassLoader.getSystemClassLoader to obtain the system class loader.
    loader = Handle(THREAD, SystemDictionary::java_system_loader());
  }
  symbolHandle sym = oopFactory::new_symbol_handle((char*) name, CHECK_0);
  jclass result = find_class_from_class_loader(env, sym, true, loader, protection_domain, true, thread);

  // If we were the first invocation of jni_FindClass, we enable compilation again
  // rather than just allowing invocation counter to overflow and decay. Controlled 
  // by flag DelayCompilationDuringStartup.
  NOT_CORE(if (first_time && !CompileTheWorld) CompilationPolicy::completed_vm_startup();)
...
JNI_ENTRY(jmethodID, jni_FromReflectedMethod(JNIEnv *env, jobject method))
  ...
  // find corresponding JNIid object
  JNIid* id = instanceKlass::cast(k1())->jni_id_for(slot);
...
JNI_ENTRY(jobject, jni_ToReflectedMethod(JNIEnv *env, jclass cls, jmethodID methodID, jboolean isStatic))
  ...
  methodHandle m (THREAD, ((JNIid*) methodID)->method());
  ...
  if (m->is_initializer()) {
    ...
    reflection_method = Reflection::new_method(m, UseNewReflection, CHECK_0);
  }
...
JNI_ENTRY(void, jni_ExceptionDescribe(JNIEnv *env))  
  ...
  if (thread->has_pending_exception()) {
    ...
    if (ex->is_a(SystemDictionary::threaddeath_klass())) {
      ...
      jio_fprintf(stderr, "Exception ");
      if (thread != NULL && thread->threadObj() != NULL) {
        ResourceMark rm(THREAD);
        jio_fprintf(stderr, "in thread \"%s\" ", thread->get_thread_name());
      }
      if (ex->is_a(SystemDictionary::throwable_klass())) {        
        ...
        ResourceMark rm(THREAD);
        jio_fprintf(stderr, ". Uncaught exception of type %s.", Klass::cast(ex->klass())->external_name());        
      }
    }
  }
JNI_END
...
class JNI_ArgumentPusher : public SignatureIterator {
 protected:
  ...
  JNI_ArgumentPusher(symbolOop signature)
...
class JNI_ArgumentPusherVaArg : public JNI_ArgumentPusher {
 protected:
  ...
  inline void get_bool()   { _arguments->push_int(va_arg(ap, jint)); } // bool is coerced to int when using va_arg
  inline void get_char()   { _arguments->push_int(va_arg(ap, jint)); } // char is coerced to int when using va_arg
  inline void get_short()  { _arguments->push_int(va_arg(ap, jint)); } // short is coerced to int when using va_arg
  inline void get_byte()   { _arguments->push_int(va_arg(ap, jint)); } // byte is coerced to int when using va_arg
  inline void get_int()    { _arguments->push_int(va_arg(ap, jint)); }

  // each of these paths is exercized by the various jck Call[Static,Nonvirtual,][Void,Int,..]Method[A,V,] tests

  inline void get_long()   { _arguments->push_long(va_arg(ap, jlong)); }
  inline void get_float()  { _arguments->push_float((jfloat)va_arg(ap, jdouble)); } // float is coerced to double w/ va_arg
  inline void get_double() { _arguments->push_double(va_arg(ap, jdouble)); }
  inline void get_object() { jobject l =  va_arg(ap, jobject); 
                                 _arguments->push_oop(Handle((oop *)l, false)); }
      
 public:
  JNI_ArgumentPusherVaArg(symbolOop signature, va_list rap)
       : JNI_ArgumentPusher(signature) {
    ap = rap;
  }
  JNI_ArgumentPusherVaArg(jmethodID methodID, va_list rap)
      : JNI_ArgumentPusher(((JNIid*)methodID)->method()->signature()) {
    ap = rap;
  }
    ...
    if ( fingerprint == CONST64(-1) ) SignatureIterator::iterate();// Must be too many arguments
      ...
      while ( 1 ) {
        switch ( fingerprint & parameter_feature_mask ) {
          ...
          case byte_parm:
          case int_parm:
            _arguments->push_int(va_arg(ap, jint)); 
            break;
          case obj_parm: 
            { 
              jobject l =  va_arg(ap, jobject); 
              _arguments->push_oop(Handle((oop *)l, false)); 
            }
            break;
          case long_parm:
            _arguments->push_long(va_arg(ap, jlong)); 
            break;
          case float_parm:
            _arguments->push_float((jfloat)va_arg(ap, jdouble)); 
            break;
          case double_parm:
            _arguments->push_double(va_arg(ap, jdouble)); 
            break;
          case done_parm:
            return;
            break;
...
class JNI_ArgumentPusherArray : public JNI_ArgumentPusher {
 protected:
  ...
  inline void get_bool()   { _arguments->push_int((jint)(ap++)->z); }
  inline void get_char()   { _arguments->push_int((jint)(ap++)->c);  }
  inline void get_short()  { _arguments->push_int((jint)(ap++)->s); }
  inline void get_byte()   { _arguments->push_int((jint)(ap++)->b); }
  inline void get_int()    { _arguments->push_int((ap++)->i); }

  inline void get_long()   { _arguments->push_long((ap++)->j);  }
  inline void get_float()  { _arguments->push_float((ap++)->f); }
  inline void get_double() { _arguments->push_double((ap++)->d);}
  inline void get_object() { _arguments->push_oop(Handle((oop *)(ap++)->l, false)); }
 public:
  JNI_ArgumentPusherArray(symbolOop signature, const jvalue *rap)
       : JNI_ArgumentPusher(signature) {
    ap = rap;
  }
  JNI_ArgumentPusherArray(jmethodID methodID, const jvalue *rap) 
      : JNI_ArgumentPusher(((JNIid*)methodID)->method()->signature()) {
    ap = rap;
  }
  ...
  void iterate( uint64_t fingerprint ) {
    if ( fingerprint == CONST64(-1) ) SignatureIterator::iterate(); // Must be too many arguments
      ...
      while ( 1 ) {
        switch ( fingerprint & parameter_feature_mask ) {
          case bool_parm:
            _arguments->push_int((jint)(ap++)->z);
            break;
          case char_parm:
            _arguments->push_int((jint)(ap++)->c);
            break;
          case short_parm:
            _arguments->push_int((jint)(ap++)->s);
            break;
          case byte_parm:
            _arguments->push_int((jint)(ap++)->b);
            break;
          case int_parm:
            _arguments->push_int((jint)(ap++)->i);
            break;
          case obj_parm:
            _arguments->push_oop(Handle((oop *)(ap++)->l, false)); 
            break;
          case long_parm:
            _arguments->push_long((ap++)->j); 
            break;
          case float_parm:
            _arguments->push_float((ap++)->f);
            break;
          case double_parm:
            _arguments->push_double((ap++)->d);
            break;
          case done_parm:
            return;
            break;
...
static void jni_invoke_static(JNIEnv *env, JavaValue* result, jobject receiver, JNICallType call_type, jmethodID methodID, JNI_ArgumentPusher *args, TRAPS) {
  JNIid *jid = (JNIid *)methodID;
  methodHandle method(THREAD, jid->method());  
  Handle recv(THREAD, JNIHandles::resolve(receiver));
  ...
  result->_type = args->get_ret_type();

  // Invoke the method. Result is returned as oop.
  JavaCalls::call(result, method, &java_args, CHECK);

  // Convert result
  if (result->_type == T_OBJECT || result->_type == T_ARRAY) {
    result->_value.h = JNIHandles::make_local(env, (oop) result->_value.h);
  }
}


static void jni_invoke_nonstatic(JNIEnv *env, JavaValue* result, jobject receiver, JNICallType call_type, jmethodID methodID, JNI_ArgumentPusher *args, TRAPS) {
  JNIid *jid = (JNIid *)methodID;
  methodHandle method(THREAD, jid->method());  
  Handle recv(THREAD, JNIHandles::resolve(receiver));
  ...
  args->push_receiver(recv); // Push jobject handle

  {
    // Protect resolved_method and resolved_receiver fields
    JNIidLocker jlo(jid);

    // See if the method hasn't been resolved yet
    // or it's a different receiver
    if ( (jid->resolved_method() == NULL) || 
        (recv.is_null() || (jid->resolved_receiver() != recv->klass() ))) { 
      if ((Klass::cast(method->method_holder()))->is_interface()) { // interface call
        method = jni_resolve_interface_call(recv, method, CHECK); 
        jid->set_resolved_method( method(), recv->klass() );
      } else if ((call_type == JNI_VIRTUAL) &&    // virtual call and 
        (!method->is_private()) &&                // method is not private and 
        (!(method->name() == vmSymbols::object_initializer_name())) ) {
                                                  // name is not <init>
        method = jni_resolve_virtual_call(recv, method, CHECK);
        jid->set_resolved_method( method(), recv->klass());
      }
    } 
    // If we have a valid resolved method, use it.
    else {
      method = jid->resolved_method();
    }
  }
...
static jmethodID get_method_id(JNIEnv *env, jclass clazz, char *name_str, char *sig, bool is_static, TRAPS) {
  // %%%% This code should probably just call into a method in the LinkResolver
  symbolHandle signature = oopFactory::new_symbol_handle(sig, CHECK_0);
  symbolHandle name;
  if (name_str == NULL) {
    name = vmSymbolHandles::object_initializer_name();
  } else {
    name = oopFactory::new_symbol_handle(name_str, CHECK_0);
  }

  // Throw a NoSuchMethodError exception if we have an instance of a primitive java.lang.Class
  ...
  // Make sure class is linked and initialized before handing id's out to methodOops. 
...
JNI_ENTRY(jmethodID, jni_GetStaticMethodID(JNIEnv *env, jclass clazz, const char *name, const char *sig))
  ...
  return get_method_id(env, clazz, (char*) name, (char*) sig, true, thread);
JNI_END
...
#define DEFINE_CALLMETHOD(ResultType, Result, Tag, unionType) \
...
DEFINE_CALLMETHOD(jboolean, Boolean, T_BOOLEAN, i)
DEFINE_CALLMETHOD(jbyte,    Byte,    T_BYTE,    i)
DEFINE_CALLMETHOD(jchar,    Char,    T_CHAR,    i)
DEFINE_CALLMETHOD(jshort,   Short,   T_SHORT,   i)

DEFINE_CALLMETHOD(jobject,  Object,  T_OBJECT,  h)
DEFINE_CALLMETHOD(jint,     Int,     T_INT,     i)
DEFINE_CALLMETHOD(jlong,    Long,    T_LONG,    l)
DEFINE_CALLMETHOD(jfloat,   Float,   T_FLOAT,   f)
DEFINE_CALLMETHOD(jdouble,  Double,  T_DOUBLE,  d)
...
#define DEFINE_CALLNONVIRTUALMETHOD(ResultType, Result, Tag, unionType) \
\
\
JNI_ENTRY(ResultType, \
...
// the runtime type of subword integral basic types is integer
DEFINE_CALLNONVIRTUALMETHOD(jboolean, Boolean, T_BOOLEAN, i)
DEFINE_CALLNONVIRTUALMETHOD(jbyte,    Byte,    T_BYTE,    i)
DEFINE_CALLNONVIRTUALMETHOD(jchar,    Char,    T_CHAR,    i)
DEFINE_CALLNONVIRTUALMETHOD(jshort,   Short,   T_SHORT,   i)

DEFINE_CALLNONVIRTUALMETHOD(jobject,  Object,  T_OBJECT,  h)
DEFINE_CALLNONVIRTUALMETHOD(jint,     Int,     T_INT,     i)
DEFINE_CALLNONVIRTUALMETHOD(jlong,    Long,    T_LONG,    l)
DEFINE_CALLNONVIRTUALMETHOD(jfloat,   Float,   T_FLOAT,   f)
DEFINE_CALLNONVIRTUALMETHOD(jdouble,  Double,  T_DOUBLE,  d)
...
#define DEFINE_CALLSTATICMETHOD(ResultType, Result, Tag, unionType) \
\
\
JNI_ENTRY(ResultType, \
...
// the runtime type of subword integral basic types is integer
DEFINE_CALLSTATICMETHOD(jboolean, Boolean, T_BOOLEAN, i)
DEFINE_CALLSTATICMETHOD(jbyte,    Byte,    T_BYTE,    i)
DEFINE_CALLSTATICMETHOD(jchar,    Char,    T_CHAR,    i)
DEFINE_CALLSTATICMETHOD(jshort,   Short,   T_SHORT,   i)

DEFINE_CALLSTATICMETHOD(jobject,  Object,  T_OBJECT,  h)
DEFINE_CALLSTATICMETHOD(jint,     Int,     T_INT,     i)
DEFINE_CALLSTATICMETHOD(jlong,    Long,    T_LONG,    l)
DEFINE_CALLSTATICMETHOD(jfloat,   Float,   T_FLOAT,   f)
DEFINE_CALLSTATICMETHOD(jdouble,  Double,  T_DOUBLE,  d)
...
JNI_ENTRY(jfieldID, jni_GetFieldID(JNIEnv *env, jclass clazz, const char *name, const char *sig))
  ...
  symbolHandle fieldname = oopFactory::new_symbol_handle((char*) name, CHECK_0);
  symbolHandle signame = oopFactory::new_symbol_handle((char*) sig, CHECK_0);
  KlassHandle k(THREAD, java_lang_Class::as_klassOop(JNIHandles::resolve_non_null(clazz)));
  // Make sure class is initialized before handing id's out to fields
  Klass::cast(k())->initialize(CHECK_0);

  fieldDescriptor fd;
  if (!Klass::cast(k())->oop_is_instance() || !instanceKlass::cast(k())->jni_find_field(fieldname(), signame(), false, &fd)) {
...
JNI_ENTRY(jobject, jni_GetObjectField(JNIEnv *env, jobject obj, jfieldID fieldID))
  ...
  // Keep JVM/DI addition small and only check enabled flag here.
  // jni_GetField_probe() assumes that is okay to create handles.
  if (jvmdi::enabled()) {
    o = jvmdi::jni_GetField_probe(thread, obj, o, k, fieldID, false);
  }
...
JNI_QUICK_ENTRY(Return, jni_Get##Result##Field(JNIEnv *env, jobject obj, jfieldID fieldID)) \
  ...
  if (jvmdi::enabled()) { \
    o = jvmdi::jni_GetField_probe_nh(thread, obj, o, k, fieldID, false); \
  } \
...
JNI_QUICK_ENTRY(void, jni_SetObjectField(JNIEnv *env, jobject obj, jfieldID fieldID, jobject value))
  ...
  if (jvmdi::enabled()) {
    jvalue field_value;
    field_value.l = value;
    o = jvmdi::jni_SetField_probe_nh(thread, obj, o, k, fieldID, false, 'L', (jvalue *)&field_value);
  }
...
JNI_ENTRY(jobject, jni_ToReflectedField(JNIEnv *env, jclass cls, jfieldID fieldID, jboolean isStatic))
  ...
  if (isStatic) {
    ...
    found = instanceKlass::cast(id->holder())->jni_find_local_field_from_offset(id->offset(), true, &fd);
  } else {
...
JNI_ENTRY(jfieldID, jni_GetStaticFieldID(JNIEnv *env, jclass clazz, const char *name, const char *sig))
  ...
  fieldDescriptor fd;
  if (!Klass::cast(k())->oop_is_instance() || !instanceKlass::cast(k())->jni_find_field(fieldname(), signame(), true, &fd)) {
    THROW_MSG_0(vmSymbols::java_lang_NoSuchFieldError(), (char*) name);
  }
...
JNI_ENTRY(jobject, jni_GetStaticObjectField(JNIEnv *env, jclass clazz, jfieldID fieldID))
  ...
  // jni_GetField_probe() assumes that is okay to create handles.
  if (jvmdi::enabled()) {
    jvmdi::jni_GetField_probe(thread, NULL, NULL, id->holder(), fieldID, true);
  }
...
JNI_ENTRY(void, jni_SetStaticObjectField(JNIEnv *env, jclass clazz, jfieldID fieldID, jobject value))
  ...
  if (jvmdi::enabled()) {
    jvalue field_value;
    field_value.l = value;
    jvmdi::jni_SetField_probe(thread, NULL, NULL, id->holder(), fieldID, true, 'L', (jvalue *)&field_value);
  }
...
// Return an address which will fault if the caller dereferences it.
...
static bool register_native(klassOop k, symbolOop name, symbolOop signature, address entry, TRAPS) {
...
JNI_ENTRY(jint, jni_RegisterNatives(JNIEnv *env, jclass clazz,
  ...
  for (int index = 0; index < nMethods; index++) {
    symbolHandle h_name = oopFactory::new_symbol_handle(methods[index].name, CHECK_0);
    symbolOop    signature = oopFactory::new_symbol(methods[index].signature, CHECK_0);
    bool res = register_native(h_k(), h_name(), signature, (address) methods[index].fnPtr, THREAD);
    if (!res) return -1;
  }
  return 0;
JNI_END
...
static jmethodID directBufferAddressMethod   = NULL; // Can be removed after 1.4 Beta 3 (obsolete)
...
// This is run "inside" the VM to circumvent the security checks done
// by jni_FindClass. (FIXME: not sure whether we in fact need to do
// this.)
...
static bool initializeDirectBufferSupport(JNIEnv* env) {
  ...
  if (atomic::compare_and_exchange(1, &directBufferSupportInitializeStarted, 0) == 0) {
    ...
    // NOTE: obsolete after 1.4 Beta 3
    directBufferAddressMethod   = env->GetMethodID(directBufferClass, "address", "()J");
    bufferCapacityField         = env->GetFieldID(bufferClass, "capacity", "I");

    if ((directByteBufferConstructor == NULL) ||
        (directBufferAddressMethod   == NULL) ||
        (bufferCapacityField         == NULL)) {
      directBufferSupportInitializeFailed = 1;
      return false;
    }
    
    // Try to get directBufferAddressField
    // Can replace directBufferAddressMethod above after Beta 3
    directBufferAddressField = env->GetFieldID(bufferClass, "address", "J");
    if (directBufferAddressField == NULL) {
      // Can remove this error handling code after Beta 3
      env->ExceptionClear();
    }
...
extern "C" void* JNICALL jni_GetDirectBufferAddress(JNIEnv *env, jobject buf)
{
  ...
  if (directBufferAddressField != NULL) {
    ...
    // Can remove this arm after Beta 3
    return (void*) env->CallLongMethod(buf, directBufferAddressMethod);
  }
}
...
jint vm_created = 0;
jint num_vm_created = 0;          // number of VMs created so far
...
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_CreateJavaVM(JavaVM **vm, void **penv, void *args) {
  ...
  if (atomic::exchange(1, &vm_created) == 1) {
    atomic::decrement(&num_vm_created);
    return JNI_ERR;
  }

  assert(vm_created == 1, "vm_created is true during the creation");

  jint result = Threads::create_vm((JavaVMInitArgs*) args);
  if (result == JNI_OK) {
    ...
    *(JNIEnv**)penv = thread->jni_environment();      
    ...
    // Notify jvmdi
    if (jvmdi::enabled()) {
       jvmdi::post_thread_start_event(thread);
    }
    ...
    if (Threads::is_supported_jni_version_including_1_1(version)) {
      ...
      *penv = (void* )jvmpi::GetInterface_1(version);	// version 1.X support
      return JNI_OK; 
               ...
               version == JVMDI_VERSION_1_1 ||
               version == JVMDI_VERSION_1_2 ||
               version == JVMDI_VERSION_1_3) {
      if (jvmdi::enabled()) {
        *penv = (void *)jvmdi::GetInterface_1(vm);
        return JNI_OK;
      } else {
        // JNI_EVERSION means the version is not supported which isn't 
