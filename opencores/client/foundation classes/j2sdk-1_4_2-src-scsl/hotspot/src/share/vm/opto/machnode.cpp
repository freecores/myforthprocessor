...
#pragma ident "@(#)machnode.cpp	1.174 03/02/13 18:50:40 JVM"
#endif
/*
...
const Node* MachNode::get_base_and_disp(intptr_t &offset, const TypePtr* &adr_type) const {
  ...
  if (oper != NULL) {
	...
	const TypeInt* ti = index->bottom_type()->isa_int();
	if (ti == NULL) {
    ...
    if( adr_type == TYPE_PTR_SENTINAL ) {
      ...
      if (t_disp != NULL) {
        ...
        if (t_base->isa_int()) {
          const TypeInt *t_offset = t_base->is_int();
          if( t_offset->is_con() ) {
...
const class TypePtr *MachNode::adr_type() const {
  ...
  if (base == NULL) {
    ...
    if (offset == Type::OffsetBot)  return TypePtr::BOTTOM;
  ...
  if (base == (Node*)-1)  return TypePtr::BOTTOM;
  ...
  if (tp == NULL)         return TypePtr::BOTTOM;
...
bool MachCallNode::returns_float_or_double() const {
  ...
  bool  float_or_double = (return_val != NULL) 
