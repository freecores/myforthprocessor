...
#pragma ident "@(#)codeCache.cpp	1.104 03/01/23 11:59:18 JVM"
#endif
/*
...
// The following version is used by MarkSweep, and is called
// at the end of MarkSweep::phase1.
void CodeCache::do_unloading(bool unloading_occurred,
                             bool& marked_for_unloading) {
  assert_locked_or_safepoint(CodeCache_lock);
  marked_for_unloading = false;
  FOR_ALL_ALIVE_BLOBS(cb) {
    cb->follow_roots_or_mark_for_unloading(unloading_occurred, marked_for_unloading);
  }
}

// The following version is used by CMS
void CodeCache::do_unloading_for_CMS(CMSIsAliveClosure* is_alive,
  ...
  marked_for_unloading = false;
  FOR_ALL_ALIVE_BLOBS(cb) {
    cb->follow_roots_or_mark_for_unloading_for_CMS(
          is_alive, keep_alive,
