...
#pragma ident "@(#)vframe.hpp	1.66 03/01/23 12:26:35 JVM"
#endif
/*
...
//     - deoptimizedVFrame  ; special javaVFrame for deoptimized off stack VFrames
//   - externalVFrame
...
class javaVFrame: public vframe {
 ...
 public:
  ...
  // count number of entries for a lightweight monitor
...
class vframeStream : StackObj {
 private:
  // common 
  frame        _frame;
  JavaThread*  _thread;
  RegisterMap  _reg_map;
  enum { interpreted_mode, compiled_mode, deoptimized_mode, at_end_mode } _mode;
...
#ifndef CORE
  inline bool fill_in_compiled_inlined_sender();
         bool fill_in_deoptimized_inlined_sender();
  inline void fill_from_compiled_frame(nmethod* code, int decode_offset);
  inline void fill_from_compiled_native_frame(nmethod* code);
         void fill_from_deoptimized_frame(vframeArray* array, int index);
#endif

  inline void fill_from_interpreter_frame();
