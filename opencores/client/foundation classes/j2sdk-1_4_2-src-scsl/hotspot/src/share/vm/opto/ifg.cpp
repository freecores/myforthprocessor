...
#pragma ident "@(#)ifg.cpp	1.48 03/03/10 21:47:38 JVM"
#endif
/*
...
int LRG::compute_degree( LRG &l ) const {
  uint8 u8tmp;
  u8tmp =  (_fat_proj || l._fat_proj)        // either is a fat-proj?
    ? (_num_regs * l.num_regs())             // then use product
    : MAX2(_num_regs,(uint8) l.num_regs());  // else use max
  return ( u8tmp );
}
  ...
  for( uint i = 0; i < _cfg._num_blocks; i++ ) {
    ...
    for( j = last_inst + 1; j > 1; j-- ) {
	  ...
	  // hold live ranges.
          ...
          if( n->is_SpillCopy()             
              && lrgs(r)._def != (Node*)-1     // MultiDef live range can still split
