...
#pragma ident "@(#)chaitin.hpp	1.142 03/01/23 12:15:40 JVM"
#endif
/*
...
class PhaseChaitin : public PhaseRegAlloc {
  ...
  bool hi_pressure_before_first_use( Block *b, uint lidx );
  ...
  void pre_split_around_loops( GrowableArray<uint> &lidxs, uint spill_cnt );
  // Helper for pre-split
  bool isolate_candidate( InnerLoop *il, Block *bpre, uint lidx ) const ;

  // Profiling Functions (Temporary)
  //uint Split_Init(uint *lrg2reach, Node ***Reaches, bool **UP, GrowableArray<uint> &lidxs);

  //uint Split_Insert_Phis(uint maxlrg, uint *lrg2reach, Node ***Reaches, bool **UP, GrowableArray<uint> &lidxs, Block *b, Node_List *phis, uint spill_cnt);

  //uint Split_Pass1(uint maxlrg, uint *lrg2reach, Node **Reachblock, bool *UPblock, GrowableArray<uint> &lidxs, Block *b, Node_List *defs);
...
private:
  // Record that this allocation has bailed out.
  void prepare_to_bailout(const char *printed_error_message) const;
  ...
  // Return false when fixup spills could not complete, node limit exceeded.
  bool fixup_spills();
  ...
  bool is_copy_of_callee( Node *def ) const;
