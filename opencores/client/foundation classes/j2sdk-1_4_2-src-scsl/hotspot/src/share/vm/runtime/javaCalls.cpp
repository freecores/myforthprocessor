...
#pragma ident "@(#)javaCalls.cpp	1.196 03/01/23 12:23:21 JVM"
#endif
/*
...
JavaCallWrapper::JavaCallWrapper(methodHandle callee_method, Handle receiver, JavaValue* result, TRAPS) {
  ...
  _is_handling_implicit_exception = _thread->is_handling_implicit_exception();
  ...
  if (_anchor.last_Java_sp() == NULL) {
    ...
    if (jvmdi::enabled()) {
      _in_native_code = jvmdi::is_cur_thread_in_native_code();
...
JavaCallWrapper::~JavaCallWrapper() {
  ...
  _thread->set_is_handling_implicit_exception(_is_handling_implicit_exception);
...
void JavaCalls::call(JavaValue* result, methodHandle method, JavaCallArguments* args, TRAPS) {
  ...
  // Check that there are shadow pages available
...
void JavaCalls::call_helper(JavaValue* result, methodHandle* m, JavaCallArguments* args, TRAPS) {
  ...
  debug_only(args->verify(method, result->_type, thread));

  // Ignore call if method is empty
  if (method->is_empty_method()) {
    assert(result->_type == T_VOID, "an empty method must return a void value");
    return;
  }
  ...
  bool oop_result_flag = (result->_type == T_OBJECT || result->_type == T_ARRAY);  

  // NOTE: if we move the computation of the result_val_address inside
  // the call to call_stub, the optimizer produces wrong code.
  intptr_t* result_val_address = (intptr_t*)&(result->_value);
  ...
  { JavaCallWrapper link(method, receiver, result, CHECK);  
    { HandleMark hm(thread);  // HandleMark used by HandleMarkCleaner
      ...
      if (oop_result_flag) {
        thread->set_vm_result((oop) result->_value.h);
      }
    }
  ...
  if (oop_result_flag) {
    result->_value.h = (jobject)thread->vm_result();
