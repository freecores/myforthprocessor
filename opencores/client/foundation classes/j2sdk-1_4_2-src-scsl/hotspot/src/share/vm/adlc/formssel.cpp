...
#pragma ident "@(#)formssel.cpp	1.150 03/01/23 11:13:15 JVM"
#endif
/*
...
//------------------------------ExpandRule-------------------------------------
ExpandRule::ExpandRule() : _instrs(), _opers(cmpstr, hashstr, Form::arena),
...
void ExpandRule::output(FILE *fp) {         // Write info to output files
  ...
  for( _instrs.reset(); (str = _instrs.iter()) != NULL; ) {
    fprintf(fp,"%s(", str);
    lst = (NameList *)_opers[str];          // Iterate over the operand list
    for( lst->reset(); (str = lst->iter()) != NULL; ) {
      fprintf(fp,"%s ", str);
    }
    fprintf(fp,");\n");
  }
...
int MatchNode::cisc_spill_merge(int left_spillable, int right_spillable) {
  ...
  if( (right_spillable == Maybe_cisc_spillable) && (right_spillable == Maybe_cisc_spillable) ) {
...
int MatchRule::is_ideal_copy() const {
  if( _rChild ) {
    const char  *opType = _rChild->_opType;
    if(    strcmp(opType,"CopyI")==0 || strcmp(opType,"CopyP")==0 
        || strcmp(opType,"CopyD")==0 || strcmp(opType,"CopyF")==0
        || strcmp(opType,"CopyL")==0 || strcmp(opType,"Copy" )==0 )
      return 1;
...
bool MatchRule::is_ideal_call_leaf() const {
  if( !_opType ) return false;
  return !strcmp(_opType,"CallLeaf") || !strcmp(_opType,"CallClearArray");
}
