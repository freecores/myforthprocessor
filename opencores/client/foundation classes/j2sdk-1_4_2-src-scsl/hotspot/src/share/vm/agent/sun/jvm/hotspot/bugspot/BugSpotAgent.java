/*
 * @(#)BugSpotAgent.java	1.5 03/01/23 11:22:40
 *
...
public class BugSpotAgent {
  ...
  private String serverBoundName;
  ...
  /** This attaches to a "debug server" on a remote machine; this
      remote machine has already attached to a process or opened a
      core file and is waiting for RMI calls on the Debugger object to
      come in. */
  public synchronized void attach(String remoteServerName)
    throws DebuggerException {
    if (debugger != null) {
      throw new DebuggerException("Already attached to a process");
    }
    if (remoteServerName == null) {
      throw new DebuggerException("Remote machine name must be specified");
    }
  ...
  private boolean detachInternal() {
    ...
    if (isServer) {
      try {
        Naming.unbind(serverBoundName);
      }
      catch (Exception e) {
        ex = new DebuggerException(e.toString());
      }
      dbg = debugger;
    } else {
  ...
  private void setupDebugger() {
    if (startupMode != REMOTE_MODE) {
      ...
      if (isServer) {
        try {
           RemoteDebuggerServer remote =
             new RemoteDebuggerServer(debugger);
           String localHostName = InetAddress.getLocalHost().getHostName();
           serverBoundName = "//" + localHostName + "/SARemoteDebugger";
           Naming.rebind(serverBoundName, remote);
         }
         catch (Exception e) {
      ...
      //
      // Add more lookup situations here for other OS/CPU combinations
      //

      // Bottom case: error message
      String errorMessage = ("Unable to find a debug server for a known OS/CPU\nconfiguration on machine \"" +
                             remoteServerName + "\"");

      String[] names = null;
      try {
        names = Naming.list(baseURL);
        String suffix = " (servers found: ";
        if ((names == null) || (names.length == 0)) {
          suffix = suffix + "none";
        } else {
          for (int i = 0; i < names.length; i++) {
            suffix = suffix + names[i];
            if (i < names.length - 1) {
              suffix = suffix + ", ";
            }
          }
        ...
        suffix = suffix + ")";
        errorMessage = errorMessage + suffix;
      }
      catch (Exception e) {
      }
  ...
  private boolean setupVM() {
    ...
    try {
      ...
      if (jvmdi.canAttach()) {
        ...
        System.err.println("Attached to Serviceability Agent's JVMDI module.");
        // Jog VM to suspended point with JVMDI module
        resume();
        suspendJava();
        suspend();
        System.err.println("Suspended all Java threads.");
      } else {
        System.err.println("Could not locate SA's JVMDI module; skipping attachment");
        jvmdi = null;
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private void connectRemoteDebugger() throws NotBoundException, MalformedURLException, RemoteException {
    RemoteDebugger remote =
      (RemoteDebugger) Naming.lookup(baseURL + "/SARemoteDebugger");
  ...
  private void setupDebuggerWin32() {
    ...
    if (cpu.equals("x86")) {
      ...
      throw new DebuggerException("Win32 only supported on x86");
    }
