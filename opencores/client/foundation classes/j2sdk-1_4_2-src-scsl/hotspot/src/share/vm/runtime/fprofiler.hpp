...
private:
  ...
  void record_compiled_tick   (frame fr, TickPosition where);
  ...
  ExtendedPC _vm_pc;   // The actual pc where tick hits VM (for +ProfileVM)
...
private:
  ...
  bool record_tick(JavaThread* thread); // result says if we resumed thread
  bool engaged;
