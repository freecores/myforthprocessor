...
#pragma ident "@(#)compactibleFreeListSpace.hpp	1.51 03/01/23 12:07:36 JVM"
#endif
/*
...
class PromotionInfo VALUE_OBJ_CLASS_SPEC {
  ...
  SpoolBlock*     _spoolTail;     // last  spooling block
 ...
 private:
  bool ensureSpoolingSpaceWork();    // ensure that spooling space exists
                                     // return true if there is spooling space
 ...
 public:
  PromotionInfo() :
    ...
    _spareSpool(NULL), _firstIndex(0),
    _nextIndex(0) {}
  ...
  inline bool hasSpoolingSpace() {
    return _spoolTail != NULL && _spoolTail->bufferSize > _nextIndex;
  }
  // ensure that spooling space exists
  bool ensureSpoolingSpace() {
    return hasSpoolingSpace() || ensureSpoolingSpaceWork();
  }
...
class CompactibleFreeListSpace: public CompactibleSpace {
  ...
  enum SomeConstants {
    ...
    IndexSetStart       = (ALIGN_ALL_OBJECTS ? 2 : 1),
    IndexSetStride      = (ALIGN_ALL_OBJECTS ? 2 : 1)
  };
 ...
 public:
  ...
  void printFLCensus(int sweepCt)	  const PRODUCT_RETURN;
...
class CFLS_LAB : public CHeapObj {
  ...
  HeapWord* _max_alloc_block_start;
  HeapWord* _max_alloc_block_end;
...
size_t PromotionInfo::refillSize() const {
  size_t sz = heap_word_size(sizeof(SpoolBlock) + sizeof(markOop) * 256);
