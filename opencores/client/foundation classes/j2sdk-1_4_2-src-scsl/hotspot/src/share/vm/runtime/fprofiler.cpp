...
#pragma ident "@(#)fprofiler.cpp	1.114 03/01/27 08:28:37 JVM"
#endif
/*
...
void FlatProfiler::record_vm_tick() {
  ...
  if( ProfileVM  ) {
    ...
    if(epc.pc() != NULL) {
      name = os::dll_address_to_function_name(epc.pc());
    }
    if (name != NULL) {
...
void FlatProfiler::record_thread_ticks() {
  // Don't want to hang on Threads_lock, 
  // so see if we will get it before trying for it.
  // Yes, there's a small window in between the ask and the grab, 
  // but we are at high priority, and the occassional hang 
  // should be in the noise
  if (Threads_lock->owner() == Mutex::INVALID_THREAD) {
    ...
    for (JavaThread* tp = Threads::first(); tp != NULL; tp = tp->next()) {
      ...
      if (tp->is_Compiler_thread()) {
        // This thread is a compiler thread.  If it is active, record the
        // tick.
      ...
      if (pp != NULL) {
        HandleMark hm;
        tp->vm_suspend();
        bool have_resumed_thread = false;
        pp = tp->get_thread_profiler();  // get profiler again to make sure it's still there
        if (pp != NULL && pp->engaged) {
          // If we get into here, then we are responsible for resuming the thread!!
...
#endif
          {
            have_resumed_thread = pp->record_tick(tp);
          }
        }
        if (!have_resumed_thread) {
	  tp->vm_resume(false);  // don't yield
        }
      }
    }
    if (interval_expired) {
...
void ThreadProfiler::record_compiled_tick(frame fr, TickPosition where) {
#ifndef CORE
  const char *name = NULL;
  ...
  if(ProfileVM){
    if(_vm_pc.pc() != NULL){
      // For more fine-grained profiling, try to see where in the VM the tick hit
      name = os::dll_address_to_function_name(_vm_pc.pc());
    }
  }
  if (method == NULL) {
    if (cb->is_i2c_adapter() || cb->is_c2i_adapter())
      adapter_update(where);
    else if (cb->is_runtime_stub())
      runtime_stub_update(cb, name, where);
    else
      unknown_compiled_update(cb, where);
  }
  else {
    if (method->is_native()) {
      stub_update(method, name, where);
    } else {
      compiled_update(method, where);
    }
  }
#endif
}
...
void ThreadProfiler::record_tick_for_running_frame(JavaThread* thread, frame fr) {
  ...
  if (CodeCache::contains(fr.pc())) {
    ...
    record_compiled_tick(fr, tp_code);
    return;
  } 
...
#ifndef CORE
  if (CodeCache::contains(fr.pc())) {
    record_compiled_tick(fr, tp_native);
    return;
  } 
...
bool ThreadProfiler::record_tick(JavaThread* thread) {
  bool have_resumed_thread = false;  // result says whether we resumed the thread to prevent deadlock.
  FlatProfiler::all_ticks++;
  thread_ticks += 1;
  if(ProfileVM) {
    ExtendedPC addr;   // no-argument constructor portably initializes to NULL
    _vm_pc = addr;
  }
  ...
  case _thread_in_vm_trans:
    if (thread->has_last_Java_frame()) {
      //get frame and then resume thread before poking around in the frame                
      frame fr;
      if (!thread->profile_last_Java_frame(&fr, &_vm_pc)) {
	unknown_ticks_array[ut_no_last_Java_frame] += 1;
	FlatProfiler::unknown_ticks += 1;
      } else {	  
#ifndef CORE
        if (fr.is_c2runtime_frame()) {
          // Very carefully, because this suspended thread might hold the CodeBlob lock,
          // find out the sender (who should be the Java caller of this native code.
	...
	thread->vm_resume(false);  // don't yield
        have_resumed_thread = true;
      }      
    } else {
      // Do we want separate counters for the various unknown cases?
      unknown_ticks_array[ut_no_last_Java_frame] += 1;
      FlatProfiler::unknown_ticks += 1;
    }
    break;
  ...
  case _thread_in_Java_trans:
    {
      frame fr;
      if (!(thread->is_vm_suspended()) || !thread->get_top_frame(&fr, NULL, true)) {
	unknown_ticks_array[ut_no_pc] += 1;
	FlatProfiler::unknown_ticks += 1;
      } else {
