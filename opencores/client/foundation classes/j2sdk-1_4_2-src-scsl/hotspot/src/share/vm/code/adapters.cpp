...
#pragma ident "@(#)adapters.cpp	1.77 03/01/23 11:59:07 JVM"
#endif
/*
...
void BasicAdapter::follow_roots_or_mark_for_unloading(bool unloading_occurred, bool& marked_for_unloading) {
  // Just one oop to preserve
  MarkSweep::follow_root((oop*)&_prototypical_signature);
}
...
: BasicAdapter(cb, sizeof(I2CAdapter), size, frame_size, oop_maps) {}
...
: BasicAdapter(cb, sizeof(C2IAdapter), size, frame_size, oop_maps) {
...
void C2IAdapter::preserve_callee_argument_oops(frame fr, const RegisterMap *reg_map, OopClosure* f) {
  fr.oops_interpreted_arguments_do(prototypical_signature(), is_static_call(), f);
}
