/*
 * @(#)ArrayTypeImpl.java	1.3 02/03/04 
...
public class ArrayTypeImpl extends ReferenceTypeImpl implements ArrayType {
  protected ArrayTypeImpl(VirtualMachine aVm, Klass aRef) { // actually, an ArrayKlass subclass
        super(aVm, aRef);
    }

    public ArrayReference newInstance(int length) {
        throw new UnsupportedOperationException("Not Read Only: ArrayType.newInstance(int)");
    }
     ...
     */
    Type findComponentType(String signature) throws ClassNotLoadedException { //fixme jjh
        byte tag = (byte)signature.charAt(0); 
        System.out.println("jjh: ArrayTypeImpl.findComponentType: char0 = " + signature.charAt(0));

        if (1 == 1) { //fixme jjh: if (PacketStream.isObjectTag(tag)) {
    ...
    public Type componentType() throws ClassNotLoadedException { //fixme jjh
