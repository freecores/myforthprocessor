...
#pragma ident "@(#)psMarkSweepDecorator.cpp	1.9 03/01/23 12:02:24 JVM"
#endif
/*
...
void PSMarkSweepDecorator::precompact() {
  ...
  while (q < t) {                                                            
    ...
    if (oop(q)->is_gc_marked()) {  
      /* prefetch beyond q */                                                
      atomic::prefetch_write(q, interval);                          
      ...
      do {                                                                   
        /* prefetch beyond end */                                            
        atomic::prefetch_write(end, interval);                            
	end += oop(end)->size();
...
void PSMarkSweepDecorator::compact(bool mangle_free_space ) {
  ...
  while (q < t) {
    if (!oop(q)->is_gc_marked()) {
      ...
      // prefetch beyond q
      atomic::prefetch_read(q, scan_interval);
      ...
      atomic::prefetch_write(compaction_top, copy_interval);

      // copy object and reinit its mark
      debug_only(MarkSweep::live_oop_moved_to(q, size, compaction_top));
      assert(q != compaction_top, "everything in this pass should be moving");
      Memory::copy_words_aligned_overlapping(q, compaction_top, size);
