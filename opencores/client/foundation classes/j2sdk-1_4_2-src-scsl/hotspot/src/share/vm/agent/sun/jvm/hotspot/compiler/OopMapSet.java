/*
 * @(#)OopMapSet.java	1.7 03/01/23 11:24:46
 *
...
public class OopMapSet extends VMObject {
  ...
  /** Returns last OopMap in that is anchored before pc.
      If precise is false than the map must be at >= pc_offset.
      If precise is true than map must be at == pc_offset. */
  public OopMap findMapAtOffset(long pcOffset, boolean atCall, boolean precise, boolean debugging) {
    int i;
    int len = (int) getSize();
    if (Assert.ASSERTS_ENABLED) {
    ...
    if (!debugging) {
      if (Assert.ASSERTS_ENABLED) {
        ...
        Assert.that(!precise || getMapAt(i).getOffset() == pcOffset, "oopmap not found");
        Assert.that(precise  || getMapAt(i).getOffset() >= pcOffset, "oopmap not found");
      }
    } else {
      if (i == len) {
    ...
    OopMap m = getMapAt(i);
    if (m.isAtCall() != atCall) {
      i += 1; // It must be the next
      if (Assert.ASSERTS_ENABLED) {
        Assert.that(i < len, "oopmap not found");
      }
      m = getMapAt(i);
      if (!debugging) {
        if (Assert.ASSERTS_ENABLED) {
          Assert.that(m.isAtCall() == atCall,         "wrong at_call state found in oopmap");
          Assert.that(m.getOffset() == pcOffset, "wrong offset found in oopmap");
        }
      } else {
        if ((m.isAtCall() != atCall) || (m.getOffset() != pcOffset)) {
          // Might as well hand the first one back
          m = getMapAt(i-1);
        }
      }
    }
  ...
  public static void allDo(Frame fr, CodeBlob cb, RegisterMap regMap, OopMapVisitor visitor, boolean debugging) {
    ...
    OopMap map = cb.getOopMapForReturnAddress(fr.getPC(), regMap.isPCAtCall(fr.getSP()), debugging);
    if (Assert.ASSERTS_ENABLED) {
  ...
  public static void updateRegisterMap(Frame fr, CodeBlob cb, RegisterMap regMap, boolean debugging) {
    ...
    if (Assert.ASSERTS_ENABLED) {
      if (VM.getVM().isServerCompiler()) {
        Assert.that(!cb.isC2IAdapter()  || nofCallee == SAVED_ON_ENTRY_REG_COUNT, "C2I must save all");
