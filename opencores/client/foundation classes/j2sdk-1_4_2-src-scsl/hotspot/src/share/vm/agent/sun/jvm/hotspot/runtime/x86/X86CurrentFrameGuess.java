/*
 * @(#)X86CurrentFrameGuess.java	1.3 03/01/23 11:46:46
 *
    ...
    if (vm.isJavaPCDbg(pc)) {
      if (vm.isClientCompiler()) {
        ...
        if (vm.getInterpreter().contains(pc)) {
          if (DEBUG) {
            System.err.println("CurrentFrameGuess: choosing interpreter frame: sp = " +
                               sp + ", fp = " + fp + ", pc = " + pc);
          }
          setValues(sp, fp, pc);
          return true;
          ...
          try {
            ...
            while (frame != null) {
              if (frame.isEntryFrame() && frame.entryFrameIsFirst()) {
                ...
                if (DEBUG) {
                  System.err.println("CurrentFrameGuess: Choosing sp = " + curSP + ", pc = " + pc);
                }
            ...
            if (DEBUG) {
              System.err.println("CurrentFrameGuess: Exception " + e + " at offset " + offset);
            }
            // Bad SP. Try another.
          }
        }
      ...
      if (DEBUG) {
        System.err.println("CurrentFrameGuess: choosing last Java frame: sp = " +
