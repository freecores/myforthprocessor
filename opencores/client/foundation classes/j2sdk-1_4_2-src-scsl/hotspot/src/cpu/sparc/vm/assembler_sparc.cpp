...
#pragma ident "@(#)assembler_sparc.cpp	1.167 03/01/23 10:59:04 JVM"
#endif
/*
...
void MacroAssembler::set_last_Java_frame(Register last_java_sp, Register post_java_state) {
  assert_not_delayed();
  // Always set post_Java_state first because once last_Java_sp is visible
  // has_last_Java_frame is true and users will look at the rest of the fields.
  st_ptr(post_java_state, Address(G2_thread, 0, in_bytes(JavaThread::post_Java_state_offset_in_bytes())));
#ifdef _LP64
#ifdef ASSERT
...
void MacroAssembler::reset_last_Java_frame() {
  ...
  Address pj_addr(G2_thread, 0, in_bytes(JavaThread::post_Java_state_offset_in_bytes()));
  ...
  bool            check_exceptions)
{
  ...
  set_last_Java_frame(last_java_sp, G0);
