...
#pragma ident "@(#)assembler_i486.cpp	1.180 03/03/04 17:48:53 JVM"
#endif
/*
...
void Assembler::membar() {
  // if the chip doesn't support the "mfence" or "cpuid"
  // instructions, it's a 486 which doesn't require any
  // memory serialization.
  if( VM_Version::supports_sse2() ) {
    emit_byte( 0x0F );		// MFENCE; faster blows no regs
    emit_byte( 0xAE );
    emit_byte( 0xF0 );
  } else if (VM_Version::supports_cpuid()) {
    pushfd();                // Save FP registers on stack
    lock();                  
    addl(Address(esp), 0);   // Assert the lock# signal here
    popfd();                 // Restore the FP registers
  }
}
