...
#pragma ident "@(#)c1_LIRAssembler_i486.cpp	1.99 03/01/23 10:52:55 JVM"
#endif
/*
...
void LIR_Assembler::emit_osr_entry(IRScope* scope, int number_of_locks, Label* continuation, int osr_bci) { 
  ...
  // ebp: interpreter frame pointer
  // esp: interpreter stack pointer
  //
  // keep old ebp around if we have locks to copy
  ...
  __ subl(esp, m->arg_size() * BytesPerWord);                        // space for arguments
  __ pushl((int)Runtime1::entry_for(Runtime1::osr_frame_return_id)); // osr adapter frame return address
  ...
  { assert(frame::interpreter_frame_monitor_size() == BasicObjectLock::size(), "adjust code below");
    ...
    for (int i = 0; i < number_of_locks; i++) {
      int slot_offset = frame::interpreter_frame_monitor_block_bottom_offset - (i+1) * BasicObjectLock::size();
#ifdef ASSERT
      { Label L;
        __ cmpl(Address(ebx, slot_offset * BytesPerWord + BasicObjectLock::obj_offset_in_bytes()), (int)NULL);
        __ jcc(Assembler::notZero, L);
        __ stop("locked object is NULL");
        __ bind(L);
      }
#endif
      __ movl(eax, Address(ebx, slot_offset * BytesPerWord + BasicObjectLock::lock_offset_in_bytes()));
      __ movl(frame_map()->address_for_monitor_lock_index(i), eax);
      __ movl(eax, Address(ebx, slot_offset * BytesPerWord + BasicObjectLock::obj_offset_in_bytes()));
      __ movl(frame_map()->address_for_monitor_object_index(i), eax);
    }
  }
...
void LIR_Assembler::emit_method_entry(LIR_Emitter* emit, IRScope* scope) {
  ...
  // Note: C1breakpoint should stop only when method is entered
  if (m->is_native() && needs_icache(m)) {
    __ align(AlignEntryCode);
    // Add nops to misalign entry...
    __ nop();
    __ nop();
  }
  ...
  if (needs_icache(m)) {
    ...
    // as the final method may be one of many methods in the hierarchy
    check_icache();
  }

  // verified entry point
  // Do not align vep for non-finals as we mostly enter through ep anyway
  if (m->is_final_method() || m->is_native()) {
    __ align(AlignEntryCode);
  }
...
void LIR_Assembler::emit_native_call(address native_entry, CodeEmitInfo* info) {
  ...
  __ movl(Address(thread, JavaThread::last_native_pc_offset()), t);  
...
void LIR_Assembler::emit_native_method_exit(CodeEmitInfo* info) {
  ...
  { Label Continue;
    ...
    // x86 never needs a deopt_suspend since it has no volatile frame state to synchronize
    ...
    // Note: result object is upacked her
    BasicType type = m->return_type()->basic_type();
    bool result_is_oop = type == T_OBJECT || type == T_ARRAY;
    if (result_is_oop) {
  ...
  return_op(norinfo, return_type == T_OBJECT || return_type == T_ARRAY);
...
void LIR_Assembler::return_op(RInfo result, bool result_is_oop) {
  ...
  __ method_exit(true);  // pop frame at exit
...
void LIR_Assembler::reg2reg(RInfo from_reg, RInfo to_reg) {
  if (to_reg.is_float_kind()) {
    assert(from_reg.is_float_kind(), "must match");
    fpu_on_tos(from_reg);
    fpu_pop(from_reg);
    fpu_push(to_reg);
...
void LIR_Assembler::emit_opTypeCheck(LIR_OpTypeCheck* op) {
  ...
  if (code == lir_store_check) {
    ...
    add_debug_info_here(op->info_for_exception());
...
//  assert(right->destroys_register(), "check");
  ...
  if (right->is_constant()) {
    ...
    add_debug_info(idivl_offset, info, false);
    if (code == lir_irem) {
...
void LIR_Assembler::emit_arraycopy(LIR_OpArrayCopy* op) {
  ...
  Register tmp = op->tmp()->rinfo().as_register();
  Address src_length_addr = Address(src, arrayOopDesc::length_offset_in_bytes());
  Address dst_length_addr = Address(dst, arrayOopDesc::length_offset_in_bytes());

  Label slow_path_with_push, slow_path_no_push, done, fast_prolog;

  //------------------pre-checks-------------------------
  // test for NULL
  __ testl(src, src);
  __ jcc(Assembler::zero, slow_path_with_push);
  __ testl(dst, dst);
  __ jcc(Assembler::zero, slow_path_with_push);

  // check if negative
  __ cmpl(length, 0);
  __ jcc(Assembler::less, slow_path_with_push);
  __ cmpl(src_pos, 0);
  __ jcc(Assembler::less, slow_path_with_push);
  __ cmpl(dst_pos, 0);
  __ jcc(Assembler::less, slow_path_with_push);
  ...
  __ jcc(Assembler::above, slow_path_with_push);
  ...
  // preserve arguments for slow store
  /*
  __ pushl(src);
  __ pushl(src_pos);
  __ pushl(dst);
  ...
  */
  ...
  __ cmpl(eax, 0);
  __ jcc(Assembler::greaterEqual, fast_prolog); // positive return value means "operation-succeded"
  __ jmp(slow_path_no_push);
  __ bind(slow_path_with_push);
  ...
  __ bind(fast_prolog);
  // Do not remove parameters as they have not modified the SP
  // __ addl(esp, 5*BytesPerWord); // remove unused slow path parameters
  __ bind(done);
  
  // may add inlined character-copy later, or when provided with a type, the
  // proper algorithm.
}
