...
// match Sparc which can not call fetch_top_frame 
...
bool JavaThread::profile_last_Java_frame(frame* _fr, ExtendedPC* _addr) {
  // Fine-grained profiling of the VM code is currently not implemented on i486
  // and *_addr is not set. It will eventually be set to the exact native PC if the
  // tick lands inside the VM code.
  if (_fr != NULL) {
    *_fr = last_frame();
    return true;
  } 
...
frame frame::sender_for_deoptimized_frame(RegisterMap* map, CodeBlob* cb) const {
  guarantee(map != NULL, "map must be present");
  JavaThread *thread = map->thread();
  guarantee(thread != NULL, "thread not set");
  vframeArray* vf = thread->vframe_array_for(this);
  jint* sender_sp;

  // If the vframe is not found then we are in the midst of unpacking for this
  // frame and the deopt_frame is really just the stub frame and not the original
  // compiled (but deopt'd) frame. It would be better to compare the pc to the
  // value we know is the call to unpack_frames. NEEDS_CLEANUP
  //
  if ( vf != NULL) {
    if (map->update_map()) {
      vf->update_register_map(map);
    }      
    sender_sp = sp() + vf->frame_size();
  } else {
    assert(!map->update_map(), "not supported");
    sender_sp = sp() + cb->frame_size();
  }
#ifdef COMPILER1
  NEEDS_CLEANUP // check if link() is correct
  // C1 needs to build a complete frame, including _fp.  C2 seems to stop
  // iterating over stack frames when a deoptimized frame is reached.
  return frame(sender_sp, link(), (address) *(sender_sp - 1));
#endif 
#ifdef COMPILER2
  // See note above about vf == NULL
  // fp can be null during deopt before skeletal frames are setup
  if (vf != NULL || fp() == NULL) return frame(sender_sp, NULL, (address) *(sender_sp - 1));
  else return frame(sender_sp, link(), (address) *(sender_sp - 1));
#endif
}
#endif
...
// Adjust index for the 2 words containing old_fp and ret_adr
...
void frame::set_sender_pc_for_unpack(frame caller, int exec_mode) {
  ...
  case Deoptimization::Unpack_exception: {
    ...
    jint* return_address = sp() + deopt_blob->frame_size() - 1;
    *return_address = (jint) caller.pc();

    NEEDS_CLEANUP;
    int saved_ebp_offset = 9;
    jint *fp = sp() + saved_ebp_offset;
    *fp = (jint) caller.fp();
  }
  break;
  default:
...
void frame::patch_for_deoptimization(JavaThread* thread, frame callee, address deopt_handler) {
  if (callee.is_interpreted_frame() || callee.is_osr_adapter_frame()) {
...
void frame::set_sender_pc_for_unpack(frame caller, int exec_mode) {
  ...
  case Deoptimization::Unpack_deopt:
  case Deoptimization::Unpack_exception: {
    jint* return_address = sp() + OptoRuntime::deoptimization_blob()->frame_size() - 1;
    *return_address = (jint) caller.pc();
    // Stuff the returned frame pointer into the uncommon_trap frame.
    // Helps PSF do stack crawls.
    assert( OptoRuntime::deoptimization_blob()->link_offset() >= 0, "" );
    jint *fp = sp() + OptoRuntime::deoptimization_blob()->link_offset();
    *fp = (jint) caller.fp();
  } 
  break;
  case Deoptimization::Unpack_uncommon_trap: {
    jint* return_address = sp() + OptoRuntime::uncommon_trap_blob()->frame_size() - 1;
    *return_address = (jint) caller.pc();
    // Stuff the returned frame pointer into the uncommon_trap frame.
    // Helps PSF do stack crawls.
    assert( OptoRuntime::uncommon_trap_blob()->link_offset() >= 0, "" );
    jint *fp = sp() + OptoRuntime::uncommon_trap_blob()->link_offset();
    *fp = (jint) caller.fp();
  } 
  break;
  default:
