...
#pragma ident "@(#)templateTable_i486.cpp	1.268 03/05/16 10:53:07 JVM"
#endif
/*
...
void TemplateTable::div0_check(Register reg) {
  if (GenerateInterpreterDiv0Checks) {
...
void TemplateTable::null_check(Register obj, int offset) {
  __ null_check(obj, offset);
}
...
void TemplateTable::patch_bytecode(Register bc, Register scratch) {
  ...
  __ cmpl(scratch, Bytecodes::_getfield);
...
void TemplateTable::ldc(bool wide) {
  transition(vtos, vtos);
  Label retry, resolved, notFloat, Done;
  __ bind(retry);
  if (wide) {
...
void TemplateTable::locals_index(Register reg) {
  __ load_unsigned_byte(reg, at_bcp(1));
  __ negl(reg);	
}
...
void TemplateTable::aload_0() {
  ...
  //
  // occur frequently. If RewriteFrequentPairs is set, the (slow) _aload_0 bytecode checks
  // if the next bytecode is either _fast_igetfield or _fast_agetfield and then rewrites the
  // current bytecode into a pair bytecode; otherwise it rewrites the current bytecode into
  // _fast_aload_0 that doesn't do the pair check anymore.
  //
  // Note: If the next bytecode is _getfield, the rewrite must be delayed, otherwise we
  //       may miss an opportunity for a pair.
...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
  ...
  if (is_jsr) {
    ...
    __ leal(eax, at_bcp((is_wide ? 5 : 3) - in_bytes(methodOopDesc::codes_offset())));
    __ subl(eax, ecx);
  ...
  if (UseLoopCounter) {
    ...
    if (UseOnStackReplacement) {
      ...
      // ebx: target bytecode
      // edx: NULL (C1) or osr adapter frame return address (C2)
      // edi: locals pointer
      // esi: bcp
...
#else
      // COMPILER1
      if (UseFixedFrameSize) {
        // get parameter size
        __ get_method(ecx);
        __ load_unsigned_word(ecx, Address(ecx, methodOopDesc::size_of_parameters_offset()));
        __ negl(ecx);                                       // so we can subtract in next step
        __ leal(ecx, Address(edi, ecx, Address::times_4, wordSize));  // adjust esp so that it points to last argument
        __ movl(Address(ebp, frame::interpreter_frame_sender_sp_offset * BytesPerWord), ecx);
      }
      // nmethod to EDX to free up EAX
      __ movl(edx,eax);
#endif // COMPILER2
      // continue at osr code
      // eax: locks pointer for C2
      // ebx: target bytecode for C1 or dead for C2
      // edx: osr nmethod
      // edi: locals pointer for C2
      // esi: bcp
...
void TemplateTable::ret() {
  ...
  __ get_method(eax);
  __ leal(esi, Address(eax, ebx, Address::times_1, methodOopDesc::codes_offset()));
...
void TemplateTable::getfield_or_static(int byte_no, bool is_static) {
  ...
  // do the JVM/DI work here to avoid disturbing the register state below
  if (jvmdi::enabled()) {
    // Check to see if a field access watch has been set before we take
    // the time to call into the VM.
    Label L1;
    __ movl(ecx, Address((int)jvmdi::get_field_access_count_addr(), relocInfo::none));
    __ testl(ecx,ecx);
  ...
  if (!is_static && RewriteBytecodes) {
    __ movl(ecx, Bytecodes::_fast_igetfield);
    patch_bytecode(ecx, ebx);
  }
  __ jmp(Done);
  ...
  __ push(atos);
  if (!is_static && RewriteBytecodes) {
    __ movl(ecx, Bytecodes::_fast_agetfield);
    patch_bytecode(ecx, ebx);
  }
  __ jmp(Done);
...
void TemplateTable::putfield_or_static(int byte_no, bool is_static) {
  ...
  if (jvmdi::enabled()) {
    ...
    // the time to call into the VM.
    Label L1;
    __ movl(ecx, Address((int)jvmdi::get_field_modification_count_addr(), relocInfo::none));
    __ testl(ecx, ecx);
    __ jcc(Assembler::zero, L1);
...
void TemplateTable::fast_accessfield(TosState state) {
  ...
  __ verify_oop(eax);
  null_check(eax, 0);
  lo = Address(eax, ebx, Address::times_1, 0*wordSize);
  hi = Address(eax, ebx, Address::times_1, 1*wordSize);

  // access field
  switch (state) {
    case itos: transition(atos, itos); __ movl(eax, lo);  break;
    case atos: transition(atos, atos); __ movl(eax, lo); __ verify_oop(eax); break;
    default:
      ShouldNotReachHere();
  }
  // Doug Lea believes this is not needed with current Sparcs(TSO) and Intel(PSO)
...
void TemplateTable::fast_xaccess(TosState state) {
  ...
  __ incl(esi);
  __ null_check(eax, 0);
  __ movl(eax, Address(eax, ebx, Address::times_1));
  __ decl(esi);
}
...
void TemplateTable::invokeinterface(int byte_no) {
  ...
  __ call_VM(noreg, CAST_FROM_FN_PTR(address,
                   InterpreterRuntime::throw_incompatible_class_change_error));
...
void TemplateTable::_new() {
  ...
  // get instance_size in instanceKlass (already aligned if ALIGN_ALL_OBJECTS
  // is set)
  ...
  if (ALIGN_ALL_OBJECTS) {
    ...
    // edx could have been odd
    Label even;
    __ jcc(Assembler::carryClear, even);
    // Clear topmost word (no jump needed if conditional assignment would
    // work here).
    __ movl(Address(eax, edx, Address::times_8, sizeof(oopDesc) - 0*oopSize), ecx);
    __ bind(even);
    // edx could be 0, need to check again
    __ jcc(Assembler::zero, initialize_header);
  }
...
void TemplateTable::arraylength() {
  ...
  null_check(eax, arrayOopDesc::length_offset_in_bytes());
