...
#pragma ident "@(#)nativeInst_i486.cpp	1.60 03/03/12 22:14:55 JVM"
#endif
/*
...
void NativeCall::insert(address code_pos, address entry) {
  ...
  *((long *)(code_pos+1)) = (int)entry - ((int)code_pos + 1 + sizeof(long));
  ...
  ICache::invalidate_range(code_pos, NativeCall::instruction_size);
}
...
void NativeJump::patch_verified_entry(address entry, address verified_entry, address dest) {
  ...
  *((jint *)(tmp)) = (intptr_t)dest - ((intptr_t)(verified_entry + 1) + sizeof(int));
...
#ifndef CORE
  // These alignment restrictions are always true for c2, for c1 until it
  // deoptimizes it is only enforced if threads are still running.
#ifndef COMPILER2
  if (!SafepointSynchronize::is_at_safepoint()) {
      ...
      // Must be in same cache line of entry
      guarantee (((uintptr_t)verified_entry & ~31) == ((uintptr_t)entry & ~31), "illegal address for code patching(2)");
      // First 5 bytes must be within the same cache line - 4827828
      assert ((uintptr_t)verified_entry / 32 == ((uintptr_t)verified_entry + 4) / 32, "illegal address for code patching(3)");
#ifndef COMPILER2
  }
#endif
#endif /* CORE */
