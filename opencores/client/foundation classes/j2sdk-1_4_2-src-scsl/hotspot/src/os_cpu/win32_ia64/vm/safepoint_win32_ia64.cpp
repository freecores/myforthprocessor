...
#pragma ident "@(#)safepoint_win32_ia64.cpp	1.11 03/01/23 11:11:35 JVM"
#endif
...
bool SafepointSynchronize::can_be_at_safepoint_before_suspend(JavaThread *thread, JavaThreadState state) {
  switch(state) {
   // Native and blocked threads should have already saved state
...
// We do not need code generated to check for a deopt suspend because top frame
// has volatile state since the jni trampoline was instituted
...
bool SafepointSynchronize::needs_deopt_suspend(JavaThread *thread) {
  ...
  // JNI trampoline removes the need to return true here.
  return false;
