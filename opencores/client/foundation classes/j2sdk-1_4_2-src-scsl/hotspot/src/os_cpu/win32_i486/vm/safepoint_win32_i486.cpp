...
#pragma ident "@(#)safepoint_win32_i486.cpp	1.6 03/01/23 11:11:03 JVM"
#endif
/*
...
bool SafepointSynchronize::can_be_at_safepoint_after_suspend(JavaThread *thread, JavaThreadState state) {
  assert(thread->thread_state() == _thread_in_vm_trans, "thread in wrong state");
  return false;
}


void SafepointSynchronize::save_thread_state_at_safepoint(JavaThread *thread) {
  // no state to save on Win32
}

// We never need code generated to check for a deopt suspend because top frame
// has no volatile state
