...
#pragma ident "@(#)objectMonitor_linux.cpp	1.39 03/05/09 11:46:36 JVM"
#endif
/*
    ...
    if (!THREAD->is_Java_thread()) {
      ...
      do {
        ...
        if (threadIsSuspended) {
          ...
          // bug. The ObjectMonitor.wait() rentry case is ruled out by
          // checking for _thread_in_native.
          //
          // assert(((JavaThread *)THREAD)->thread_state() != _thread_in_native ||
          //   osthread->suspend_action() == SR_SUSPENDED, "just checking");
  
          //
          // This logic isn't needed for JVM/DI or JVM/PI raw monitors on
          // Solaris or Win32. However, on Linux it is needed because we
          // can be granted a raw monitor while suspended. This logic is
          // also needed for the ObjectMonitor.wait() reentry phase.
