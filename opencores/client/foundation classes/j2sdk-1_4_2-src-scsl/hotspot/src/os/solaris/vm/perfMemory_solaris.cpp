...
#pragma ident "@(#)perfMemory_solaris.cpp	1.15 03/04/17 05:14:40 JVM"
#endif
/*
...
static void save_memory_to_file(char* addr, size_t size) {
  ...
  snprintf(destfile, nbytes, "%s_%d", PERFDATA_NAME, os::current_process_id());
...
static bool make_user_tmp_dir(const char* dirname) {
  ...
  if (mkdir(dirname, S_IRWXU|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH) == OS_ERR) {
    if (errno == EEXIST) {
      ...
      //
      if (PrintMiscellaneous && Verbose) {
        jio_fprintf(stderr, "could not create directory %s: %s\n",
...
static int create_sharedmem_resources(const char* dirname, const char* filename, size_t size) {
  ...
  if (result == OS_ERR) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "could not create file %s: %s\n",
  ...
  if (result == OS_ERR) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "could not set shared memory file size: %s\n",
                          strerror(errno));
    }
    RESTARTABLE(::close(fd), result);
    return -1;
...
static char* mmap_create_shared(size_t size) {
  ...
  if (mapAddress == MAP_FAILED) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "mmap failed -  %s\n", strerror(errno));
    }
...
static size_t sharedmem_filesize(int fd, TRAPS) {
  ...
  if (result == OS_ERR) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "fstat failed: %s\n", strerror(errno));
    }
