...
#pragma ident "@(#)perfMemory_win32.cpp	1.17 03/02/27 17:49:07 JVM"
#endif
/*
...
static char* create_standard_memory(size_t size) {
  ...
  if (!os::commit_memory(mapAddress, size)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "Could not commit PerfData memory\n");
    }
...
static void save_memory_to_file(char* addr, size_t size) {
  ...
  size_t nbytes = strlen(PERFDATA_NAME) + UINT_CHARS + 1;

  char* destfile = NEW_C_HEAP_ARRAY(char, nbytes);

  // create the name of the file for retaining the instrumentation memory
  _snprintf(destfile, nbytes, "%s_%d", PERFDATA_NAME, os::current_process_id());

  int fd = ::_open(destfile, _O_BINARY | _O_CREAT | _O_WRONLY | _O_TRUNC, _S_IREAD|_S_IWRITE);
  if (fd == OS_ERR) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "Could not create Perfdata save file: %s: %s\n",
  ...
  for (size_t remaining = size; remaining > 0;) {
    ...
    if (nbytes == OS_ERR) {
      if (PrintMiscellaneous && Verbose) {
        jio_fprintf(stderr, "Could not write Perfdata save file: %s: %s\n",
                            destfile, strerror(errno));
      }
      FREE_C_HEAP_ARRAY(char, destfile);
      return;
...
static bool is_directory_secure(const char* path) {
  ...
  if (fa == 0xFFFFFFFF) {
    ...
    if (lasterror == ERROR_FILE_NOT_FOUND) {
      ...
      if (PrintMiscellaneous && Verbose) {
        jio_fprintf(stderr, "could not get attributes for file %s: ",
                            " lasterror = %d\n", path, lasterror);
      }
      return false;
    }
  ...
  if (fa & FILE_ATTRIBUTE_REPARSE_POINT) {
    ...
    //
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "%s is a reparse point\n", path);
    }
    return false;
  }
  ...
  if (fa & FILE_ATTRIBUTE_DIRECTORY) {
    ...
    //
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "%s is not a directory, file attributes = "
                    INTPTR_FORMAT "\n", path, fa);
    }
    return false;
  }
...
static void remove_file(const char* dirname, const char* filename) {
  ...
  if (::unlink(path) == OS_ERR) {
    if (PrintMiscellaneous && Verbose) {
      if (errno != ENOENT) {
        jio_fprintf(stderr, "Could not unlink shared memory backing"
                            " store file %s : %s\n", path, strerror(errno));
      }
    }
  }
...
static bool is_alive(int pid) {
  ...
  if (ph == NULL) {
    ...
    if (PrintMiscellaneous && Verbose) {
      ...
      if (lastError != ERROR_INVALID_PARAMETER) {
        jio_fprintf(stderr, "OpenProcess failed: %d\n", GetLastError());
      }
    }
    return false;
  }
  ...
  if (!GetExitCodeProcess(ph, &exit_status)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "GetExitCodeProcess failed: %d\n", GetLastError());
    }
    CloseHandle(ph);
    return false;
  }
...
static bool is_filesystem_secure(const char* path) {
  ...
  if (PerfBypassFileSystemCheck) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "bypassing file system criteria checks for %s\n",
                           path);
    }
    return true;
  }
  ...
  if (first_colon == NULL) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "expected device specifier in path: %s\n", path);
    }
    return false;
  }
  ...
  if (!GetVolumeInformation(root_path, NULL, 0, NULL, &maxpath,
    ...
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "could not get device information for %s: "
                          " path = %s: lasterror = %d\n",
  ...
  if ((flags & FS_PERSISTENT_ACLS) == 0) {
    ...
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "file system type %s on device %s does not support"
                          " ACLs\n", fs_type, root_path);
    }
    return false;
  }
  ...
  if ((flags & FS_VOL_IS_COMPRESSED) != 0) {
    ...
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "file system type %s on device %s is compressed\n",
                           fs_type, root_path);
    }
    return false;
  }
...
static HANDLE create_file_mapping(const char* name, HANDLE fh, LPSECURITY_ATTRIBUTES fsa, size_t size) {
  ...
  if (fmh == NULL) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "CreateFileMapping failed, lasterror = %d\n",
                          GetLastError());
    }
    return NULL;
  }
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS) {
    ...
    //
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "file mapping already exists, lasterror = %d\n",
                          GetLastError());
    }
...
static PSID get_user_sid(HANDLE hProcess) {
  ...
  if (!OpenProcessToken(hProcess, TOKEN_READ, &hAccessToken)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "OpenProcessToken failure: lasterror = %d \n",
                          GetLastError());
    }
    return NULL;
  }
  ...
  if (!GetTokenInformation(hAccessToken, TokenUser, NULL, rsize, &rsize)) {
    ...
    if (lasterror != ERROR_INSUFFICIENT_BUFFER) {
      if (PrintMiscellaneous && Verbose) {
        jio_fprintf(stderr, "GetTokenInformation failure: lasterror = %d,"
                            " rsize = %d\n", lasterror, rsize);
      }
...
static bool add_allow_ace(PSECURITY_DESCRIPTOR pSD, PSID pSID, DWORD mask) {
  ...
  if (!GetSecurityDescriptorDacl(pSD, &exists, &pSDACL, &isdefault)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "GetSecurityDescriptor failure: lasterror = %d \n",
                          GetLastError());
    }
    return false;
  }
  ...
  if (pSDACL == NULL || (exists && isdefault)) {
    ...
    if (!InitializeAcl(pACL, ACL_SIZE, ACL_REVISION)) {
      if (PrintMiscellaneous && Verbose) {
        jio_fprintf(stderr, "InitializeAcl failure: lasterror = %d \n",
                            GetLastError());
      }
      FREE_C_HEAP_ARRAY(char, pACL);
      return false;
  ...
  if (!AddAccessAllowedAce(pACL, ACL_REVISION, mask, pSID)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "AddAccessAllowedAce failure: lasterror = %d \n",
                          GetLastError());
    }
  ...
  if (!SetSecurityDescriptorDacl(pSD, TRUE, pACL, FALSE)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "SetSecurityDescriptorDacl failure:"
                          " lasterror = %d \n", GetLastError());
    }
    if (pACL != pSDACL) FREE_C_HEAP_ARRAY(char, pACL);
    return false;
...
static LPSECURITY_ATTRIBUTES make_security_attr(ace_data_t aces[], int count) {
  ...
  if (!InitializeSecurityDescriptor(pSD, SECURITY_DESCRIPTOR_REVISION)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "InitializeSecurityDescriptor failure: "
                          "lasterror = %d \n", GetLastError());
    }
...
static LPSECURITY_ATTRIBUTES make_user_everybody_security_attr(DWORD umask, DWORD emask) {
  ...
  if (!AllocateAndInitializeSid( &SIDAuth, 1, SECURITY_WORLD_RID,
    ...
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "AllocateAndInitializeSid failure: "
                          "lasterror = %d \n", GetLastError());
    }
    return NULL;
  }
  ...
  if (!CreateDirectory(dirname, pDirSA)) {
    ...
    if (lasterror == ERROR_ALREADY_EXISTS) {
      ...
      if (!is_directory_secure(dirname)) {
        ...
        if (PrintMiscellaneous && Verbose) {
          jio_fprintf(stderr, "%s directory is insecure\n", dirname);
        }
        return false;
      }
    }
      ...
      if (PrintMiscellaneous && Verbose) {
        jio_fprintf(stderr, "CreateDirectory failed: %d\n", GetLastError());
      }
      return false;
    }
  }
...
static HANDLE create_sharedmem_resources(const char* dirname, const char* filename, const char* objectname, size_t size) {
  ...
  if (fh == INVALID_HANDLE_VALUE) {
    DWORD lasterror = GetLastError();
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "could not create file %s: %d\n",
                          filename, lasterror);
    }
    return NULL;
  }
...
static HANDLE open_sharedmem_object(const char* objectname, DWORD ofm_access, TRAPS) {
  ...
  if (fmh == NULL) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "OpenFileMapping failed for shared memory object %s:"
...
static char* mapping_create_shared(size_t size) {
  ...
  if (mapAddress == NULL) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "MapViewOfFile failed, lasterror = %d\n",
                          GetLastError());
    }
...
static size_t sharedmem_filesize(const char* filename, TRAPS) {
  ...
  if (::stat(filename, &statbuf) == OS_ERR) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "stat %s failed: %s\n", filename, strerror(errno));
    }
  ...
  if ((statbuf.st_size == 0) || (statbuf.st_size % os::vm_page_size() != 0)) {
    if (PrintMiscellaneous && Verbose) {
      jio_fprintf(stderr, "unexpected file size: size = " SIZE_FORMAT "\n",
                          statbuf.st_size);
    }
...
static void open_file_mapping(const char* user, int vmid,
  ...
  *sizep = size;
  ...
  if (PerfTraceMemOps) {
    jio_fprintf(stderr, "mapped " SIZE_FORMAT " bytes for vmid %d at "
...
void PerfMemory::create_memory_region(size_t size) {
  ...
  if (PerfDisableSharedMem || !os::win32::is_nt()) {
    ...
    if (_start == NULL) {
      ...
      //
      if (PrintMiscellaneous && Verbose) {
        jio_fprintf(stderr, "Reverting to non-shared PerfMemory region.\n");
      }
      PerfDisableSharedMem = true;
      _start = create_standard_memory(size);
    }
