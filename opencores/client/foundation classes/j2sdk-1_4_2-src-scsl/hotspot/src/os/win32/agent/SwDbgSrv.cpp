...
#pragma ident "@(#)SwDbgSrv.cpp	1.7 03/01/23 11:06:48 JVM"
#endif
/*
...
startChildProcess(DWORD pidToDebug,
  ...
  // Create an event used by the child process to signal the parent
  // process when a write has completed
  char eventName[256];
  sprintf(eventName, "SwDbgSrvEvent%u", pidToDebug);
...
sendMessage(ChildInfo* child, Message* message) {
  ResetEvent(child->getChildWriteDoneEvent());
...
forwardReplyToClient(ChildInfo* child, ClientInfo* client) {
  ...
  while (true) {
    // Wait for some data to show up or for the write to be signaled
    // as done
    DWORD numAvail;
    PeekNamedPipe(child->getReadFromStdoutHandle(), NULL, 0, NULL, &numAvail, NULL);
    if (numAvail == 0) {
      // See whether we're done
      DWORD res = WaitForSingleObject(child->getChildWriteDoneEvent(), 0);
      if (res == WAIT_OBJECT_0) {
        ResetEvent(child->getChildWriteDoneEvent());
        if (!client->getIOBuf()->flush()) {
#ifdef DEBUGGING
          cerr << "Forward failed because final flush failed" << endl;
#endif
          return false;
        }
#ifdef DEBUGGING
        cerr << "Successfully forwarded " << total << " bytes to client" << endl;
#endif
        return true;
      } else {
        if (res != WAIT_TIMEOUT) {
...
#ifdef DEBUGGING
          cerr << "Forward failed because event wait failed" << endl;
#endif
          return false;
        }
      ...
      // Some data is available. Copy it using the generic copy routine.
      DWORD temp;
      if (!client->getIOBuf()->fillFromFileHandle(child->getReadFromStdoutHandle(),
                                                  &temp)) {
        ResetEvent(child->getChildWriteDoneEvent());
#ifdef DEBUGGING
          cerr << "Forward failed because IOBuf::fillFromFileHandle failed" << endl;
#endif
        return false;
      }
      total += temp;
