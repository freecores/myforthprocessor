...
#pragma ident "@(#)SwDbgSub.cpp	1.8 03/01/23 11:06:51 JVM"
#endif
/*
...
#ifdef DEBUGGING
    printError("peek's SetEvent() failed");
#endif
  }
  delete[] memBuf;
#ifdef DEBUGGING
...
main(int argc, char **argv)
{
  ...
  // Obtain event handle used to signal parent process when writes are
  // complete
  char eventName[256];
  sprintf(eventName, "SwDbgSrvEvent%u", pid);
  writeEvent = OpenEvent(EVENT_ALL_ACCESS, FALSE, eventName);
  if (writeEvent == NULL) {
    // Failed. Tell the server.
    char res = 0;
    DWORD numBytes;
