...
public class Realm implements Cloneable {
	...
	if ((realm != null && other.realm == null) ||
     ...
     * in turn to IBM.COM.
     *
     * NOTE: This is my current understanding of [capaths].
     *       It is possible that my understanding is completely wrong!!!
     */
    ...
    private static String[] parseCapaths(String cRealm, String sRealm) throws KrbException {
        ...
        try {
            retList = (String[]) tempList.toArray(retList);
    ...
    /*****
     * Test getRealmsList.
     * Usage: Realm crealm srealm
     * Usage: Realm [<number_of_iterations>]
     * The second case chooses client and server realms
     * at random from an internal list of realms.
     * The number of times this is done 
     * <number_of_iterations>defaults to 10.
     *****/
    ...
    public static void main(String[] args) throws KrbException {
        if (false) {
	    String usage = "Usage: Realm cRealm sRealm\n \t Realm #_of_iterations";
	    boolean dbg = Krb5.DEBUG;
	    ...
	    if (args.length == 2)
		{
		    ...
		    list = getRealmsList(args[0], args[1]);
	    ...
	    else
		{
		    ...
		    // String arrays for client and server realms
		    
		    String[] realms = {
			"JAVASECURITY.AUSTIN.NA.TIVOLI.COM",		// 0
			"GORDODOM.AUSTIN.NA.TIVOLI.COM",			// 1
			"DKHLINUX.AUSTIN.IBM.COM",				// 2
			"aixdce.connectathon.org",				// 3
			"aixkrb.connectathon.org",				// 4
			"ATHENA.MIT.EDU",					// 5
			"CYGNUS.COM",					// 6
			"SEAM1.COM",					// 7
			"LDAPAIXC1.AUSTIN.IBM.COM",			// 8
			"LDAPFVT2.AUSTIN.IBM.COM",				// 9
			"VIVACE.RALEIGH.IBM.COM",				// 10
			"MONOLOTHIC1",					// 11
			"MONOLOTHIC1.EDU",					// 12
			"MONOLOTHIC2",					// 13
			"MONOLOTHIC2.EDU",					// 14
			"JAVASECURITY.TIVOLI.COM",				// 15
			"GORDODOM.TIVOLI.COM",				// 16
			"DKHLINUX.IBM.COM",				// 17
			"VIVACE.IBM.COM",					// 18
			"LAB.JAVASECURITY.TIVOLI.COM",			// 19
			"BP5.JAVASECURITY.AUSTIN.NA.TIVOLI.COM",		// 20
			"F2.GORDODOM.AUSTIN.NA.TIVOLI.COM",		// 21
			"F3.F2.GORDODOM.AUSTIN.NA.TIVOLI.COM",		// 22
			"F3.GORDODOM.AUSTIN.NA.TIVOLI.ORG",		// 23
			"F3.GORDODOM.AUSTIN.NA.TIVOLI.COM",		// 24
			"sj.connectathon.org",				// 25
			/*
			 * Capaths
			 * Including these entries distorts the
			 * distribution of realms - results in too
			 * many direct cross-realm cases.
			 *
			 "ANL.GOV",						// 26
			 "PNL.GOV",						// 27
			 "ES.NET",						// 28
			...
			*/
		    ...
		    while (i++ < iterations)
			{
			    cIndex = getRandIndex(realms.length); 
			    sIndex = getRandIndex(realms.length); 
			    
			    if (dbg)
				System.out.println("["+ i +"]" + " cIndex=" + cIndex + ", sIndex=" + sIndex);
			    
			    cRealm = realms[cIndex];
			    sRealm = realms[sIndex];
			    
			    if (!dbg)
				{
				    System.out.println("Run #: " + i);
				    System.out.println("Client realm: " + cRealm);
				    System.out.println("Server realm: " + sRealm);
				}
			    
			    list = getRealmsList(cRealm, sRealm);
