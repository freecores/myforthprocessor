...
/*
 * @(#)EncryptedData.java	1.7 03/06/24
 *
...
public class EncryptedData implements Cloneable {
    ...
    public static final int KERB_ETYPE_RC4_HMAC = 23;    // 23                                                       
    public static final int KERB_ETYPE_RC4_HMAC_EXP = 24; // 24                                                       
	...
	throws KdcErrException, KrbApErrException, KrbCryptoException {
	    // should check for matching eType and kvno here
	    EType etypeEngine = EType.getInstance(eType);
	    etypeEngine.decrypt(cipher, key.getBytes());
	    plain = cipher;
	    cipher = null;
	...
	throws KdcErrException, KrbApErrException, KrbCryptoException {
	    // should check for matching eType and kvno here
	    EType etypeEngine = EType.getInstance(eType);
	    etypeEngine.decrypt(cipher, key.getBytes(), ivec);
	    plain = cipher;
	    cipher = null;
	    return etypeEngine.decryptedData(plain);
	}
	...
	throws KdcErrException, KrbApErrException, KrbCryptoException {
	    EncryptionKey key = new EncryptionKey(password);
	    // should check for matching eType here
	    EType etypeEngine = EType.getInstance(eType);
	    etypeEngine.decrypt(cipher, key.getBytes());
	    plain = cipher;
	    cipher = null;
     ...
     *
     */
    public EncryptedData(DerValue encoding) throws Asn1Exception, IOException { 
	DerValue der = null;
	...
	if (kvno != null) {
	    temp.putInteger(BigInteger.valueOf(this.kvno.intValue()));
	    bytes.write(DerValue.createTag(DerValue.TAG_CONTEXT, 
					   true, (byte)0x01), temp);
	    temp = new DerOutputStream();
	}
