 ...
 *  addresses[9] HostAddresses OPTIONAL, --Encrypted AuthorizationData encoding
 *	enc-authorization-data[10] EcryptedData OPTIONAL, --Encrypted AuthorizationData encoding
		...
		if ((der.getTag() & (byte)0x1F) == (byte)0x08) {
			...
			if (subDer.getTag() == DerValue.tag_SequenceOf) {				
				while(subDer.getData().available() > 0) {
					v.addElement(new Integer(subDer.getData().getBigInteger().intValue()));
				}
				eType = new int[v.size()];
				for (int i = 0; i < v.size(); i++) {
					eType[i] = (((Integer)v.elementAt(i)).intValue());
				}
			}
            else
				throw new Asn1Exception(Krb5.ASN1_BAD_ID);			
