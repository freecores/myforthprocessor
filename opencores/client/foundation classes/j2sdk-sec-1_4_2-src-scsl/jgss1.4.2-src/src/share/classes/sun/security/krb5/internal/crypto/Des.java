...
public final class Des {
	...
	try {
	    CharToByteConverter cbc = CharToByteConverter.getDefault();
	    cbytes = cbc.convertAll(passwdChars);
	} catch (Exception e) {
    ...
    public static byte[] string_to_key_bytes(StringBuffer passwd) throws KrbCryptoException {
	char[] passwdChars = new char[passwd.length()];
	passwd.getChars(0, passwd.length(), passwdChars, 0);
	byte[] key = long2octet(char_to_key(passwdChars));
	// clear the password
	if (passwd != null) {
