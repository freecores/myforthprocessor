...
/*
 * @(#)FileCredentialsCache.java	1.17 03/06/24
 *
...
public class FileCredentialsCache extends CredentialsCache
    ...
    private synchronized static boolean needsChmod() {
        if (!loadRequired) {
	   return false; // Already determined that load not necessary
        }

        if (loadedLibrary) {
	   return true;  // Library needed and loaded
        }

        // Determine whether load is required
        String os_name = (String)java.security.AccessController.doPrivileged(
	                 new sun.security.action.GetPropertyAction("os.name"));
        loadRequired = os_name.startsWith("Linux");

        if (loadRequired) {
	   // Load library and set loadedLibrary
	   java.security.AccessController.doPrivileged(
	        new sun.security.action.LoadLibraryAction("native_chmod"));
	   loadedLibrary = true;
        }
        return loadedLibrary;
    }

    public static synchronized FileCredentialsCache acquireInstance(PrincipalName principal,
								    String cache) {
        try {
    ...
    static synchronized FileCredentialsCache New(PrincipalName principal, String name) {
		...
		// invalid cache name or the file doesn't exist
            ...
            if (needsChmod()) {
               int result = chmod(cacheName, USER_READ_WRITE);
               if (result != 0) {
		  if (DEBUG)
    ...
    public synchronized void update(Credentials c) {
		...
		for (int i = 0; i < credentialsList.size(); i++) {
		    tmp = (Credentials)(credentialsList.elementAt(i));
	...
	if ((credentialsList == null) || (credentialsList.isEmpty())) {
	    ...
	    for (int i = 0; i < credentialsList.size(); i++) {
		tmp[i] = (Credentials)(credentialsList.elementAt(i));
	    }
	    return tmp;
	}
    ...
    public Credentials getDefaultCreds() {
		...
		// TBD: Does this work with cross realm?
	...
	if (osname != null) { 
		...
		try {
		    ...
		    Method method = c.getMethod("getUid",null);
		    uid =  ((Long)method.invoke(obj,null)).longValue();
		    ...
		    if (DEBUG) {
			System.out.println ("Exception in obtaininig uid "+
					    "for Unix platforms " +
	...
	try {
	    ...
	    Process p =
		(Process)java.security.AccessController.doPrivileged
