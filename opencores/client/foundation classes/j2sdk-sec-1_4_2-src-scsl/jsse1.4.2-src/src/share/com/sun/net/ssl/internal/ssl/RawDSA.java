 ...
 * Algorithm), as described in FIPS 186 of the National Instute of
 * Standards and Technology (NIST), using FIPS 180-1 (SHA-1).
 *
 * <p>This version creates and verifies DSA signatures based on the 
 * pre-computed SHA-1 digest of the input data, i.e., it assumes that the
 * input passed to it already represents the SHA-1 digest of the data to be
 * signed or verfied, respectively.
 *
...
final class RawDSA extends Signature {
    ...
    // The SHA1 digest
    private byte[] sha1Data;

    // The index into the digest data
    private int sha1Index = 0;
    ...
    /**
     * Get an implementation for the RawDSA signature. JCA provider
     * implementation if possible, internal implementation otherwise.
     *
     * Note: the NoSuchAlgorithmException is declared but currently
     * never thrown. This may change in future versions.
     */
	...
	BigInteger temp = new BigInteger(1, sha1Data);
   ...
   static int[] SHA_7(int [] m1, int[] h) {
	...
	if (len > (sha1Data.length - sha1Index))
	    throw new IllegalArgumentException("SHA1 digest too long");
	System.arraycopy(data, off, sha1Data, sha1Index, len);
	sha1Index += len;
    }
	...
	String printable = "RawDSA Signature";
