 ...
 *
 * @version 1.68, 06/24/03
 * @author David Brownell
 */
final class InputRecord extends ByteArrayInputStream implements Record {

    private HandshakeHash	handshakeHash;
    private int			lastHashed;
    private boolean		formatVerified = true;	// SSLv2 ruled out?
    ...
    void decrypt(CipherBox engine) throws BadPaddingException {
	int len = count - headerSize;
	count = headerSize + engine.decrypt(buf, headerSize, len);
    }
     ...
     */
    public void queueHandshake(InputRecord r) throws IOException {
	int len;
    ...
    private static int readFully(InputStream s, byte b[], int off, int len)
	    throws IOException {
	int n = 0;
	while (n < len) {
     ...
     */
    public void read(InputStream s, OutputStream o) throws IOException {
	if (isClosed) {
	    return;
	}
	 ...
	 */
	int really = readFully(s, buf, 0, headerSize);
	if (really == -1) {
	    throw new EOFException("SSL peer shut down incorrectly");
	} else if (really != headerSize) {
	    throw new SSLProtocolException(
		"bad header read; count = " + really);
	}
	pos = headerSize;
	lastHashed = pos;
	 ...
	 */
	count = ((buf[3] & 0x0ff) << 8) + (buf[4] & 0xff);

	/*
	 * Check for upper bound should really be if count is greater
	 * than maxRecordSize minus headerSize but see comment below.
	 * Checking for Short.MAX_VALUE is an arbitrarily chosen limit.
	 * Perhaps we should do some experiments to determine the actual
	 * maximum record size that Microsoft's SSL will send.
	 */
	 ...
	 */
	if (count > buf.length - headerSize) {
	    byte[] newbuf = new byte[count + headerSize];
	    System.arraycopy(buf, 0, newbuf, 0, headerSize);
	    buf = newbuf;
	}

	int really = readFully(s, buf, headerSize, count);
	if (really != count) {
	...
	if (((buf[0] & 0x080) != 0) && buf[2] == 1) {
		...
		(buf[1] & 0xff) - 3;
	    byte[] v2Msg = new byte[len];
	    int really = readFully(s, v2Msg, 0, len);
	    if (really == -1) {
		throw new EOFException("SSL peer shut down incorrectly");
	    } else if (really != len) {
		throw new SSLProtocolException(
		    "Bad SSL v2.0 ClientHello record");
	    }
	    hashInternal(buf, 2, 3);
	    hashInternal(v2Msg, 0, len);
	    V2toV3ClientHello(v2Msg);
	    lastHashed = count;
