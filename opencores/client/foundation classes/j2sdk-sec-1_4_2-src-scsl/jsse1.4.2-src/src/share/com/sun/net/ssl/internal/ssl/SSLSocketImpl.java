 ...
 * @see SSLServerSocket
 *
 * @version 1.155 06/24/03
 * @author David Brownell
 */
final class SSLSocketImpl extends BaseSSLSocketImpl {
     ...
     *
     * The closeLock is held while close() is closing the connection.
     * It is used to force secondary invokers of close() that need to
     * take action to wait until the primary invoker has completed
     * the close().
     */
	...
	while (r.contentType() == Record.ct_application_data) {
	    ...
	    case cs_ERROR:
		fatal(alert_close_notify, "error while writing to socket");
		break; // dummy
	...
	while (((state = getConnectionState()) != cs_CLOSED) &&
		...
		try {
		    fatal(alert_unexpected_message, e);
		} catch (IOException x) {
		...
		if (r.contentType() == Record.ct_handshake) {
		    ...
		    fatal(alert_bad_record_mac, "bad record MAC");
		}
	    }

	    // if (!r.decompress(c))
	    //     fatal(alert_decompression_failure, "decompression failure");
		...
		case Record.ct_handshake:
		    ...
		    // we received a handshake message. The handshaker state
		    // machine will ensure that it's a finished message.
		    expectingFinished = false;
		    /*
		     * process the handshake record ... may contain just
		     * a partial handshake message or multiple messages.
		     */
		    handshaker.process_record(r);
		...
		default:
		    ...
		    //
		    if (debug != null && Debug.isOn("ssl")) {
			System.out.println("Received record type: "
				+ r.contentType());
		    }
		    continue;
	      } // switch
	...
	try {
	    ...
	    /*
	     * If we sent a close, then recvAlert(close_notify) would
	     * cause close to be called again while in cs_SENT_CLOSE state.
	     */
	    case cs_SENT_CLOSE:
		if (debug != null && Debug.isOn("ssl")) {
		    System.out.println("close invoked again; state = " +
			getConnectionState());
		}
		if (selfInitiated) {
		    // Wait for primary invoker of close() to be done
		    synchronized (closeLock) {

			// Don't need to do anything more here because
			// we can reach this point iff the primary invoker
			// of close() has released closeLock.

			// Should add assertion that
			// state == cs_CLOSED || state == cs_ERROR

			if ((debug != null) && Debug.isOn("ssl")) {
			    System.out.println("after primary close; state = " +
				getConnectionState());
			}
		    }
		}
		...
		synchronized (closeLock) {
		    ...
		    // Make sure we do this atomically
		    synchronized (this) {
			warning(alert_close_notify);
			connectionState = cs_SENT_CLOSE;
		    }
		    if (self == this) {
			super.close();
		    ...
		    // Set state to cs_CLOSED before releasing closeLock
		    setConnectionState(cs_CLOSED);
		 }
		 break;
	    }
	} finally {
	    // Upon exit from this method, the state is always >= cs_CLOSED
	    int newState = (state == cs_APP_CLOSED) ? cs_APP_CLOSED
						    : cs_CLOSED;
	...
	if (debug != null && Debug.isOn("ssl")) {
	    System.out.println("waiting for close_notify or alert: state "
		+ getConnectionState());
	}
	...
	try {
	    ...
	    if (debug != null && Debug.isOn("ssl")) {
		System.out.println("Exception while waiting for close " +e);
	    }
	    if (rethrow) {
     ...
     */
    synchronized void handleException(Exception e) throws IOException {
	if ((debug != null) && Debug.isOn("ssl")) {
	    System.out.println(threadName()
			+ ", handling exception: " + e.toString());
	}
	if (e instanceof InterruptedIOException) { // SocketTimeoutException
	    // don't close the Socket in case of timeouts or interrupts
	    // we don't really support them, but we don't want to
	    // unneccessarily break applications by closing the socket
	    throw (IOException)e;
	}
	...
	if (closeReason != null) {
		...
		throw getSSLException(alert_internal_error, e,
	...
	if (isSSLException) {
		...
		alertType = alert_handshake_failure;
	    } else {
		alertType = alert_unexpected_message;
	    }
	} else {
	    alertType = alert_internal_error;
	}
	fatal(alertType, e);
    }
	...
	sendAlert(alert_warning, description);
    }
	 ...
	 */
	if (closeReason == null) {
	    sendAlert(alert_fatal, description);
	    if (cause instanceof SSLException) { // only true if != null
		closeReason = (SSLException)cause;
	    } else {
		closeReason = getSSLException(description, cause, diagnostic);
	    }
	}
	...
	if (description == -1) { // check for short message
	    fatal(alert_illegal_parameter, "Short alert message");
	}
	...
	if (debug != null && (Debug.isOn("record") ||
		...
		System.out.println(alertDescription(description));
	    }
	}
	...
	if (level == alert_warning) {
	    ...
	    if (closeReason == null) {
		closeReason = getSSLException(description, reason);
	    }
	    fatal(alert_unexpected_message, reason);
	}
    }
	...
	try {
	    ...
	    if (useDebug) {
		System.out.println("Exception sending alert: " + e);
	    }
	}
    }
	...
	if (getConnectionState() == cs_HANDSHAKE) {
		...
		if (debug != null && Debug.isOn("handshake")) {
		      System.out.println("IOException in getSession():  " + e);
		}
	    }
	}
	synchronized (this) {
	...
	doClientAuth = (flag ?
	    SSLSocketImpl.clauth_required : SSLSocketImpl.clauth_none);
	...
	return (doClientAuth == SSLSocketImpl.clauth_required);
    }
	...
	doClientAuth = (flag ?
	    SSLSocketImpl.clauth_requested : SSLSocketImpl.clauth_none);
	...
	return (doClientAuth == SSLSocketImpl.clauth_requested);
    }
	...
	default:
	    /*
	     * For the time being, you can't shift between
	     * modes once SSL traffic has started.  We may
	     * decide later to allow this on a renegotiation.
	     */
	    if (debug != null && Debug.isOn("ssl")) {
		System.out.println(
	...
	if ((debug != null) && Debug.isOn("ssl")) {
	    System.out.println("setSoTimeout(" + timeout + ") called");
	}
	if (self == this) {
	...
	private Set targets;	// who gets notified
	private HandshakeCompletedEvent	event;		// the notification

	NotifyHandshakeThread(Set entrySet, HandshakeCompletedEvent e) {
	...
	public void run() {
		...
		final HandshakeCompletedListener l =
		    (HandshakeCompletedListener) entry.getKey();
