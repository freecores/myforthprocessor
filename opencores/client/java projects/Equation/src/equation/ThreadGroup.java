...
class ThreadGroup {
     ...
     */
    public ThreadGroup() {	// called from FORTH code
	this.name = "system";
    ...
    private void arraycopy(ThreadGroup [] groups, int begin, ThreadGroup [] groupsSnapshot, int offset, int len)
     {
      for(int i = offset; i < len;)
       groupsSnapshot[i++] = groups[begin++];
     }

    private void arraycopy(Thread [] groups, int begin, Thread [] groupsSnapshot, int offset, int len)
     {
      for(int i = offset; i < len;)
       groupsSnapshot[i++] = groups[begin++];
     }
	...
	synchronized (this) {
		...
		arraycopy(groups, 0, newgroups, 0, ngroups);
		groups = newgroups;
	    }
	    groups[ngroups] = g;
	...
	synchronized (this) {
		...
		if (groups[i] == g) {
		    ngroups -= 1;
		    arraycopy(groups, i + 1, groups, i, ngroups - i);
	...
	synchronized (this) {
		...
		arraycopy(threads, 0, newthreads, 0, nthreads);
		threads = newthreads;
	    }
	    threads[nthreads] = t;
	...
	synchronized (this) {
	    ...
	    for (int i = 0 ; i < nthreads ; i++) {
		if (threads[i] == t) {
		    arraycopy(threads, i + 1, threads, i, --nthreads - i);
...
/*    void list(PrintStream out, int indent) {
	...
	return "ThreadGroup[name=" + getName() + ",maxpri=" + maxPriority + "]";
    }
}
