    ...
    implements /*java.io.Serializable,*/ Comparable, CharSequence
{
    ...
    public String(String original) {
 	...
 	if (original.value.length > this.count) {
 	    ...
 	    arraycopy(original.value, original.offset,
   ...
   private static void arraycopy(char [] svalue, int soffset, char [] dvalue, int doffset, int count)
    {
     if (count > 0)
      {
       int source = ( (JavaArray) svalue).getElem(soffset);
       int dest = ( (JavaArray) dvalue).getElem(doffset);
       #ass "source dest count 2* MOVE";
      }
    }
...
*/
	...
	//checkBounds(bytes, offset, length);
        value = new char[length - offset + 1];
	count = value.length;
...
*/
	...
	scan: {
        ...
        arraycopy(val, off, result, 0, firstUpper);

            // normal, fast loop
            for (int i = firstUpper; i < len; ++i) {
                result[i] = toLowerCase(val[off+i]);
            }
	...
	arraycopy(val, off, result, 0, firstLower);
