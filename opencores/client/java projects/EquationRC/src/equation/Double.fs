...
:LOCAL Double.Double§1485259776
   ...
   s DUP INCREFERENCE
   std2156 VAL
   Double.valueOf§-1535871488 DUP TO 0§
   A:R@
...
:LOCAL Double.byteValue§-1994235392
   ...
   \ new statement
   §this CELL+ @ 28 + ( equation.Double.value )    2@ A:R@  F>D NIP 127 U>! NIP
   IF
      0FFFFFF00H OR
   ENDIF
   A:DROP
...
: Double.classMonitorEnter§-433167616
...
: Double.classMonitorLeave§-1691393280
...
:LOCAL Double.compareTo§1893819136
   ...
   \ new statement
   anotherDouble TRUE JavaArray.kill§1620077312
   PURGE 2
...
:LOCAL Double.doubleValue§-330275840
   ...
   \ new statement
   §this CELL+ @ 28 + ( equation.Double.value )    2@ A:R@
   A:DROP
   0 §break2129 BRANCH
...
:LOCAL Double.intValue§-370906880
   ...
   \ new statement
   §this CELL+ @ 28 + ( equation.Double.value )    2@ A:R@  F>D D>S
   A:DROP
   0 §break2127 BRANCH
...
:LOCAL Double.longValue§1506831360
   ...
   \ new statement
   §this CELL+ @ 28 + ( equation.Double.value )    2@ A:R@  F>D
   A:DROP
   0 §break2128 BRANCH
...
: Double.parseDouble§1159491584
   ...
   s A:R@
   R> -1067748352 TRUE ( equation.String.trim§-1067748352 ) EXECUTE-METHOD DUP TO 0§
   A:R@
   R> -601004288 TRUE ( equation.String.getBytes§-601004288 ) EXECUTE-METHOD
   b TRUE JavaArray.kill§1620077312 TO b 0§ TRUE JavaArray.kill§1620077312
...
:LOCAL Double.shortValue§715813632
   ...
   \ new statement
   §this CELL+ @ 28 + ( equation.Double.value )    2@ A:R@  F>D NIP 32767 U>! NIP
   IF
      0FFFF0000H OR
   ENDIF
   A:DROP
...
:LOCAL Double.~destructor§-1086882304
   LOCAL §this
...
VARIABLE Double._staticThread
2VARIABLE Double.one
2VARIABLE Double.serialVersionUID
2VARIABLE Double.zero

A:HERE VARIABLE Double§-1583528960.table 16 DUP 2* CELLS ALLOT R@ ! A:CELL+
...
-1086882304 R@ ! A:CELL+ Double.~destructor§-1086882304 VAL R@ ! A:CELL+
A:DROP
